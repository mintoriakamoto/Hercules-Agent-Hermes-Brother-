# OpenAI-Compatible API Server for Hercules Agent
...
SSE streaming. By exposing this endpoint, hercules-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  hercules-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.hercules/.env`
2. Run `hercules gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with hercules-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns hercules-agent as a model) |
...
Authorization: Bearer hercules-api-key-here
...
But hercules-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "hercules-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured hercules-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of hercules-agent's core prompt
...
Once implemented, hercules-agent works as a drop-in backend for:
