...
//!
//! Driven when the installer is launched as `Hercules-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Hercules desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `hercules update`
//!      or repair bootstrap can race locked files),
//!   2. run `hercules update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in hercules_cli/main.py),
//!   3. run `hercules desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `hercules update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_hercules) and
...
/// `hercules update` exit code meaning "another hercules process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// hercules_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `hercules update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let hercules_home = crate::paths::hercules_home();
    let install_root = hercules_home.join("hercules-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_hercules`
    ...
    let hercules = resolve_hercules(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the hercules CLI under {}. Is Hercules installed? \
    ...
    // async on Windows. If it still holds the venv shim, `hercules update`
    ...
    // ---- stage 2: hercules update -----------------------------------------
    // Pass --branch so `hercules update` targets the branch this installer was
    ...
    // without --branch, `hercules update` switches the checkout to `main` (a
    ...
    // --force skips `hercules update`'s Windows running-exe guard (which would
    ...
    // time `hercules update` runs there is no legitimate hercules.exe to protect,
    // and the guard would only produce a false "Hercules is still running" stop.
    //
    ...
    // probe only checks the hercules.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Hercules windows" message.
    ...
    // Retry-once for the update-boundary crash. `hercules update` lazily imports
    ...
    // second `hercules update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Hercules" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Hercules is still running. Close all Hercules windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "hercules update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::hercules_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: hercules desktop --build-only ----------------------------
    // `hercules update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `hercules update` already does above, and `hercules update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `hercules desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Hercules manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_hercules_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Hercules to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend hercules.exe (or the desktop Hercules.exe
            ...
            // force-kill every Hercules.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Hercules still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_hercules();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_hercules(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Hercules.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Hercules.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `hercules.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `hercules.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\hercules.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startHercules()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// hercules.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named hercules.exe.)
fn force_kill_other_hercules() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `hercules <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv hercules shim under an install root, regardless of existence.
fn venv_hercules(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("hercules.exe")
    } else {
        install_root.join("venv").join("bin").join("hercules")
    }
}

/// Resolve the hercules CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `hercules` on PATH.
fn resolve_hercules(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_hercules(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "hercules.exe" } else { "hercules" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let hercules_home = crate::paths::hercules_home();
    let mut envs = vec![(
        "HERCULES_HOME".to_string(),
        hercules_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        hercules_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hercules_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Hercules.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.hercules-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.hercules-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::hercules_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_hercules_is_under_install_root() {
        let root = Path::new("/x/hercules-agent");
        let shim = venv_hercules(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_hercules(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/hercules-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".hercules-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Hercules.app"]),
            Some(PathBuf::from("/Applications/Hercules.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Hercules.app");
        let tmp = base.join("Hercules.app.hercules-update-new");
        let old = base.join("Hercules.app.hercules-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Hercules.app");
        let tmp = base.join("Hercules.app.hercules-update-new"); // intentionally absent
        let old = base.join("Hercules.app.hercules-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Hercules.app");
        let tmp = base.join("Hercules.app.hercules-update-new"); // absent
        let old = base.join("Hercules.app.hercules-update-old");
        write_marker(&target, "OLD");
