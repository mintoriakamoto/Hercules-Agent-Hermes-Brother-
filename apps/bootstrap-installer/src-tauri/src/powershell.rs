...
///
/// `hercules_home_override` propagates to the child as $HERCULES_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, hercules_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = hercules_home_override {
        cmd.env("HERCULES_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, hercules_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = hercules_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_hercules_home() {
