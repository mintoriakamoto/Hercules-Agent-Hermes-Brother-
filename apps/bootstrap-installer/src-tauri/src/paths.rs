...
//!
//! Mirrors `hercules_constants.get_hercules_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\hercules
//!   macOS:   ~/.hercules
//!   Linux:   ~/.hercules  (override via $HERCULES_HOME)
//!
//! NOTE (macOS): Python's get_hercules_home(), scripts/install.sh, and the
//! Electron desktop's resolveHerculesHome() ALL use ~/.hercules on macOS — there
...
/// Returns the canonical Hercules home directory, respecting $HERCULES_HOME if set.
pub fn hercules_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("HERCULES_HOME") {
    ...
    {
        // %LOCALAPPDATA%\hercules — matches scripts/install.ps1's $HerculesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("hercules");
        }
    }

    // macOS + Linux + fallback: ~/.hercules (matches Python get_hercules_home(),
    // install.sh, and the Electron desktop's resolveHerculesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".hercules");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".hercules")
}

pub fn log_dir() -> PathBuf {
    hercules_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    hercules_home().join("bootstrap-cache")
}
...
/// HERCULES_HOME so it survives repo checkout deletion (unlike anything under
/// hercules-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\hercules\hercules-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_hercules`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under HERCULES_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves HERCULES_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    hercules_home().join(".hercules-update-in-progress")
}
...
pub fn copy_self_to_hercules_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to HERCULES_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_HERCULES_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".hercules-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under HERCULES_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[hercules-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("HERCULES_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_hercules_home() -> String {
    hercules_home().to_string_lossy().into_owned()
}
