//! Hercules Setup — Tauri entrypoint.
//!
...
pub fn run() {
    // Tracing → bootstrap-installer.log under HERCULES_HOME/logs/ so install
    ...
    // Hercules is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Hercules installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Hercules is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Hercules" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Hercules" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_hercules_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hercules_home().join("hercules-agent");
                if bootstrap::hercules_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_hercules_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "hercules already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_hercules_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_hercules_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running Hercules Setup");
}
