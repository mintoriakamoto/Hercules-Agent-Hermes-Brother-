...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $HERCULES_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Hercules-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Hercules-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("HERCULES_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/NousResearch/hercules-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "hercules-setup/0.0.1")
        .send()
        .await
