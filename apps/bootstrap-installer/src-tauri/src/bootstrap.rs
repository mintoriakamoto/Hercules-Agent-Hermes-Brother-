...
//! HERCULES_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for HERCULES_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub hercules_home: Option<String>,
}
...
/// Spawn the locally-built Hercules desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_hercules_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_hercules_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Hercules desktop at {}. The desktop build step \
             may have been skipped or failed. Run `hercules desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Hercules desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Hercules
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_hercules_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Hercules.exe"),
            ("win-arm64-unpacked", "Hercules.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Hercules.app/Contents/MacOS", "Hercules"),
            ("mac-arm64/Hercules.app/Contents/MacOS", "Hercules"),
        ]
    } else {
        &[("linux-unpacked", "hercules")]
    };
...
pub(crate) fn resolve_hercules_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_hercules_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Hercules.app/Contents/MacOS/Hercules -> .../Hercules.app
...
/// path so a bare re-open just opens Hercules instead of re-running setup.
pub(crate) fn hercules_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".hercules-bootstrap-complete").exists()
        && resolve_hercules_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hercules_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Hercules desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_hercules_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::hercules_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $HerculesHome which Stage-Repository clones
    // the repo INTO at $HerculesHome\hercules-agent. Mirrors hercules_constants.
    let hercules_home = args
        .hercules_home
        .clone()
        .unwrap_or_else(|| crate::paths::hercules_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&hercules_home).join("hercules-agent");

    // Copy ourselves to HERCULES_HOME/hercules-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hercules_home() {
        tracing::warn!(?err, "failed to copy installer into HERCULES_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    hercules_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, hercules_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Hercules"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Hercules.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Hercules.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("hercules");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Hercules.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_hercules_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_hercules_desktop_app(&root)
    ...
    #[test]
    fn resolve_hercules_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_hercules_desktop_app(&root).is_none(),
