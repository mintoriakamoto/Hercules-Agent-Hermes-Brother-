...
import { launchHerculesDesktop } from '../store'
import { AlertCircle } from 'lucide-react'

/*
 * Success screen. HERCULES AGENT wordmark stays as the visual anchor
 ...
 * Hercules.exe doesn't exist). We catch the Tauri error and surface it
 * inline rather than silently doing nothing — the previous version
 * had `onClick={() => void launchHerculesDesktop()}` which swallowed
...
export default function Success() {
  ...
  return (
    <div className="hercules-fade-in flex h-full flex-col items-center justify-center gap-8 px-12 py-10">
      <div className="w-full max-w-2xl min-w-0 text-center">
        ...
        >
          <span>
            <span>Hercules is ready</span>
          </span>
          <span aria-hidden="true">Hercules is ready</span>
        </p>

        <p className="m-0 text-center text-base leading-normal tracking-tight text-muted-foreground">
          You can launch from here, or any time from your terminal with{' '}
          <code className="font-mono text-sm text-foreground/80">hercules desktop</code>.
        </p>
      </div>
