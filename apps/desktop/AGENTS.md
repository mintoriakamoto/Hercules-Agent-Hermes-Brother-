...
How to build Hercules Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Hercules surface can also
...
- A **runtime home change** (switching the underlying `HERCULES_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Hercules — do not assume
