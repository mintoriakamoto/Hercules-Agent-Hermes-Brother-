...
describe('Hercules REST session helpers', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'herculesDesktop')
  })
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getHerculesConfig, '/api/config'],
      [getHerculesConfigDefaults, '/api/config/defaults'],
