...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.herculesDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.herculesDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.herculesDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.herculesDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.herculesDesktop && text && normalizeExternalUrl(text) === target) {
