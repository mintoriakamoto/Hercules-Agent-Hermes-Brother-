...
const HERCULES_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type HerculesRefType = (typeof HERCULES_REF_TYPES)[number]
...
const ICON_PATHS: Record<HerculesRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as HerculesRefType] ?? ICON_FALLBACK
...
const HERCULES_DIRECTIVE_RE = new RegExp(
...
export const herculesDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(HERCULES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as HerculesRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: HerculesRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...herculesDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Hercules directives (`@file:...`, `@image:...`) as
