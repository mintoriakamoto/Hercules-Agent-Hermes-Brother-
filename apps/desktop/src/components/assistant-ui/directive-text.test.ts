...
import { formatRefValue, herculesDirectiveFormatter } from './directive-text'
...
describe('herculesDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = herculesDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = herculesDirectiveFormatter.parse('@file:src/main.tsx the entry point')
