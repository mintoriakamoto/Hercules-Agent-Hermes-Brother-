...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Hercules gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.herculesDesktop
  Object.defineProperty(window, 'herculesDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'herculesDesktop', { configurable: true, value: original })
}
