...
export function setZoomPercent(percent: number): void {
  window.herculesDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.herculesDesktop?.zoom) {
  void window.herculesDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.herculesDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
