import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@hercules/shared'
...
const isOpen = (gateway: HerculesGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<HerculesGateway | null>(null)
...
let primaryGateway: HerculesGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: HerculesGateway | null, profile = 'default'): void {
...
export function activeGateway(): HerculesGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<HerculesGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
