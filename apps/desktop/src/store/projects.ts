...
import type { HerculesGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/hercules'
...
const PROJECT_SCOPE_KEY = 'hercules.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Hercules gateway is not connected')
  }
...
// repo, returning where Hercules should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<HerculesGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.herculesDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.herculesDesktop?.writeClipboard?.(path)
  }
}
