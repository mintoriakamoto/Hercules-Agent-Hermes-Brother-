...
import type { OAuthProvider } from '@/types/hercules'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('hercules-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('hercules-desktop-onboarded-v1')).toBe('1')
  })
