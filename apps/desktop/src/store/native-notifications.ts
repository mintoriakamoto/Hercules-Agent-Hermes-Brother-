...
const STORAGE_KEY = 'hercules:native-notifications'
...
// "Backgrounded" = the user isn't on Hercules. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.herculesDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.herculesDesktop
