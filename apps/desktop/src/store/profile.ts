...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/hercules'
...
import type { ProfileInfo } from '@/types/hercules'
...
// /api/profiles/active `current`). "default" is the root ~/.hercules. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'hercules.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'hercules.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.herculesDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new HERCULES_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.herculesDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.herculesDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'hercules.desktop.showAllProfiles'
...
// Switch to the default (root ~/.hercules) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.herculesDesktop?.touchBackend?.(target).catch(() => undefined)
}
