...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.herculesDesktop?.openSessionWindow === 'function'
}
...
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.herculesDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.herculesDesktop.openNewSessionWindow(), 'Could not open new session window')
}
