...
import type { HerculesRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: HerculesRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<HerculesRepoStatus | null>) {
  ;(window as unknown as { herculesDesktop?: unknown }).herculesDesktop = { git: { repoStatus: impl } }
}
