...
const checkHerculesUpdateSpy = vi.fn()
const updateHerculesSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/hercules', () => ({
  checkHerculesUpdate: (...args: unknown[]) => checkHerculesUpdateSpy(...args),
  updateHercules: (...args: unknown[]) => updateHerculesSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkHerculesUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkHerculesUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'hercules update',
      message: null,
    ...
    expect(checkHerculesUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkHerculesUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      herculesDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'hercules update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('hercules update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Hercules to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateHerculesSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateHerculesSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkHerculesUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      herculesDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },
