...
import type { SessionInfo, UsageStats } from '@/types/hercules'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'hercules.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'hercules.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'hercules.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'hercules.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'hercules.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'hercules.desktop.lastSessionId'
...
function workspaceCwdKey(connection: HerculesConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.herculesDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.herculesDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<HerculesConnection | null>(null)
...
export const setConnection = (next: Updater<HerculesConnection | null>) => updateAtom($connection, next)
