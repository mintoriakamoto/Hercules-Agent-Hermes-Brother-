...
import type { HerculesReviewFile, HerculesReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Hercules' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'hercules.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'hercules.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'hercules.desktop.reviewTreeMode'
const SELECTED_KEY = 'hercules.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<HerculesReviewFile[]>([])
...
export const $reviewShipInfo = atom<HerculesReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['herculesDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: HerculesReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.herculesDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.herculesDesktop?.openExternal?.(url)
    }
