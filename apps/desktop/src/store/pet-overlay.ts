 ...
 * app's bounds and stays visible while Hercules is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'hercules.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'hercules.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.herculesDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.herculesDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.herculesDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.herculesDesktop?.petOverlay?.close()
}
