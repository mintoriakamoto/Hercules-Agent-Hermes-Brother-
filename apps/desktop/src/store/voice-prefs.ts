...
import { getHerculesConfigRecord, saveHerculesConfig } from '@/hercules'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getHerculesConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveHerculesConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
