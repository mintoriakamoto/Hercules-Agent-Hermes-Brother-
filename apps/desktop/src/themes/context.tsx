...
import { BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, herculesTheme } from './presets'
...
const SKIN_KEY = 'hercules-desktop-theme-v2'
const MODE_KEY = 'hercules-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'hercules-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'hercules-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'hercules-desktop-active-profile-v1'
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? herculesTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...herculesTheme.typography, ...theme.typography }
  ...
  root.dataset.herculesTheme = skinName
  root.dataset.herculesMode = rendered
  ...
  window.herculesDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('hercules-boot-background', chromeBg)
    window.localStorage.setItem('hercules-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.herculesThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.herculesDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Hercules backend theme on connect. */
