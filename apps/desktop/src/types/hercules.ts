...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Hercules doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in hercules_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `hercules model` uses — instead of
...
export type HerculesConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `hercules model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors hercules_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Hercules, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `hercules tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Hercules); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/hercules/update/check` — the backend's own update state.
