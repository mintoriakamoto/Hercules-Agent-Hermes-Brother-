import { JsonRpcGatewayClient } from '@hercules/shared'
...
// "Timed out connecting to Hercules backend" that hangs the UI (#48504).
//
...
export class HerculesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Hercules gateway connection closed',
      connectErrorMessage: 'Could not connect to Hercules gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Hercules gateway is not connected',
...
// the call; each pooled backend already has its own HERCULES_HOME, so no backend
...
export async function listSessions(
  ...
  const result = await window.herculesDesktop.api<PaginatedSessions>({
    path:
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.herculesDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.herculesDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.herculesDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(id: string, profile?: string | null): Promise<SessionMessagesResponse> {
  ...
  return window.herculesDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.herculesDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.herculesDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.herculesDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.herculesDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getHerculesConfig(): Promise<HerculesConfig> {
  return window.herculesDesktop.api<HerculesConfig>({
    ...profileScoped(),
    path: '/api/config',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
...
export function getHerculesConfigRecord(): Promise<HerculesConfigRecord> {
  return window.herculesDesktop.api<HerculesConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getHerculesConfigDefaults(): Promise<HerculesConfigRecord> {
  return window.herculesDesktop.api<HerculesConfigRecord>({
    ...profileScoped(),
...
export function getHerculesConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.herculesDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveHerculesConfig(config: HerculesConfigRecord): Promise<{ ok: boolean }> {
  return window.herculesDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.herculesDesktop.api<MemoryProviderConfig>({
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.herculesDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.herculesDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function revealEnvVar(key: string): Promise<{ key: string; value: string }> {
  return window.herculesDesktop.api<{ key: string; value: string }>({
    ...profileScoped(),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.herculesDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.herculesDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string): Promise<OAuthStartResponse> {
  return window.herculesDesktop.api<OAuthStartResponse>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.herculesDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(providerId: string, sessionId: string): Promise<OAuthPollResponse> {
  return window.herculesDesktop.api<OAuthPollResponse>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.herculesDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getSkills(): Promise<SkillInfo[]> {
  return window.herculesDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
}
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.herculesDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function getLearningNode(id: string): Promise<LearningNodeDetail> {
  return window.herculesDesktop.api<LearningNodeDetail>({
    ...profileScoped(),
...
export function deleteLearningNode(id: string): Promise<{ message: string; ok: boolean }> {
  return window.herculesDesktop.api<{ message: string; ok: boolean }>({
    ...profileScoped(),
...
export function toggleSkill(name: string, enabled: boolean): Promise<{ ok: boolean; name: string; enabled: boolean }> {
  return window.herculesDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(),
...
export function testMcpServer(name: string): Promise<McpTestResult> {
  return window.herculesDesktop.api<McpTestResult>({
    ...profileScoped(),
 ...
 *  `saveHerculesConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function getToolsets(): Promise<ToolsetInfo[]> {
  return window.herculesDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export function getToolsetConfig(name: string): Promise<ToolsetConfig> {
  return window.herculesDesktop.api<ToolsetConfig>({
    ...profileScoped(),
...
export function getToolsetModels(name: string, provider?: string): Promise<ToolsetModelsResponse> {
  ...
  return window.herculesDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(),
...
export function selectToolsetModel(
  ...
  return window.herculesDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(),
...
export function selectToolsetProvider(
  ...
  return window.herculesDesktop.api<{ ok: boolean; name: string; provider: string }>({
    ...profileScoped(),
...
export function runToolsetPostSetup(name: string, key: string): Promise<ActionResponse & { key: string }> {
  return window.herculesDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.herculesDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function grantComputerUsePermissions(): Promise<ActionResponse> {
  return window.herculesDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.herculesDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.herculesDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.herculesDesktop.api<MessagingPlatformTestResponse>({
...
export function getCronJobs(): Promise<CronJob[]> {
  return window.herculesDesktop.api<CronJob[]>({
    path: '/api/cron/jobs',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.herculesDesktop.api<{ runs: SessionInfo[] }>({
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.herculesDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.herculesDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.herculesDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.herculesDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.herculesDesktop.api<ProfileSetupCommand>({
    path: `/api/profiles/${encodeURIComponent(name)}/setup-command`
  })
}

export function getUsageAnalytics(days = 30): Promise<AnalyticsResponse> {
  return window.herculesDesktop.api<AnalyticsResponse>({
    ...profileScoped(),
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.herculesDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
// curation `hercules model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(provider: string): Promise<RecommendedDefaultModel> {
  return window.herculesDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(),
...
export function setGlobalModel(
  ...
  return window.herculesDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body: {
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.herculesDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.herculesDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.herculesDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  return window.herculesDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function updateHercules(): Promise<ActionResponse> {
  return window.herculesDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/hercules/update',
    method: 'POST'
  })
}
...
export function checkHerculesUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.herculesDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/hercules/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200): Promise<ActionStatusResponse> {
  return window.herculesDesktop.api<ActionStatusResponse>({
    ...profileScoped(),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.herculesDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.herculesDesktop.api<AudioSpeakResponse>({
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.herculesDesktop.api<ElevenLabsVoicesResponse>({
    path: '/api/audio/elevenlabs/voices'
  })
}

// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `hercules skills`
...
export function getSkillHubSources(): Promise<SkillHubSourcesResponse> {
  return window.herculesDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(),
...
export function searchSkillsHub(query: string, source = 'all', limit = 20): Promise<SkillHubSearchResponse> {
  ...
  return window.herculesDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(),
...
export function previewSkillHub(identifier: string): Promise<SkillHubPreview> {
  return window.herculesDesktop.api<SkillHubPreview>({
    ...profileScoped(),
...
export function scanSkillHub(identifier: string): Promise<SkillHubScanResult> {
  return window.herculesDesktop.api<SkillHubScanResult>({
    ...profileScoped(),
...
// `hercules mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveHerculesConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.herculesDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getMcpCatalog(): Promise<McpCatalogResponse> {
  return window.herculesDesktop.api<McpCatalogResponse>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `hercules memory` / `hercules curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.herculesDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.herculesDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.herculesDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.herculesDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `hercules doctor` / `hercules security
// audit` / `hercules backup` / `hercules debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.herculesDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.herculesDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.herculesDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.herculesDesktop.api<DebugShareResponse>({
