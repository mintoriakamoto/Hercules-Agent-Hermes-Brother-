...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'hercules:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.herculesDesktop?.saveImageFromUrl) {
        if (await window.herculesDesktop.saveImageFromUrl(src)) {
