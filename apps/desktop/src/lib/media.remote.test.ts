...
describe('filePathFromMediaPath', () => {
  it('passes through a plain path', () => {
    expect(filePathFromMediaPath('/home/u/.hercules/images/a.png')).toBe('/home/u/.hercules/images/a.png')
  })
...
describe('gatewayMediaDataUrl', () => {
  ...
  beforeEach(() => {
    api.mockClear()
    vi.stubGlobal('window', { herculesDesktop: { api } })
    $connection.set({ mode: 'remote' } as never)
  })
  ...
  it('reads gateway media through the desktop fs bridge instead of /api/media roots', async () => {
    const url = await gatewayMediaDataUrl('/home/u/.hercules/skills/demo/images/a b.png')

    expect(url).toBe('data:image/png;base64,ZHVtbXk=')
    expect(api).toHaveBeenCalledWith({
      path: '/api/fs/read-data-url?path=%2Fhome%2Fu%2F.hercules%2Fskills%2Fdemo%2Fimages%2Fa%20b.png'
    })
  })
})
...
describe('downloadGatewayMediaFile', () => {
  ...
  beforeEach(() => {
    api.mockClear()
    vi.stubGlobal('window', { herculesDesktop: { api }, setTimeout: vi.fn() })
    vi.stubGlobal(
      'fetch',
