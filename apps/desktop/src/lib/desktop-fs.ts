import type {
  HerculesConnection,
  HerculesReadDirResult,
  HerculesReadFileTextResult,
  HerculesSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: HerculesConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<HerculesReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<HerculesReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<HerculesReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<HerculesReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: HerculesSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
