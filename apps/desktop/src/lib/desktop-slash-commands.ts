...
/** Why a known Hercules command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Hercules profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Hercules Agent version', surface: exec() },
...
function isKnownHerculesSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Hercules' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownHerculesSlashCommand(normalized)
}
