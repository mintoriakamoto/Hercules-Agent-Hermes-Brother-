...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateHercules } from '@/hercules'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/hercules'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `hercules logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateHercules()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.herculesActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
