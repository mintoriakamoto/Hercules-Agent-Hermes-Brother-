...
const HERCULES_PATHS_MIME = 'application/x-hercules-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(HERCULES_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(HERCULES_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.herculesDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (hercules --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[hercules-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.herculesDesktop?.terminal?.write(sessionId, `${command}\r`)
