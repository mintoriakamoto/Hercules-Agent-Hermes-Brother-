...
import type { HerculesReadDirEntry, HerculesReadDirResult } from '@/global'
...
export type ProjectTreeEntry = HerculesReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: HerculesReadDirEntry) {
...
async function filterIgnored(entries: HerculesReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<HerculesReadDirResult> {
  if (!window.herculesDesktop) {
