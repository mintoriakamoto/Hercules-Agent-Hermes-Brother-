...
import type { HerculesReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<HerculesReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { herculesDesktop: { readDir: typeof readDir } }).herculesDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { herculesDesktop?: unknown }).herculesDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): HerculesReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { herculesDesktop: unknown }).herculesDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: HerculesReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<HerculesReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { herculesDesktop: unknown }).herculesDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.herculesDesktop is missing', async () => {
    delete (window as unknown as { herculesDesktop?: unknown }).herculesDesktop
