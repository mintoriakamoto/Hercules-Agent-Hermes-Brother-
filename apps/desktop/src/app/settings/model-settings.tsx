...
import { invalidateHerculesConfig, setHerculesConfigCache, useHerculesConfigRecord } from '../hooks/use-config-record'
...
// Hercules' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Hercules default (medium), shown as Medium.
...
// backend now surfaces the full `hercules model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in hercules_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useHerculesConfigRecord()
  const setConfig = setHerculesConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `hercules model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Hercules runs the flow for you.`}
          </p>
        )}
