...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: HerculesConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: HerculesConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // HERCULES_QWEN_ shares the HERCULES_ stem with other integrations.
      expect(providerGroup('HERCULES_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
