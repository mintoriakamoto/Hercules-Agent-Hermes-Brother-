...
import { disconnectOAuthProvider, listOAuthProviders } from '@/hercules'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/hercules'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.herculesDesktop?.terminal)
...
//      in hercules_cli/provider_catalog.py) — the SAME provider identity
//      `hercules model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Hercules can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Hercules can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Hercules never deletes creds another tool owns behind a silent API call.
