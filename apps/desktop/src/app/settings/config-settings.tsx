...
import { getElevenLabsVoices, getHerculesConfigSchema, saveHerculesConfig } from '@/hercules'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, HerculesConfigRecord } from '@/types/hercules'

import { setHerculesConfigCache, useHerculesConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: HerculesConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<HerculesConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useHerculesConfigRecord()
  ...
  const {
    ...
    queryKey: ['hercules-config-schema'],
    queryFn: getHerculesConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: HerculesConfigRecord) => {
