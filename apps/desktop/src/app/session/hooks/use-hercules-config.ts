...
import { getHerculesConfig, getHerculesConfigDefaults } from '@/hercules'
...
interface HerculesConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useHerculesConfig({ activeSessionIdRef, refreshProjectBranch }: HerculesConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshHerculesConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getHerculesConfig(), getHerculesConfigDefaults().catch(() => ({}))])
  ...
  return { refreshHerculesConfig, sttEnabled, voiceMaxRecordingSeconds }
}
