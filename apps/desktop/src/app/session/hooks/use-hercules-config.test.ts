...
import { getHerculesConfig } from '@/hercules'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useHerculesConfig } from './use-hercules-config'

vi.mock('@/hercules', () => ({
  getHerculesConfig: vi.fn(),
  getHerculesConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'hercules.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getHerculesConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getHerculesConfig>>)

describe('useHerculesConfig refreshHerculesConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshHerculesConfig()
    })
