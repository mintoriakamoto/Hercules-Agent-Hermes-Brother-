...
export async function attachmentPreviewDataUrl(filePath: string): Promise<string> {
  try {
    const local = await window.herculesDesktop?.readFileDataUrl?.(filePath)
...
export const HERCULES_PATHS_MIME = 'application/x-hercules-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Hercules sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.herculesDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(HERCULES_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.herculesDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.herculesDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.herculesDesktop?.getPathForFile ? window.herculesDesktop.getPathForFile(file) : ''
