        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.herculesDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Hercules to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.herculesDesktop?.watchPreviewFile ||
      !window.herculesDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.herculesDesktop.onPreviewFileChanged(payload => {
    ...
    void window.herculesDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.herculesDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.herculesDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:hercules-preview')
