...
export const HERCULES_SESSION_MIME = 'application/x-hercules-session'
...
export function writeSessionDrag(transfer: DataTransfer, payload: SessionDragPayload) {
  transfer.setData(HERCULES_SESSION_MIME, JSON.stringify(payload))
  transfer.effectAllowed = 'copy'
}

export function dragHasSession(transfer: DataTransfer | null) {
  return Boolean(transfer) && Array.from(transfer!.types || []).includes(HERCULES_SESSION_MIME)
}

export function readSessionDrag(transfer: DataTransfer | null): null | SessionDragPayload {
  const raw = transfer?.getData(HERCULES_SESSION_MIME)
