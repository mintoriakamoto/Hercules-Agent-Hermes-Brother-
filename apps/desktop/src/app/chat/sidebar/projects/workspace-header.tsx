...
const branchActionLabel = (branch: HerculesGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `hercules/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<HerculesGitBranch[]>([])
  ...
  const convert = async (branch: HerculesGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
