...
import type { HerculesGitWorktree } from '@/global'
import type { ProjectInfo, SessionInfo } from '@/types/hercules'
...
describe('baseName', () => {
  it('returns the final path segment, ignoring trailing slashes and separators', () => {
    expect(baseName('/www/hercules-agent/')).toBe('hercules-agent')
...
describe('mergeRepoWorktreeGroups (visual enhancer)', () => {
  it('injects a linked worktree lane discovered by git that has no sessions yet', () => {
    ...
    const discovered: HerculesGitWorktree[] = [
  ...
  it('surfaces a user-named "New worktree" under .worktrees/ as its own lane', () => {
    const discovered: HerculesGitWorktree[] = [
      {
        branch: 'hercules/test-gui-stuff',
        detached: false,
        isMain: false,
        locked: false,
    ...
    expect(merged.map(g => g.label)).toContain('hercules/test-gui-stuff')
  })

  it('relabels a dir-named linked worktree lane to its live checked-out branch', () => {
    // Backend labels the lane by the worktree dir (`hercules-agent-ci`); the live
...
describe('overlayLiveLanes', () => {
  ...
  it('places a session into an out-of-tree (sibling) worktree lane by its path', () => {
    // `hercules-agent-ci` is a linked worktree living BESIDE the repo, not under
