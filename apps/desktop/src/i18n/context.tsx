...
import { getHerculesConfigRecord, type HerculesConfigRecord, saveHerculesConfig } from '@/hercules'
...
export interface I18nConfigClient {
  getConfig: () => Promise<HerculesConfigRecord>
  saveConfig: (config: HerculesConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.herculesDesktop?.api) {
      return Promise.resolve({})
    }

    return getHerculesConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.herculesDesktop?.api) {
    ...
    return saveHerculesConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: HerculesConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: HerculesConfigRecord, locale: Locale): HerculesConfigRecord {
