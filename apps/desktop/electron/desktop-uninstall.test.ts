...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'hercules_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'hercules_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'hercules_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Hercules.app/Contents/MacOS/Hercules', 'darwin'),
    '/Applications/Hercules.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Hercules.app/Contents/MacOS/Hercules', 'darwin'),
    '/Users/x/Applications/Hercules.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Hercules\\Hercules.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Hercules'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hercules-desktop\\Hercules.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hercules-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Hercules.exe', 'win32'), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Hercules.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Hercules.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.hercules/hercules-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.hercules/hercules-agent',
    uninstallArgs: ['-m', 'hercules_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/hercules/linux-unpacked',
    herculesHome: '/home/x/.hercules'
  })
  ...
  assert.match(script, /'-m' 'hercules_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/hercules\/linux-unpacked'/)
  assert.match(script, /export HERCULES_HOME='\/home\/x\/\.hercules'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.hercules/hercules-agent',
    agentRoot: '/home/x/.hercules/hercules-agent',
    uninstallArgs: ['-m', 'hercules_cli.uninstall', '--mode', 'full'],
    appPath: null,
    herculesHome: '/home/x/.hercules'
  })

  // System python + source on PYTHONPATH so import hercules_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.hercules\/hercules-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'hercules_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'hercules_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    herculesHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'hercules_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    pythonPath: 'C:\\hercules',
    agentRoot: 'C:\\hercules',
    uninstallArgs: ['-m', 'hercules_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Hercules',
    herculesHome: 'C:\\Users\\x\\AppData\\Local\\hercules'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import hercules_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\hercules;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "hercules_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Hercules" >nul 2>&1/)
