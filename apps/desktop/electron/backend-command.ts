...
// Backend subcommand routing for the desktop-managed Hercules process.
//
// The desktop app launches its own headless backend via `hercules serve` — it
...
// app hasn't updated yet, or an older `hercules` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Hercules profile to pin via `--profile`.
 */
 ...
 * `-m hercules_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `hercules_cli/subcommands/dashboard.py` source registers
