...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/hercules  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/hercules',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/hercules/'), 'https://gw.example.com/hercules')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/hercules?x=1#frag'), 'https://gw.example.com/hercules')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/hercules'), 'https://host/hercules')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/hercules', 't'), 'wss://host/hercules/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/hercules', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/hercules/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'hercules_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-hercules_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-hercules_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'hercules_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'hercules_session_rt', value: 'x' }]), false)
...
test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'hercules_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-hercules_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-hercules_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'hercules_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-hercules_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-hercules_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'hercules_session_at', value: 'a' },
      { name: 'hercules_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'hercules_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'hercules_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'hercules_session_at', value: '' },
      { name: 'hercules_session_rt', value: '' }
    ]),
    false
  )
})
...
test('cookiesHavePrivySession does NOT treat hercules gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'hercules_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-hercules_session_rt', value: 'x' }]), false)
})
