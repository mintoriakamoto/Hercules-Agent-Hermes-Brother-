 ...
 *     `X-Hercules-Session-Token` header; WS uses `?token=`.
...
// hercules_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - hercules_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - hercules_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / hercules #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-hercules_session_at', '__Secure-hercules_session_at', 'hercules_session_at']
const RT_COOKIE_VARIANTS = ['__Host-hercules_session_rt', '__Secure-hercules_session_rt', 'hercules_session_rt']

// The Nous portal (NAS) does NOT use Hercules gateway session cookies — it is a
...
// Hercules Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://hercules.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a hercules session ACCESS-token cookie
 ...
 * the Hercules gateway session cookies, so cloud sign-in / discovery liveness
