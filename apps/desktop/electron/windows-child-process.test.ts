...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /herculesProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'hercules_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvHerculesCommand\(command, backendArgs\)/)
  assert.match(source, /function getVenvSitePackagesEntries\(venvRoot\)/)
  assert.match(source, /path\.join\(venvRoot, 'Lib', 'site-packages'\)/)
  assert.match(source, /args: \['-m', 'hercules_cli\.main', \.\.\.backendArgs\]/)
})
...
test('desktop backend launches console python so child consoles are inherited, not pythonw', () => {
  ...
  // HERCULES_DASHBOARD_READY stdout line — no ready-file side channel is set.
...
test('desktop backend teardown tree-kills Windows backend descendants', () => {
  ...
  const resetIndex = source.indexOf('function resetHerculesConnection()')
  assert.notEqual(resetIndex, -1, 'missing resetHerculesConnection')
  const resetSnippet = source.slice(resetIndex, resetIndex + 300)
  assert.match(resetSnippet, /stopBackendChild\(herculesProcess\)/)
  assert.doesNotMatch(resetSnippet, /herculesProcess\.kill\('SIGTERM'\)/)
  ...
  assert.match(quitSnippet, /stopBackendChild\(herculesProcess\)/)
  assert.doesNotMatch(quitSnippet, /herculesProcess\.kill\('SIGTERM'\)/)
})
