...
export function markerPath(herculesHome) {
  return path.join(herculesHome, '.hercules-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(herculesHome)
  let raw
 ...
 *
 * The Tauri-based hercules-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(herculesHome, pid, { now = Date.now } = {}) {
  const file = markerPath(herculesHome)
