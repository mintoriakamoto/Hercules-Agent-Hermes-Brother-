...
// no longer bundles the Hercules Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Hercules.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Hercules'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Hercules.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'hercules'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default HERCULES_HOME for non-sandboxed runs -- matches main.ts's
// resolveHerculesHome(). On Windows it's %LOCALAPPDATA%\hercules; elsewhere
// it's ~/.hercules. The fresh-install sandbox launchFresh() sets its own
// HERCULES_HOME and never touches this.
const DEFAULT_HERCULES_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hercules')
  }
  return path.join(os.homedir(), '.hercules')
})()
const VENV_ROOT = path.join(DEFAULT_HERCULES_HOME, 'hercules-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'hercules-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.HERCULES_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Hercules-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Hercules-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Hercules-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Hercules-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.HERCULES_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.HERCULES_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const herculesHome = path.join(sandbox, 'hercules-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(herculesHome, { recursive: true })
  ...
  env.HERCULES_DESKTOP_CWD = cwd
  env.HERCULES_DESKTOP_IGNORE_EXISTING = '1'
  env.HERCULES_DESKTOP_TEST_MODE = 'fresh-install'
  env.HERCULES_DESKTOP_USER_DATA_DIR = userDataDir
  env.HERCULES_HOME = herculesHome
  delete env.HERCULES_DESKTOP_HERCULES
  delete env.HERCULES_DESKTOP_HERCULES_ROOT
  ...
  console.log(`  HERCULES_HOME: ${herculesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(herculesHome, 'hercules-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Hercules Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of hercules_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'hercules-agent', 'hercules_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Hercules
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + HERCULES_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  HERCULES_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
