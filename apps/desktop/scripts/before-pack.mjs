 ...
 * product name (`Hercules`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Hercules'
 *
 * This is a hard failure with no obvious cause for the user — `hercules desktop`
 ...
 * (the mac-unpacked Hercules.app bundle) and Windows (win-unpacked), so we
