...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{HERCULES_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``HERCULES_HOME`` is a **durable** store — on Hercules
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``hercules gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting HERCULES_HOME."""
    base = home if home is not None else get_hercules_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable HERCULES_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (HERCULES_HOME is a
    durable Fly volume on Hercules Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable HERCULES_HOME volume is
