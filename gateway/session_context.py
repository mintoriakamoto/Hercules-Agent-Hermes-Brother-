"""
Session-scoped context variables for the Hercules gateway.

Replaces the previous ``os.environ``-based session state
(``HERCULES_SESSION_PLATFORM``, ``HERCULES_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["HERCULES_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("HERCULES_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("HERCULES_SESSION_PLATFORM", "")
...
# intentionally separate from HERCULES_SESSION_ID: the latter is the durable
...
_VAR_MAP = {
    ...
    "HERCULES_SESSION_SOURCE": _SESSION_SOURCE,
    "HERCULES_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "HERCULES_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "HERCULES_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "HERCULES_SESSION_USER_ID": _SESSION_USER_ID,
    "HERCULES_SESSION_USER_NAME": _SESSION_USER_NAME,
    "HERCULES_SESSION_KEY": _SESSION_KEY,
    "HERCULES_SESSION_ID": _SESSION_ID,
    "HERCULES_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "HERCULES_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "HERCULES_SESSION_PROFILE": _SESSION_PROFILE,
    "HERCULES_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "HERCULES_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "HERCULES_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``HERCULES_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("HERCULES_SESSION_ID")`` with an ``os.environ`` fallback,
...
def reset_session_vars() -> None:
    ...
    *A's* ``HERCULES_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``HERCULES_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``HERCULES_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("HERCULES_SESSION_*", default)``.
