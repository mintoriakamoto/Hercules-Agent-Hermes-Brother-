...
Defenses 1 and 2 (the ``_HERCULES_GATEWAY`` guard on ``hercules gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.hercules.gateway")``,
...
State lives in ``<HERCULES_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_hercules_home() / "gateway" / "restart_loop.json"
