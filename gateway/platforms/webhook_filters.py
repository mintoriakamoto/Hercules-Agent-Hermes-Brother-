...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.hercules to the active profile home."""
    ...
    if raw.startswith("~/.hercules/"):
        return hercules_home / raw.removeprefix("~/.hercules/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return hercules_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under HERCULES_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from hercules_constants import get_hercules_home

    scripts_root = (get_hercules_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.hercules" or raw_text.startswith("~/.hercules/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__hercules_ignore__") is True
        ):
            return False, None
