...
There are two ways to add a platform to the Hercules gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.hercules/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Hercules code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `hercules gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `hercules_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Hercules core.
...
And add it to the `hercules-gateway` composite:

```python
"hercules-gateway": {
    "includes": [..., "hercules-your-platform"]
}
```
...
## 12. Status Display (`hercules_cli/status.py`)
...
## 13. Gateway Setup Wizard (`hercules_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ hercules_cli/ toolsets.py \
  --include="*.py" -l | sort -u
