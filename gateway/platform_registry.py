...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``hercules setup`` display).
    ...
    # built-ins).  Used by ``hercules gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `hercules`
        # invocation -- including plain `hercules chat`, which never touches any
        ...
        # `hercules setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``hercules setup``/``gateway status``, channel
