...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        try:
            ...
            return
        env_override = os.environ.get("HERCULES_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban notifier: disabled via HERCULES_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        try:
            cfg = _load_config()
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # HERCULES_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `hercules kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours HERCULES_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        env_override = os.environ.get("HERCULES_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via HERCULES_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `hercules kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Hercules profile
            ...
            PATH, missing venv, credential loss for a real Hercules profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from hercules_cli import kanban_decompose as _decomp
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`hercules kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
