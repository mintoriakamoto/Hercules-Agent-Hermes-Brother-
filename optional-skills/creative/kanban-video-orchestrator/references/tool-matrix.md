...
Maps each role archetype to the Hercules skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public hercules-agent
...
## Hercules skills relevant to video production

### Visual / rendering skills (`hercules-agent/skills/creative/`)
...
### Design / pre-production skills (`hercules-agent/skills/creative/`)
...
### Audio / media skills (`hercules-agent/skills/creative/` + `skills/media/`)
...
These are **not** Hercules skills but external CLIs / APIs that profiles invoke.
...
## Built-in Hercules tools for media review

These are native Hercules tools — not invoked via terminal but through their own
...
key is present in `${HERCULES_HOME:-~/.hercules}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Hercules profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HERCULES_HOME:-~/.hercules}/.env` → environment variable.
...
When a new Hercules-public video skill ships:
