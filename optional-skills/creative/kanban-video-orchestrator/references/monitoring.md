...
hercules kanban watch --tenant <project-slug>

# Snapshot of the board
hercules kanban list --tenant <project-slug>
hercules kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
hercules kanban stats --tenant <project-slug>

# Visual dashboard (browser)
hercules dashboard

# Inspect a specific task (includes comments + events)
hercules kanban show <task-id>

# Follow a single task's event stream
hercules kanban tail <task-id>
```

Verify available subcommands with `hercules kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `hercules kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `hercules kanban block <id>` then `hercules kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `hercules kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `hercules kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `hercules profile list` (profile exists?), `hercules status` (gateway/dispatcher up?), and verify tenant. |
...
hercules kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
hercules kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(hercules kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
hercules kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
hercules kanban block <task-id>
hercules kanban archive <task-id>

# Diagnose what happened
hercules kanban show <task-id>      # task body, comments, recent events
hercules kanban tail <task-id>      # follow the live event stream
hercules kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `hercules kanban create` for the director
...
while true; do
    clear
    hercules kanban list --tenant <slug>
    echo "---"
    hercules kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `hercules kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `hercules profile list` and start
