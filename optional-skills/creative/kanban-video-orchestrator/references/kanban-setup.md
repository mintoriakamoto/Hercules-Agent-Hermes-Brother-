...
the actual `setup.sh` that creates the project workspace, configures Hercules
...
2. **Create profiles** — `hercules profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.hercules/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `hercules kanban create` assigned to the director
...
```bash
hercules profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.hercules/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hercules/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Hercules
...
Each profile gets a `SOUL.md` at `~/.hercules/profiles/<name>/SOUL.md` that
...
```bash
hercules kanban create "Direct production of <video title>" \
    --assignee director \
...
the Hercules `.env` (`${HERCULES_HOME:-$HOME/.hercules}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hercules_env="${HERCULES_HOME:-$HOME/.hercules}/.env"
    if grep -q "^${var}=" "$_hercules_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_hercules_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_hercules_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY hercules ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY hercules OPENROUTER_API_KEY || exit 1
# ...
```
