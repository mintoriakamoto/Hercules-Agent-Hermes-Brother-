...
Usage (via cron with --no-agent):
    ...
    hercules cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$HERCULES_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Hercules-Watcher/1.0"})
