...
Usage (via cron with --no-agent):
    ...
    hercules cron create hercules-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HERCULES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hercules-issues --repo NousResearch/hercules-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Hercules .env file
(``${HERCULES_HOME:-~/.hercules}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Hercules-Watcher/1.0",
    }
