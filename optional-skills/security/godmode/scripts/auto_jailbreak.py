...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HERCULES_HOME", os.path.expanduser("~/.hercules")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERCULES_HOME", Path.home() / ".hercules")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Hercules models — already uncensored, just needs clean prompt
    "hercules": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "hercules" in model_lower or "nous" in model_lower:
        return "hercules"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Hercules config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hercules/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Hercules for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Hercules for changes to take effect.")
