...
All per-subject data lives under PDD_DATA_DIR (default: $HERCULES_HOME/unbroker),
which is the same trust boundary Hercules uses for .env and OAuth tokens.
"""
...
def hercules_home() -> Path:
    return Path(os.environ.get("HERCULES_HOME") or (Path.home() / ".hercules"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else hercules_home() / "unbroker"
