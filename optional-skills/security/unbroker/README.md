...
The design is **Hercules-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
hercules skills install official/security/unbroker
```

Then start a new Hercules session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Hercules session:
...
- **PII handling.** Dossiers live under the Hercules home directory (`0600`, optionally
