...
$HERCULES_OSINT_CACHE/icij/ (default: ~/.cache/hercules-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("HERCULES_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "hercules-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "hercules-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HERCULES_OSINT_CACHE/icij or ~/.cache/hercules-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
