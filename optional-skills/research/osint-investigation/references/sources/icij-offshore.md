...
- **Note:** The previous Open Refine reconciliation endpoint at
  ...
  the ZIP locally (default `~/.cache/hercules-osint/icij/`, refreshes after
...
First call downloads the ~70 MB ZIP under `~/.cache/hercules-osint/icij/`
(or `$HERCULES_OSINT_CACHE/icij/`). Subsequent calls reuse the cache for 30 days.
