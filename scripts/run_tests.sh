#!/usr/bin/env bash
# Canonical test runner for hercules-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.hercules/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.hercules/hercules-agent/venv"; do
...
if [ -f "$HOME/.hercules/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.hercules"
...
exec env -i \
  ...
  ${HERCULES_RUN_SLOW_PET_TESTS:+HERCULES_RUN_SLOW_PET_TESTS="$HERCULES_RUN_SLOW_PET_TESTS"} \
