...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with hercules-agent)
2. User plugins: ``$HERCULES_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HERCULES_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HERCULES_HOME/plugins/model-providers/`` if it exists."""
    try:
        from hercules_constants import get_hercules_home

        d = get_hercules_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HERCULES_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_hercules_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HERCULES_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with hercules-agent.
    ...
    # 2. User plugins — under $HERCULES_HOME/plugins/model-providers/<name>/.
