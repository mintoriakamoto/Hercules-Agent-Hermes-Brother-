...
Registry and ABC for every inference provider Hercules knows about.
...
`$HERCULES_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `hercules_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `hercules_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `hercules_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `hercules_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `hercules_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$HERCULES_HOME/plugins/model-providers/` for a private plugin).
