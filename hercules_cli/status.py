"""
Status command for hercules CLI.
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``hercules config``'s
...
def show_status(args):
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: hercules portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: hercules model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: hercules auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: hercules auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: hercules model)"
    ...
    print(color("  Run 'hercules doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'hercules setup' to configure", Colors.DIM))
    print()
