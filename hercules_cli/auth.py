...
DEFAULT_NOUS_CLIENT_ID = "hercules-cli"
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``HERCULES_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Hercules at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``HERCULES_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("HERCULES_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: hercules auth add nous",
        provider="nous",
        code=reason,
...
# so a new `hercules --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${HERCULES_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<hercules-root>/shared/nous_auth.json`` where ``<hercules-root>`` is what
# ``get_default_hercules_root()`` returns — ``~/.hercules`` on Linux/macOS,
# ``%LOCALAPPDATA%\hercules`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's HERCULES_HOME so named profiles (which
# typically live under ``<hercules-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``HERCULES_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hercules-root>/shared/``, where ``<hercules-root>`` is what
    :func:`hercules_constants.get_default_hercules_root` returns — so
    Linux/macOS classic installs land at ``~/.hercules/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hercules\\shared\\``, and
    Docker / custom ``HERCULES_HOME`` deployments at
    ``<HERCULES_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("HERCULES_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from hercules_constants import get_default_hercules_root
    return get_default_hercules_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Hercules root, refuse rather than silently corrupt cross-profile
    # state. Tests must set HERCULES_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from hercules_constants import get_default_hercules_root
        real_home_shared = (
            get_default_hercules_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set HERCULES_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No HERCULES_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Hercules
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Hercules's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Hercules's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Hercules install sharing "
            "~/.hercules/auth.json) called POST /api/oauth/token with Hercules's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Hercules may call the "
            "refresh endpoint. For health checks, use `hercules auth status` "
            "instead.\n"
            "Re-authenticate with: hercules auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Hercules models — they're not reliable for agentic tool-calling
            if "hercules" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Hercules is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # HERCULES_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "hercules-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``hercules auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``hercules auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `hercules auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Hercules is not logged into Nous Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("HERCULES_PORTAL_BASE_URL")
...
# ~350ms even on the failure path, and read-only UI surfaces (`hercules tools`, status panels,
# subscription-feature checks) call it many times per render — `hercules tools` → "All Platforms"
...
# `hercules auth login/logout/add/remove` invalidate naturally on the next call.
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Hercules login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `hercules model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Hercules builds still see
        ...
        if not selected_model:
            ...
            print("  Run `hercules model` again to switch to Nous Portal.")
            return
