...
The provider list shown by ``hercules model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`hercules_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``hercules model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`hercules_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``hercules model`` shows.
"""
...
# hercules_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `hercules model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``hercules model`` universe.
    ...
    # Hercules overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = HERCULES_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Hercules overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
