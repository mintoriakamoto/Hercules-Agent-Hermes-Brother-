"""Safe Hercules Console command engine.

This module backs ``hercules console`` and is intentionally narrower than the
full Hercules CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["HerculesConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'hercules doctor'")
        and last.startswith("Run 'hercules setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="hercules", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `hercules "):
        return ""
    return summary
...
# dashboard opens a fresh HerculesConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("hercules_cli.main")
    builder = getattr(module, builder_name)
    ...
    *,
    ...
    for child_path in paths:
        ...
        engine.register(
            ...
            confirmation=confirmation or f"Run `hercules {usage}`?",
            contexts=ALL_CONTEXTS if child_key in hosted_paths else LOCAL_CONTEXTS,
        )


class HerculesConsoleEngine:
    """Curated line-command executor for Hercules Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "hercules":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Hercules Console does not run shell syntax. Use one supported "
                    "Hercules command at a time."
                )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Hercules Console."""

        extracted = {
            "version": (
                "hercules_cli.subcommands.version",
            ...
            "debug": (
                "hercules_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "hercules_cli.subcommands.prompt_size",
            ...
            "insights": (
                "hercules_cli.subcommands.insights",
            ...
            "security": (
                "hercules_cli.subcommands.security",
            ...
            "backup": (
                "hercules_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "hercules_cli.subcommands.import_cmd",
            ...
            "config": (
                "hercules_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "hercules_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "skills": (
                "hercules_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "hercules_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "hercules_cli.subcommands.pairing",
            ...
            "webhook": (
                "hercules_cli.subcommands.webhook",
            ...
            "hooks": (
                "hercules_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "hercules_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "hercules_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Hercules configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("hercules_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("hercules_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        for size in range(min(len(tokens), 3), 0, -1):
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        f"`hercules {command.usage}` is not available in "
                        f"{self.context} Hercules Console."
                    )
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Hercules Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        blocked_pairs = {
            ...
            ("portal", "login"): "`portal login` is interactive and is not available in Hercules Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Hercules Console.",
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Hercules Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Hercules Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Hercules Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Hercules Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Hercules Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Hercules Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _status(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Hercules Console.")
    ...
    from hercules_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from hercules_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from hercules_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: HerculesConsoleEngine, args: list[str]) -> str:
...
def _profile_status(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="hercules_cli.subcommands.profile",
...
def _cron_list(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from hercules_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from hercules_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from hercules console")
...
def _cron_resume(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: HerculesConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``hercules console`` REPL."""
    ...
    while True:
        if interactive:
            print("hercules> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
