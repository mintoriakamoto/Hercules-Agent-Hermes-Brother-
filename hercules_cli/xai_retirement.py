...
Pure logic: walks a Hercules config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `hercules doctor` and a future `hercules migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Hercules config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Hercules config and return retirement issues.
...
def apply_migration(
    ...
    from hercules_cli.config import require_readable_config_before_write
