"""Profile distributions — shareable, packaged Hercules profiles via git.

A distribution is a Hercules profile published as a git repository (or
...
* ``hercules profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``hercules skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``hercules profile``, not a parallel tree):

    hercules profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "hercules_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".hercules_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "hercules-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            hercules_requires=str(data.get("hercules_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.hercules_requires:
            out["hercules_requires"] = self.hercules_requires
        if self.author:
...
def check_hercules_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Hercules {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``hercules profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Hercules profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Hercules distribution."
        )

    # Version check up-front so we fail fast
    check_hercules_requires(manifest.hercules_requires, hercules_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.hercules).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="hercules_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `hercules profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`hercules profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="hercules_dist_update_") as tmp:
        plan = plan_install(
