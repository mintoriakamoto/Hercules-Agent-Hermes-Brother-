...
Used by ``hercules kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``hercules_cli/goals.py`` — same aux
...
HERCULES_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("HERCULES_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)
...
def _profile_author() -> str:
    """Mirror of ``hercules_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=HERCULES_KANBAN_SPECIFY_MAX_TOKENS,
