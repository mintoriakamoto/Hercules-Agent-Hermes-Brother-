...
Usage:
    hercules pairing list              # Show all pending + approved users
    hercules pairing approve <platform> <code>  # Approve a pairing code
    hercules pairing revoke <platform> <user_id> # Revoke user access
    hercules pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle hercules pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: hercules pairing {list|approve|revoke|clear-pending}")
        print("Run 'hercules pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.hercules/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'hercules pairing list' to see pending codes.\n")
