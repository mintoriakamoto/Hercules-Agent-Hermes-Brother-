"""Windows ConPTY bridge for the `hercules dashboard` chat tab.

Drop-in counterpart to ``hercules_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``hercules_cli.web_server`` consumes: ``spawn``, ``read``,
