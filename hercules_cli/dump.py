"""
Dump command for hercules CLI.

Outputs a compact, plain-text summary of the user's Hercules setup
...
from hercules_cli.config import get_hercules_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.hercules/.env.
    ...
    exports. ``hercules debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.hercules_build_sha`` by the Dockerfile's
    ``HERCULES_GIT_SHA`` build-arg (see ``hercules_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``hercules dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from hercules_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(hercules_home: Path) -> int:
...
def _cron_summary(hercules_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = hercules_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"hercules_home:      {display_hercules_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.hercules/.env: a managed
        ...
        # A credential added via `hercules auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `hercules auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["hercules-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(hercules_home)}")
    lines.append(f"  skills:             {_count_skills(hercules_home)}")
