...
on Japanese locales, etc.).  Hercules's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``hercules_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Hercules is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``HERCULES_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("HERCULES_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # already-running shells don't see the change — which means hercules
    ...
    # install without Hercules-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Hercules's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Hercules's
    ``hercules config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Hercules (User env var in Windows Settings, or
    export in a PowerShell profile) and Hercules picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\hercules\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``hercules`` from right after install) retain their old PATH.

    Any subprocess Hercules spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Hercules-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "hercules", "git", "cmd"),
        os.path.join(local_appdata, "hercules", "git", "bin"),
        os.path.join(local_appdata, "hercules", "git", "usr", "bin"),
        # Hercules venv Scripts directory — host of the hercules.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes hercules, but harmless to include.
        os.path.join(local_appdata, "hercules", "hercules-agent", "venv", "Scripts"),
