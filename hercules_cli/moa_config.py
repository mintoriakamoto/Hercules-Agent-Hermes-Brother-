...
MOA_MARKER_PREFIX = "__HERCULES_MOA_TURN_V1__"
...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model Hercules agent
...
def exact_moa_preset_name(config: Any, text: str) -> str | None:
    ...
    ``hercules_cli/model_switch.py``) to recognize a bare ``/model <preset>``
