...
def nous_portal_billing_url(account_info: Optional[NousPortalAccountInfo] = None) -> str:
    ...
    try:
        from hercules_cli.auth import DEFAULT_NOUS_PORTAL_URL
    except Exception:
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Hercules could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `hercules model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Hercules cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`hercules model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `hercules model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Hercules could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `hercules model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Hercules could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `hercules model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `hercules model` to refresh Hercules."
        return message
...
def _fresh_account_info(
    *,
    ...
    try:
        from hercules_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from hercules_cli.auth import _decode_jwt_claims
    except Exception:
        return None
