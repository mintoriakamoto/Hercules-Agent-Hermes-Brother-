#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Hercules Skills Hub.

Powers both:
  - `hercules skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `hercules skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.hercules/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.hercules/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `hercules skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: hercules skills inspect <identifier> to preview, "
            "hercules skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "hercules-index".
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from the Hercules project[/]")
    c.print()
    ...
    # The identifier is what you pass to `hercules skills install`. Browse used
    ...
    c.print("[dim]Tip: 'hercules skills inspect <identifier>' to preview, "
            "'hercules skills install <identifier>' to install, "
            "'hercules skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]hercules skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by the Hercules project.[/]\n\n"
                "It ships with hercules-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_hercules_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.hercules.blueprint block, it is a runnable automation. Register it as
        ...
        if spec is not None:
            ...
            if registered is not None:
                ...
                c.print(
                    ...
                    "or via[/] [bold]hercules cron add[/][dim].[/]\n"
                )
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="hercules skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "hercules-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"hercules-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying hercules-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``hercules -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``HERCULES_HOME`` at process
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `hercules update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `hercules update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   hercules skills diff <name>[/]")
    c.print("[dim]Resume updates: hercules skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: hercules skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: hercules skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: hercules skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: hercules skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: hercules skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_hercules_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Hercules Skills Hub.\n\n"
                        f"This skill was scanned by the Hercules Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `hercules skills <subcommand>` — called from hercules_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: hercules skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: hercules skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: hercules skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'hercules skills <command> --help' for details.\n")
