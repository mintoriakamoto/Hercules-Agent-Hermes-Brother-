...
* ``hercules_cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``hercules_cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (the hosting account service's
...
PUBLIC_API_PATHS: frozenset[str] = frozenset({
    ...
    # External-scheduler cron-fire webhook. NOT cookie-gated: it
