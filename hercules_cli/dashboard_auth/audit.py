...
Profile-aware location: ``$HERCULES_HOME/logs/dashboard-auth.log``.
...
from ``hercules_constants`` or other hercules_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$HERCULES_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``hercules_constants.get_hercules_home`` semantics: env var wins,
    else ``~/.hercules``. A local copy avoids an import cycle with the
    middleware which lives below ``hercules_cli``.
    """
    home = os.environ.get("HERCULES_HOME") or str(Path.home() / ".hercules")
