"""
Hercules Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hercules desktop`` / ``hercules gui``)
     Built inside the agent checkout under ``$HERCULES_HOME/hercules-agent/``:
       ...
       - ``$HERCULES_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Hercules.app`` or ``~/Applications/Hercules.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Hercules`` (NSIS per-user)
...
the app name ("Hercules"), separate from ``$HERCULES_HOME``:
  - macOS:   ``~/Library/Application Support/Hercules``
  - Windows: ``%APPDATA%\\Hercules``
  - Linux:   ``$XDG_CONFIG_HOME/Hercules`` (default ``~/.config/Hercules``)
...
``hercules uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(hercules_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return hercules_home / "hercules-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Hercules"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Hercules"
...
def source_built_gui_artifacts(hercules_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``hercules desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``hercules-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(hercules_home)
    ...
    return [
        ...
        hercules_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Hercules" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Hercules.app"),
            home / "Applications" / "Hercules.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Hercules).
            local_base / "Programs" / "Hercules",
            # Older / alternate layout some builds used.
            local_base / "hercules-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Hercules")
    else:
        ...
        paths += [
            data_base / "applications" / "hercules.desktop",
            data_base / "applications" / "Hercules.desktop",
        ]
    return paths


def agent_is_installed(hercules_home: Path) -> bool:
    """Return True when a usable Python agent install exists under HERCULES_HOME.
    ...
    if (agent_root / "hercules_cli").is_dir():
        return True
...
def gui_is_installed(hercules_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(hercules_home):
        if p.exists():
            return True
...
def gui_install_summary(hercules_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = hercules_home if hercules_home is not None else get_hercules_home()
...
def uninstall_gui(hercules_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hercules-agent/hercules_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$HERCULES_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove hercules' or "
            "'sudo dnf remove hercules'). AppImage builds are a single file you "
