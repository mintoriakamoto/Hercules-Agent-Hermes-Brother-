"""Slash command definitions and autocomplete for the Hercules CLI.
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`hercules_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/hercules`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Hercules ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Hercules
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``hercules skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``hercules skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``hercules skills list`` and
...
# unrelated command landed. These claim their slots right after /hercules,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /hercules <command>).
...
# native slot, the alias spelling stays reachable via /hercules reset).
...
# ``/hercules <command>`` on Slack only. They remain native on every other
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/hercules <verb>`` subcommand.
    ...
    can still reach them via ``/hercules <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/hercules`` is always reserved as the first entry so the
    legacy ``/hercules <subcommand>`` form keeps working for anything that
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_HERCULES_ONLY:
            # Intentionally Slack-via-/hercules only (see _SLACK_VIA_HERCULES_ONLY).
            return
    ...
    # everything except /hercules, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /hercules handler.

    Maps both canonical names and aliases so /hercules bg do stuff works
    the same as /hercules background do stuff.

    Plugin-registered slash commands are included so ``/hercules <plugin-cmd>``
