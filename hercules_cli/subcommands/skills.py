"""``hercules skills`` subcommand parser.
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.hercules/skills/.bundled_manifest) "
            "so future 'hercules update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `hercules update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `hercules update` reports as user-modified and skips. "
            "Use `hercules skills diff <name>` to see changes and `hercules skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`hercules update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `hercules update`. Pass --sync to re-seed now."
        ),
    )
