"""``hercules dashboard`` / ``hercules serve`` subcommand parsers.
...
``hercules_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `hercules dashboard` / `hercules serve` cmdlines and SIGTERM them directly —
    # the same path `hercules update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Hercules web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Hercules web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Hercules desktop app shells (<= 0.15.x) spawn the
    # backend as `hercules dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Hercules couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Hercules Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Hercules backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Hercules backend server — the JSON-RPC/WebSocket gateway the "
