"""``hercules debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hercules Agent. Use 'hercules debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hercules debug share              Upload debug report (asks for confirmation)
    hercules debug share --yes        Skip confirmation (for scripts/CI)
    hercules debug share --lines 500  Include more log lines
    hercules debug share --expire 30  Keep paste for 30 days
    hercules debug share --local      Print report locally (no upload)
    hercules debug share --no-redact  Disable upload-time secret redaction
    hercules debug share --nous       Upload to Nous-internal storage (private)
    hercules debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'hercules debug share'",
    )
