"""``hercules gateway`` and ``hercules proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if HERCULES_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy hercules.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Hercules gateway unit files "
            "(e.g. hercules.service) left over from older installs. Profile "
            "units (hercules-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.hercules/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. xAI). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider: xai (default: xai). See `hercules proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
