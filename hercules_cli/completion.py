"""Shell completion script generation for hercules CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_hercules_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_hercules_profiles() {{
    local profiles_dir="$HOME/.hercules/profiles"
...
_hercules_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_hercules_profiles)" -- "$cur"))
        return
    fi
...
complete -F _hercules_completion hercules
"""
...
_hercules_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.hercules/profiles" ]]; then
        profiles+=($HOME/.hercules/profiles/*(N/:t))
    fi
...
_hercules() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_hercules_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'hercules command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "function __hercules_profiles",
        "    echo default",
        "    if test -d $HOME/.hercules/profiles",
        "        for d in $HOME/.hercules/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c hercules -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c hercules -f -s p -l profile"
        " -d 'Profile name' -xa '(__hercules_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__hercules_profiles)' -d 'Profile name'"
                )
