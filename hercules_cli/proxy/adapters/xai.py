...
# xAI's public API is OpenAI-compatible for the endpoints Hercules commonly
# uses. The Responses endpoint is included because Hercules' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Hercules-managed OAuth credentials."""

    auth_hint = "hercules auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`hercules auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`hercules auth reset xai-oauth` or re-authenticate with "
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `hercules auth add xai-oauth --type oauth`."
            )
