"""`hercules checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hercules/checkpoints/``.  Actions:

    hercules checkpoints               # same as `status`
    hercules checkpoints status        # total size, project count, breakdown
    hercules checkpoints list          # per-project checkpoint counts + workdir
    hercules checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    hercules checkpoints clear [-f]    # nuke the entire base (asks first)
    hercules checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    hercules checkpoints
    hercules checkpoints prune --retention-days 3 --max-size-mb 200
    hercules checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: hercules checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``hercules checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `hercules checkpoints` → status
