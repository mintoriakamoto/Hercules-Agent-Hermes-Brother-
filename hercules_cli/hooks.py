"""hercules hooks — inspect and manage shell-script hooks.

Usage::

    hercules hooks list
    hercules hooks test <event> [--for-tool X] [--payload-file F]
    hercules hooks revoke <command>
    hercules hooks doctor

Consent records live under ``~/.hercules/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.hercules/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``hercules hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: hercules hooks {list|test|revoke|doctor}")
        print("Run 'hercules hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.hercules/config.yaml.")
        print("See `hercules hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `hercules hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `hercules hooks test` and `hercules hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Hercules wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `hercules hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `hercules hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `hercules hooks doctor`.")
