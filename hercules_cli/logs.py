"""``hercules logs`` — view and filter Hercules log files.
...
Usage examples::
    ...
    hercules logs                    # last 50 lines of agent.log
    hercules logs -f                 # follow agent.log in real time
    hercules logs errors             # last 50 lines of errors.log
    hercules logs gateway -n 100    # last 100 lines of gateway.log
    hercules logs gui -f            # follow gui.log (dashboard/pty/ws)
    hercules logs desktop -f        # follow desktop.log (Electron app boot/backend)
    hercules logs --level WARNING    # only WARNING+ lines
    hercules logs --session abc123   # filter by session ID substring
    hercules logs --component tools  # only tool-related lines
    hercules logs --since 1h         # lines from the last hour
    hercules logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_hercules_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Hercules runs — try 'hercules chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_hercules_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_hercules_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_hercules_home()}/logs/")
        return

    print(f"Log files in {display_hercules_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'hercules chat' to generate logs)")
