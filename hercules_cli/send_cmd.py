"""CLI subcommand: ``hercules send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"hercules send: {file_path} is not a text file. --file reads the "
                ...
                f'  hercules send --to telegram "MEDIA:{file_path}"\n'
                f'  hercules send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  hercules send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"hercules send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"hercules send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"hercules send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `hercules gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.hercules/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hercules_env() -> None:
    """Populate ``os.environ`` from ``~/.hercules/.env`` AND bridge top-level
    ...
    startup that ``hercules send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.hercules/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.hercules/config.yaml`` into
       ...
       via ``hercules config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``hercules send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.hercules/.env and ~/.hercules/config.yaml into os.environ so the
    ...
    if message is None or not message.strip():
        print(
            "hercules send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `hercules send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Hercules "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.hercules/.env + ~/.hercules/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `hercules send --list telegram`.",
    )
