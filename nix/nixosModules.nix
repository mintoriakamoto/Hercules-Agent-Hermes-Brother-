# nix/nixosModules.nix — NixOS module for hercules-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: hercules runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $HERCULES_HOME/.env
# and read by hercules at startup — no container recreation needed for env changes.
#
# Tool resolution: the hercules wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.hercules-agent;
    effectivePackage =
    ...
    hercules-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-hercules-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "hercules-config-attrs";
      description = "Hercules YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "hercules-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # HERCULES_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "hercules-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'HERCULES_DOC_EOF'\n${value}\nHERCULES_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "hercules-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/hercules";
    ...
    # hercules user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "hercules-container-entrypoint" ''
      set -eu

      HERCULES_UID="''${HERCULES_UID:?HERCULES_UID must be set}"
      HERCULES_GID="''${HERCULES_GID:?HERCULES_GID must be set}"

      # ── Group: ensure a group with GID=$HERCULES_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$HERCULES_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="hercules"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$HERCULES_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$HERCULES_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$HERCULES_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$HERCULES_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="hercules"
        TARGET_HOME="/home/hercules"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$HERCULES_UID" -g "$HERCULES_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$HERCULES_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$HERCULES_UID:$HERCULES_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure HERCULES_HOME is owned by the target user.
      ...
      if [ -n "''${HERCULES_HOME:-}" ] && [ -d "$HERCULES_HOME" ]; then
        find "$HERCULES_HOME" \! -user "$HERCULES_UID" -exec chown "$HERCULES_UID:$HERCULES_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/hercules-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/hercules-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/hercules ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/hercules
        chmod 0440 /etc/sudoers.d/hercules
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$HERCULES_UID" --regid="$HERCULES_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $HERCULES_HOME/.env.
    ...
    # Default: /var/lib/hercules/workspace → /data/workspace.
  ...
  in {
    options.services.hercules-agent = with lib; {
      ...
      package = mkOption {
        ...
        description = "The hercules-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/hercules";
        description = "State directory. Contains .hercules/ subdir (HERCULES_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Hercules config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $HERCULES_HOME/.env at activation time.
          Hercules reads this file on every startup via load_hercules_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $HERCULES_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $HERCULES_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `hercules gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the hercules user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the hercules plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Hercules discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          hercules_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as hercules (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in hercules-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the hercules CLI to environment.systemPackages and export
          HERCULES_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.hercules symlink to the service
            stateDir. These users are automatically added to the hercules group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.hercules-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hercules CLI to system PATH and export HERCULES_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.hercules/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.HERCULES_HOME = "${cfg.stateDir}/.hercules";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.hercules-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the hercules user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.hercules-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed hercules
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure hercules is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.hercules        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hercules/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hercules/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hercules/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hercules/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hercules/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."hercules-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.hercules
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.hercules ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.hercules ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.hercules -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.hercules/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.hercules/$_subdir"
            chmod 2770 "${cfg.stateDir}/.hercules/$_subdir"
            find "${cfg.stateDir}/.hercules/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # hercules-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.hercules/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.hercules/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hercules/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.hercules/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.hercules/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hercules/.managed
          chmod 0644 ${cfg.stateDir}/.hercules/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.hercules/.container-mode <<'HERCULES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hercules_bin=${containerDataDir}/current-package/bin/hercules
    HERCULES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hercules/.container-mode
            chmod 0644 ${cfg.stateDir}/.hercules/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.hercules/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.hercules";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.hercules" ]; then
                  rm -f "${symlinkPath}"
                  echo "hercules-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.hercules -> stateDir/.hercules for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.hercules";
                target = "${cfg.stateDir}/.hercules";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "hercules-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.hercules/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.hercules/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.hercules/auth.json
              fi
            ''}
          ''}
          ...
          # Hercules reads $HERCULES_HOME/.env at startup via load_hercules_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.hercules/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'HERCULES_NIX_ENV_EOF'
    ${envFileContent}
    HERCULES_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.hercules/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.hercules/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hercules/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hercules-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            HERCULES_HOME = "${cfg.stateDir}/.hercules";
            HERCULES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $HERCULES_HOME/.env by the activation script. load_hercules_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/hercules"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the hercules group can read/write them.
            UMask = "0007";
        ...
        systemd.services.hercules-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              HERCULES_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              HERCULES_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env HERCULES_UID="$HERCULES_UID" \
                --env HERCULES_GID="$HERCULES_GID" \
                --env HERCULES_HOME=${containerDataDir}/.hercules \
                --env HERCULES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/hercules gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
