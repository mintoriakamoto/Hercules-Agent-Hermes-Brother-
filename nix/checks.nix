...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      hercules-agent = self'.packages.default;
      herculesVenv = hercules-agent.herculesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "hercules-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${herculesVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "hercules-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "hercules-build-package" { } ''
          echo "PASS: package built at ${hercules-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "hercules-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "hercules-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${hercules-agent}/bin/hercules || (echo "FAIL: hercules binary missing"; exit 1)
          test -x ${hercules-agent}/bin/hercules-agent || (echo "FAIL: hercules-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${hercules-agent}/bin/hercules version 2>&1 | grep -qi "hercules" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hercules-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in hercules hercules-agent hercules-acp; do
            test -x ${hercules-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hercules-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking hercules --help ==="
          ${hercules-agent}/bin/hercules --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${hercules-agent}/bin/hercules --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hercules-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${hercules-agent}/share/hercules-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${hercules-agent}/share/hercules-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERCULES_BUNDLED_SKILLS" ${hercules-agent}/bin/hercules || \
            (echo "FAIL: HERCULES_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: HERCULES_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hercules-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${hercules-agent}/share/hercules-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${hercules-agent}/share/hercules-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERCULES_BUNDLED_PLUGINS" ${hercules-agent}/bin/hercules || \
            (echo "FAIL: HERCULES_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: HERCULES_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "hercules-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${hercules-agent}/share/hercules-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${hercules-agent}/share/hercules-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${hercules-agent}/share/hercules-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HERCULES_BUNDLED_LOCALES" ${hercules-agent}/bin/hercules || \
            (echo "FAIL: HERCULES_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: HERCULES_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (HERCULES_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HERCULES_BUNDLED_LOCALES=${hercules-agent}/share/hercules-agent/locales \
            ${herculesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with HERCULES_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${herculesVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${herculesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without HERCULES_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "hercules-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${hercules-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${hercules-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERCULES_TUI_DIR" ${hercules-agent}/bin/hercules || \
            (echo "FAIL: HERCULES_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: HERCULES_TUI_DIR set in wrapper"
        ...
        # Verify HERCULES_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        hercules-node = pkgs.runCommand "hercules-node-version" { } ''
          set -e
          echo "=== Checking HERCULES_NODE in wrapper ==="
          grep -q "HERCULES_NODE" ${hercules-agent}/bin/hercules || \
            (echo "FAIL: HERCULES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERCULES_NODE present in wrapper"

          HERCULES_NODE=$(sed -n "s/^export HERCULES_NODE='\(.*\)'/\1/p" ${hercules-agent}/bin/hercules)
          test -x "$HERCULES_NODE" || (echo "FAIL: HERCULES_NODE=$HERCULES_NODE not executable"; exit 1)
          echo "PASS: HERCULES_NODE executable at $HERCULES_NODE"

          NODE_MAJOR=$("$HERCULES_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All HERCULES_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify HERCULES_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hercules-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(HERCULES_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking HERCULES_MANAGED guards ==="
          check_blocked "config set" ${hercules-agent}/bin/hercules config set model foo
          check_blocked "config edit" ${hercules-agent}/bin/hercules config edit
        ...
        extra-python-packages = let
          ...
          herculesWithExtra = hercules-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "hercules-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${herculesWithExtra}/bin/hercules || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${herculesWithExtra}/bin/hercules || \
          ...
          if grep -q "PYTHONPATH" ${hercules-agent}/bin/hercules; then
        ...
        extra-dependency-groups = let
          herculesWithGroups = hercules-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "hercules-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${herculesWithGroups}"
          echo "venv: ${herculesWithGroups.herculesVenv}"
        ...
        messaging-variant = pkgs.runCommand "hercules-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.herculesVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "hercules-config-roundtrip" {
          ...
          merge_and_load() {
            local hercules_home="$1"
            export HERCULES_HOME="$hercules_home"
            ${configMergeScript} ${nixSettings} "$hercules_home/config.yaml"
            ${herculesVenv}/bin/python3 -c '
import json, sys
