...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Hercules ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Hercules source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``HERCULES_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
...
# Apply on import — entry points just need ``import hercules_bootstrap``
# (or ``from hercules_bootstrap import apply_windows_utf8_bootstrap``) at
