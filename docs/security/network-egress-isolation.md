...
When running Hercules inside Docker, the default `network_mode: host` gives the
...
The Hercules [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ hercules-agent │   │ hercules-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=hercules,hercules-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://hercules-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Hercules trust model and vulnerability reporting
