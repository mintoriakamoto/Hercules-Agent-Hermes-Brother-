...
Hercules middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Hercules'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Hercules calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Hercules retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Hercules guardrails,
...
- `telemetry_schema_version`: currently `hercules.observer.v1`
- `middleware_schema_version`: currently `hercules.middleware.v1`
...
Hercules stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Hercules runs
them as a nested chain in registration order. Middleware failures are fail-open:
Hercules logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Hercules applies middleware in this order:
...
For each tool call, Hercules applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Hercules pre-execution path against the effective arguments:
...
```bash
hercules plugins enable <plugin-name>
```

For isolated local testing, use one `HERCULES_HOME` for plugin enablement and the
agent run:

```bash
export HERCULES_HOME=/tmp/hercules-middleware-test
mkdir -p "$HERCULES_HOME"
hercules plugins enable <plugin-name>
hercules chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run hercules plugins enable <plugin-name>
uv run hercules chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["hercules_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/hercules-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Hercules expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Hercules treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Hercules preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Hercules does not convert downstream
