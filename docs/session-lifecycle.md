...
The canonical transcript store is SQLite via `SessionDB` (from `hercules_state`). The
...
Counts consecutive restarts via a JSON file (`{HERCULES_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `hercules update`, `hercules gateway restart`,
or `/restart`.
