# Hercules Observer Hooks

Hercules observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Hercules' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Hercules catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Hercules only builds
