...
This document is the formal interface between the **Hercules gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `hercules gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`hercules gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `hercules` CLI.
