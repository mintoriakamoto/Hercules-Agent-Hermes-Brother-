...
Hercules supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.hercules/config.yaml`:
...
Or set the env var: `HERCULES_KANBAN_DISPATCH_IN_GATEWAY=false`
