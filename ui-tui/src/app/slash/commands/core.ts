import { forceRedraw, type MouseTrackingMode } from '@hercules/ink'
...
export const coreCommands: SlashCommand[] = [
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      // Exit code 42 signals the Python wrapper to exec `hercules update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try HERCULES_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Hercules #${i + 1}`
