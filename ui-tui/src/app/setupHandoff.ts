import type { RunExternalProcess } from '@hercules/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`hercules ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching hercules: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`hercules ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
