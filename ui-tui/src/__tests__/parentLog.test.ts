...
// ~/.hercules. To exercise the real persistence path we clear that gate, point
// HERCULES_HOME at a temp dir, and re-import the module fresh (path + enabled
// flag are captured at module load).
const loadFresh = async (home: string) => {
  vi.resetModules()
  vi.stubEnv('VITEST', '')
  vi.stubEnv('HERCULES_HOME', home)
...
describe('recordParentLifecycle', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'hercules-parentlog-'))
  })
