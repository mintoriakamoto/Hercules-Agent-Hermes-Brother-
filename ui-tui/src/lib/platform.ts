 ...
 * hercules-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``hercules_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but hercules-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS hercules-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on hercules-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is hercules-ink's Alt signal and accepting it would
