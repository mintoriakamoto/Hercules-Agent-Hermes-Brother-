...
const logDir = join(process.env.HERCULES_HOME?.trim() || join(homedir(), '.hercules'), 'logs')
...
// a real ~/.hercules (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('hercules-tui: parent lifecycle log unavailable\n')
    }
  }
}
