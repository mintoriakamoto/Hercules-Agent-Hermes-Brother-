...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because HERCULES_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.HERCULES_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.hercules', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `hercules-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.HERCULES_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.HERCULES_HEAPDUMP_MAX_BYTES?.trim()
