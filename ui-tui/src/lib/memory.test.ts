...
const ENV_KEYS = ['HERCULES_AUTO_HEAPDUMP', 'HERCULES_HEAPDUMP_DIR', 'HERCULES_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'hercules-heapdump-test-'))
    process.env.HERCULES_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without HERCULES_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without HERCULES_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when HERCULES_AUTO_HEAPDUMP=1', async () => {
    process.env.HERCULES_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.HERCULES_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of HERCULES_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.HERCULES_HEAPDUMP_DIR
    savedMax = process.env.HERCULES_HEAPDUMP_MAX_BYTES
    delete process.env.HERCULES_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'hercules-heapdump-prune-'))
    process.env.HERCULES_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.HERCULES_HEAPDUMP_DIR
    } else {
      process.env.HERCULES_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.HERCULES_HEAPDUMP_MAX_BYTES
    } else {
      process.env.HERCULES_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.HERCULES_HEAPDUMP_MAX_BYTES = String(2 * 1024)
