...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.HERCULES_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Hercules on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via HERCULES_TUI_TRUECOLOR=1.
