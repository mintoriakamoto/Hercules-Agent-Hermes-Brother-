...
React + Ink terminal UI for Hercules. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `HERCULES_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `hercules setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.hercules/.hercules_history` or under `HERCULES_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `hercules setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/hercules-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external hercules setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.hercules/tui-parent.log
    ...
    types/
      hercules-ink.d.ts            type declarations for @hercules/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent HerculesCLI subprocess for slash commands
```