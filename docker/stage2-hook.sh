...
#
# Wired into the image as /etc/cont-init.d/01-hercules-setup by the
...
HERCULES_HOME="${HERCULES_HOME:-/opt/data}"
INSTALL_DIR="/opt/hercules"

# Drop to hercules via s6-setuidgid, but skip it when already non-root.
as_hercules() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid hercules "$@"; }
...
# starts non-root. The baked install tree under /opt/hercules is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $HERCULES_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass HERCULES_UID/HERCULES_GID — or the PUID/PGID aliases — which the
...
# `id -u hercules` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# hercules UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hercules)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-hercules UID).
...
and the baked /opt/hercules install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e HERCULES_UID=\$(id -u) -e HERCULES_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the hercules user to that UID/GID at boot and chowns the data
...
# --- Bootstrap HERCULES_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hercules mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom HERCULES_HOME paths whose parents only
# root can create (e.g. `HERCULES_HOME=/home/hercules/.hercules` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for HERCULES_UID/HERCULES_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  HERCULES_UID/
# HERCULES_GID still win when both are set.  See #15290, salvages #25872.
HERCULES_UID="${HERCULES_UID:-${PUID:-}}"
HERCULES_GID="${HERCULES_GID:-${PGID:-}}"

if [ -n "${HERCULES_UID:-}" ] && validate_uid_gid "$HERCULES_UID" && [ "$HERCULES_UID" != "$(id -u hercules)" ]; then
    echo "[stage2] Changing hercules UID to $HERCULES_UID"
    usermod -u "$HERCULES_UID" hercules
fi
if [ -n "${HERCULES_GID:-}" ] && validate_uid_gid "$HERCULES_GID" && [ "$HERCULES_GID" != "$(id -g hercules)" ]; then
    echo "[stage2] Changing hercules GID to $HERCULES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HERCULES_GID" hercules 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised hercules user (UID 10000)
...
# hercules process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes hercules. Idempotent across container
...
#   - hercules is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G hercules 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] hercules already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" hercules 2>/dev/null; then
        echo "[stage2] Added hercules to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group hercules failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When HERCULES_UID is remapped or the top-level $HERCULES_HOME isn't owned by
# the runtime hercules UID, restore ownership to hercules — but ONLY for the
# directories hercules actually writes to. The full $HERCULES_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of hercules-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hercules_uid=$(id -u hercules)

path_has_symlink_component() {
    path="$1"
    root="${2:-$HERCULES_HOME}"
...
chown_hercules_tree() {
    ...
    chown -R hercules:hercules "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$HERCULES_HOME" 2>/dev/null)" != "$actual_hercules_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $HERCULES_HOME (targeted) to hercules ($actual_hercules_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $HERCULES_HOME: chown the directory itself (not its contents)
    # so hercules can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$HERCULES_HOME"; then
        :
    else
        chown hercules:hercules "$HERCULES_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $HERCULES_HOME failed (rootless container?) — continuing"
    fi
    # Hercules-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by hercules (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$HERCULES_HOME/$sub" ]; then
            chown_hercules_tree "$HERCULES_HOME/$sub"
        fi
    done
fi
...
# hercules user. Hosted/container instances keep mutable state under
# $HERCULES_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# HERCULES_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hercules root-owned and
...
# $HERCULES_HOME/lazy-packages dir on the data volume (Dockerfile sets
# HERCULES_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to hercules
...
# Always reset ownership of $HERCULES_HOME/profiles to hercules on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> hercules …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as hercules and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$HERCULES_HOME/profiles" ]; then
    chown_hercules_tree "$HERCULES_HOME/profiles"
fi

# Always reset ownership of $HERCULES_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged hercules runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$HERCULES_HOME/cron" ]; then
    chown_hercules_tree "$HERCULES_HOME/cron"
fi
...
# hercules pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged hercules gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $HERCULES_HOME is
...
# is negligible.
if [ -d "$HERCULES_HOME/platforms/pairing" ]; then
    chown_hercules_tree "$HERCULES_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$HERCULES_HOME/pairing" ]; then
    chown_hercules_tree "$HERCULES_HOME/pairing"
fi

# Reset ownership of hercules-owned top-level state files on every boot.
# The targeted data-volume chown above only covers hercules-owned
# *subdirectories*; loose state files living directly under $HERCULES_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> hercules …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged hercules runtime then hits
...
# sweep so host-owned files in a bind-mounted $HERCULES_HOME are never
...
# entries of hercules_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$HERCULES_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$HERCULES_HOME/$f"; then
            :
        else
            chown hercules:hercules "$HERCULES_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the hercules runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$HERCULES_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$HERCULES_HOME/config.yaml"; then
        :
    else
        chown hercules:hercules "$HERCULES_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$HERCULES_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as hercules user ---
# Run as hercules via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $HERCULES_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_hercules mkdir -p \
    "$HERCULES_HOME/backups" \
    "$HERCULES_HOME/cron" \
    "$HERCULES_HOME/sessions" \
    "$HERCULES_HOME/logs" \
    "$HERCULES_HOME/logs/gateways" \
    "$HERCULES_HOME/hooks" \
    "$HERCULES_HOME/memories" \
    "$HERCULES_HOME/skills" \
    "$HERCULES_HOME/skins" \
    "$HERCULES_HOME/plans" \
    "$HERCULES_HOME/workspace" \
    "$HERCULES_HOME/home" \
    "$HERCULES_HOME/pairing" \
    "$HERCULES_HOME/platforms/pairing" \
    "$HERCULES_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/hercules/.install_method (see Dockerfile), NOT written here into
# $HERCULES_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $HERCULES_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.hercules:/opt/data) and sometimes shared with a
...
# 'hercules update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $HERCULES_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/hercules stamp, so deleting the data-dir copy is safe).
if [ -f "$HERCULES_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$HERCULES_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$HERCULES_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$HERCULES_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$HERCULES_HOME/$dest"; then
            :
        else
            as_hercules cp "$INSTALL_DIR/$src" "$HERCULES_HOME/$dest"
        fi
    fi
}
...
if [ -f "$HERCULES_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$HERCULES_HOME/.env"; then
        :
    else
        chown hercules:hercules "$HERCULES_HOME/.env" 2>/dev/null || true
        chmod 600 "$HERCULES_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $HERCULES_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `hercules update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set HERCULES_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$HERCULES_HOME/config.yaml" ]; then
    s6-setuidgid hercules "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$HERCULES_HOME/auth.json" ] && [ -n "${HERCULES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$HERCULES_HOME/auth.json"; then
        :
    else
        printf '%s' "$HERCULES_AUTH_JSON_BOOTSTRAP" > "$HERCULES_HOME/auth.json"
        chown hercules:hercules "$HERCULES_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$HERCULES_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via HERCULES_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$HERCULES_HOME/auth.json" ] && [ -n "${HERCULES_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$HERCULES_HOME/auth.json"; then
        :
    else
        s6-setuidgid hercules "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$HERCULES_HOME/auth.json" \
...
# can set HERCULES_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# HERCULES_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$HERCULES_HOME/gateway_state.json" ] && \
        [ "${HERCULES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$HERCULES_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$HERCULES_HOME/gateway_state.json"
        chown hercules:hercules "$HERCULES_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$HERCULES_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_hercules "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/hercules/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Hercules spawns for the
...
# main-wrapper.sh propagates it into the supervised ``hercules`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-hercules, dashboard, etc.).
