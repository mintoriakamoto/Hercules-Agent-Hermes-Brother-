# Hercules Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: hercules-achievements plugin
...
### `GET /api/plugins/hercules-achievements/achievements`
...
### `POST /api/plugins/hercules-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/hercules-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/hercules-achievements/overview`
...
- Persist plugin data under:
  - `~/.hercules/plugins/hercules-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.hercules/plugins/hercules-achievements/scan_checkpoint.json`
...
- `~/.hercules/plugins/hercules-achievements/`
