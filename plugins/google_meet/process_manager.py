...
session-scoped state file under ``$HERCULES_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $HERCULES_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_hercules_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this hercules install, leave it first —
    ...
    env = os.environ.copy()
    env["HERCULES_MEET_URL"] = url
    env["HERCULES_MEET_OUT_DIR"] = str(out)
    env["HERCULES_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["HERCULES_MEET_HEADED"] = "1"
    if auth_state:
        env["HERCULES_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["HERCULES_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if HERCULES_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["HERCULES_MEET_MODE"] = mode
    if realtime_model:
        env["HERCULES_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["HERCULES_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["HERCULES_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["HERCULES_MEET_REALTIME_KEY"] = realtime_api_key
