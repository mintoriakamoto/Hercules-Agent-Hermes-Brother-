...
Let the hercules agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where hercules runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hercules meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hercules meet` CLI |
...
| `cli.py` | `hercules meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HERCULES_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `hercules meet node {run,list,approve,remove,status,ping}` |
...
hercules meet install                                      # pip + Chromium
hercules meet setup                                        # preflight
hercules meet auth                                         # optional
hercules meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hercules meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hercules/.env
hercules meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hercules meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hercules meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hercules/.env
hercules meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, hercules will **not** switch your system audio input automatically — the
...
hercules plugins enable google_meet
hercules meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
hercules meet node ping my-mac
