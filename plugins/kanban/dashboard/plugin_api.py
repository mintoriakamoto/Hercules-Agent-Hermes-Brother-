...
``hercules_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__HERCULES_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``hercules_cli/web_server.py``.

This means ``hercules dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from hercules_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``hercules_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from hercules_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``hercules_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from hercules_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from hercules_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``HERCULES_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from hercules_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``hercules kanban
...
# spawn failures, stuck-blocked). See hercules_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``hercules kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``hercules kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``hercules kanban specify <task_id>``.
    ...
    # HERCULES_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from hercules_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``hercules kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.hercules/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Hercules profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.hercules/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from hercules_constants import get_hercules_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_hercules_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``hercules profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from hercules_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``hercules kanban decompose <task_id>``.
    ...
    # HERCULES_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from hercules_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.hercules/config.yaml.
