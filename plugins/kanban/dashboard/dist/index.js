/**
 * Hercules Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.hercules/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__HERCULES_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "hercules-kanban-dot-triage",
    todo: "hercules-kanban-dot-todo",
    ready: "hercules-kanban-dot-ready",
    running: "hercules-kanban-dot-running",
    blocked: "hercules-kanban-dot-blocked",
    done: "hercules-kanban-dot-done",
    archived: "hercules-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-hercules-task";
  ...
  const DOCS_URL = "https://hercules-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://hercules-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "hercules.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="hercules-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])hercules-kanban-md-code\1/i.test(attrs)) {
      return ' class="hercules-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "hercules-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "hercules-kanban-md",
  ...
  // standard `drop` event and our `hercules-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("hercules-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("hercules-kanban-column--drop");
          if (target) target.classList.add("hercules-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("hercules-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("hercules-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("hercules-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__HERCULES_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "hercules-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "hercules-kanban-attention",
        "hercules-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "hercules-kanban-attention-bar" },
        h("span", { className: "hercules-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "hercules-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "hercules-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "hercules-kanban-attention-row",
                  "hercules-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "hercules-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "hercules-kanban-attention-row-id" }, task.id),
                h("span", { className: "hercules-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "hercules-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "hercules-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "hercules-kanban-diag-action-btn",
      action.suggested ? "hercules-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " hercules-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".hercules-kanban-drawer-comment-row input, .hercules-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "hercules-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("hercules-kanban-diag", sevClass) },
      h("div", { className: "hercules-kanban-diag-header" },
        h("span", { className: "hercules-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "hercules-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "hercules-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "hercules-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "hercules-kanban-diag-data-row" },
                  h("span", { className: "hercules-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "hercules-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "hercules-kanban-diag-data-row" },
                h("span", { className: "hercules-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "hercules-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "hercules-kanban-diag-reassign-row" },
            h("span", { className: "hercules-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "hercules-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "hercules-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "hercules-kanban-diag-msg",
              msg.ok ? "hercules-kanban-diag-msg--ok" : "hercules-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "hercules-kanban-section" },
      h("div", { className: "hercules-kanban-section-head-row" },
        h("span", { className: "hercules-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "hercules-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "hercules-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "hercules-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "hercules-kanban-docs-link",
      title: "Open Hercules Kanban docs in a new tab",
      "aria-label": "Hercules Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `hercules kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "hercules-kanban-msg-ok" : "hercules-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "hercules-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "hercules-kanban-boardswitcher" },
      h("div", { className: "hercules-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "hercules-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "hercules-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "hercules-kanban-dialog-title" },
        ...
        h("div", { className: "hercules-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Hercules profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "hercules-kanban-bulk" },
      h("span", { className: "hercules-kanban-bulk-count" },
      ...
      h("div", { className: "hercules-kanban-bulk-priority",
      ...
      h("div", { className: "hercules-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Hercules profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "hercules-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("hercules-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("hercules-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "hercules-kanban-trash",
        dragOver ? "hercules-kanban-trash--drop" : "",
        props.draggingTaskId ? "hercules-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "hercules-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "hercules-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".hercules-kanban-card")) return true;
      if (target.closest && target.closest(".hercules-kanban-column-add")) return true;
      if (target.closest && target.closest(".hercules-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("hercules-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("hercules-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".hercules-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "hercules-kanban-columns",
        isScrollable ? "hercules-kanban-columns--scrollable" : "",
        isPanning ? "hercules-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("hercules-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("hercules-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "hercules-kanban-column",
        dragOver ? "hercules-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "hercules-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "hercules-kanban-col-check",
        ...
        h("span", { className: cn("hercules-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "hercules-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "hercules-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "hercules-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "hercules-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "hercules-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "hercules-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "hercules-kanban-lane" },
                  h("div", { className: "hercules-kanban-lane-head" },
                    h("span", { className: "hercules-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "hercules-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "hercules-kanban-card--stale-red";
    if (age >= tier.amber) return "hercules-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".hercules-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "hercules-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "hercules-kanban-card",
        props.selected ? "hercules-kanban-card--selected" : "",
        props.failed ? "hercules-kanban-card--failed" : "",
        props.draggingSource ? "hercules-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "hercules-kanban-card-content" },
          h("div", { className: "hercules-kanban-card-row" },
            h("label", {
              className: "hercules-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "hercules-kanban-card-check",
            ...
            h("span", { className: "hercules-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or hercules kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "hercules-kanban-warning-badge",
                    "hercules-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "hercules-kanban-needs-assignee",
          ...
          h("div", { className: "hercules-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "hercules-kanban-card-row hercules-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "hercules-kanban-assignee",
                            title: `Assigned to Hercules profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "hercules-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "hercules-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "hercules-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "hercules-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Hercules profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Hercules profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "hercules-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "hercules-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "hercules-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "hercules-kanban-drawer-close",
        ...
        data ? h("div", { className: "hercules-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "hercules-kanban-section" },
      h("div", { className: "hercules-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "hercules-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "hercules-kanban-drawer-body" },
      h("div", { className: "hercules-kanban-drawer-title" },
        h("span", { className: cn("hercules-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "hercules-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "hercules-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "hercules-kanban-section" },
        h("div", { className: "hercules-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "hercules-kanban-section" },
        h("div", { className: "hercules-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "hercules-kanban-comment" },
                h("div", { className: "hercules-kanban-comment-head" },
                  h("span", { className: "hercules-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "hercules-kanban-comment-ago" },
      ...
      h("div", { className: "hercules-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "hercules-kanban-event",
              isDiag ? "hercules-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "hercules-kanban-event-header" },
                  h("span", { className: "hercules-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "hercules-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "hercules-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "hercules-kanban-event-header-plain" },
                  h("span", { className: "hercules-kanban-event-kind" }, e.kind),
                  h("span", { className: "hercules-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "hercules-kanban-event-phantom-row" },
                  h("span", { className: "hercules-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "hercules-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "hercules-kanban-event-payload" },
    ...
    return h("div", { className: "hercules-kanban-section" },
      h("div", { className: "hercules-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "hercules-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `hercules-kanban-run--${r.outcome || r.status || "ended"}`
          : "hercules-kanban-run--active";
        return h("div", { key: r.id, className: cn("hercules-kanban-run", outcomeClass) },
          h("div", { className: "hercules-kanban-run-head" },
            h("span", { className: "hercules-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "hercules-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "hercules-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "hercules-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "hercules-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "hercules-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "hercules-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "hercules-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "hercules-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "hercules-kanban-pre hercules-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "hercules-kanban-meta-row" },
      h("span", { className: "hercules-kanban-meta-label" }, props.label),
      h("span", { className: "hercules-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "hercules-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "hercules-kanban-meta-row" },
        h("span", { className: "hercules-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "hercules-kanban-meta-value hercules-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "hercules-kanban-meta-row" },
        h("span", { className: "hercules-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "hercules-kanban-meta-value hercules-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "hercules-kanban-section" },
      h("div", { className: "hercules-kanban-section-head-row" },
        h("span", { className: "hercules-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "hercules-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "hercules-kanban-section" },
      h("div", { className: "hercules-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "hercules-kanban-deps-row" },
        h("span", { className: "hercules-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "hercules-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "hercules-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "hercules-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "hercules-kanban-dep-chip-x",
      ...
      h("div", { className: "hercules-kanban-deps-row" },
        h("span", { className: "hercules-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "hercules-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "hercules-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "hercules-kanban-msg-ok"
          : "hercules-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "hercules-kanban-section" },
      ...
      h("div", { className: "hercules-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "hercules-kanban-home-sub hercules-kanban-home-sub--on"
              : "hercules-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__HERCULES_PLUGINS__ && typeof window.__HERCULES_PLUGINS__.register === "function") {
    window.__HERCULES_PLUGINS__.register("kanban", KanbanPage);
  }
})();
