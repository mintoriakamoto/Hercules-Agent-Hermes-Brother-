...
three virtual model IDs so the ``hercules tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `hercules tools` → Image "
                    "Generation → OpenAI to configure, or `hercules setup` "
                    "to add the key."
                ),
