...
Credential storage mirrors every other Hercules channel:

    * runtime SDK creds  -> ``~/.hercules/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.hercules/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a hercules dependency
...
# registers Hercules as its own client_id.
...
# Default name of the project Hercules provisions for the operator.
DEFAULT_PROJECT_NAME = "Hercules Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of hercules-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.hercules/auth.json`` honouring the active Hercules profile."""
    try:
        from hercules_constants import get_hercules_home
        return Path(get_hercules_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.hercules")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.hercules/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.hercules/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.hercules/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: hercules_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `hercules photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `hercules photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `hercules photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `hercules photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `hercules photon setup`)"
