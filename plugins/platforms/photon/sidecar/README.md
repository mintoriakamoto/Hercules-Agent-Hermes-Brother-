...
Small Node helper that bridges Hercules Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Hercules is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Hercules-Sidecar-Token`)
...
The Hercules plugin's `hercules photon setup` command runs `npm install`
here automatically.
