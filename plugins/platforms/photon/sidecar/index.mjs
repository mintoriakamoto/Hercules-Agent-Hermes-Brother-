// Hercules Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Hercules): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Hercules -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Hercules-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `hercules photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Hercules can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Hercules'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Hercules allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Hercules, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-hercules-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Hercules supplied them so a known-good
