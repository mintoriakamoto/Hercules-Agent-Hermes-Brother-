...
This plugin connects Hercules Agent to iMessage (and other Spectrum
...
TypeScript-only, Hercules runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Hercules-Sidecar-Token`.
...
There is no separate `login` command; like every other Hercules channel,
...
Run `hercules photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.hercules/.env` (the same place every other
...
Management metadata lives in `~/.hercules/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `hercules photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `hercules photon status`, a DM and a group roundtrip,
