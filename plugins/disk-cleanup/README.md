...
Scoped strictly to `$HERCULES_HOME` and `/tmp/hercules-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HERCULES_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `HERCULES_HOME` or `/tmp/hercules-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HERCULES_HOME/disk-cleanup/` is itself excluded
- `$HERCULES_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
