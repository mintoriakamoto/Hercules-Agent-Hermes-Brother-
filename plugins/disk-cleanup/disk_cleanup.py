...
Rules:
  ...
  - empty dirs    → always delete (under HERCULES_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly HERCULES_HOME and /tmp/hercules-*
Never touches: ~/.hercules/logs/ or any system directory.
"""
...
try:
    ...
    def get_hercules_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("HERCULES_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".hercules").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$HERCULES_HOME/logs/``."""
    return get_hercules_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$HERCULES_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HERCULES_HOME or ``/tmp/hercules-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    hercules_home = get_hercules_home()
    try:
        path.resolve().relative_to(hercules_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/hercules-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("hercules-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "hercules-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $HERCULES_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so HERCULES_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        hercules_home = get_hercules_home()
        for parent in ("cron", "cronjobs"):
            base = hercules_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside HERCULES_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HERCULES_HOME, but never recurse into known
    # durable state trees.  Some installs place the Hercules checkout, venv,
    # and desktop build under HERCULES_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    hercules_home = get_hercules_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in hercules_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    hercules_home = get_hercules_home()
    try:
        rel = path.resolve().relative_to(hercules_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "hercules-agent",
        }:
            return None
        ...
        # Path isn't under HERCULES_HOME (e.g. /tmp/hercules-*) — fall through.
        pass
