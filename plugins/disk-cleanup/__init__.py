"""disk-cleanup plugin — auto-cleanup of ephemeral Hercules session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``HERCULES_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$HERCULES_HOME/disk-cleanup/cleanup.log``.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/hercules-x/test_foo.py`
        try:
...
All operations are scoped to HERCULES_HOME and /tmp/hercules-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside HERCULES_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Hercules session files.",
    )
