...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Hercules peer ID in OpenViking (default: hercules)
...
_DEFAULT_AGENT = "hercules"
_AGENT_PROMPT_LABEL = "Hercules peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "HERCULES_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Hercules slash-skill scaffolding before sending content to OpenViking.
...
def _load_hercules_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("HERCULES_HOME", "")).expanduser() if os.environ.get("HERCULES_HOME") else Path.home() / ".hercules"
...
def _save_hercules_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Hercules session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Hercules only", "write values only to Hercules .env"),
    ...
    _save_hercules_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Hercules .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See hercules-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_hercules_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Hercules peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, hercules_home: str, config: dict) -> None:
        ...
        from hercules_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        hercules_home_path = Path(hercules_home)
        env_path = hercules_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_hercules_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Hercules run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Hercules' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Hercules canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See hercules-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/hercules/.overview.md).
