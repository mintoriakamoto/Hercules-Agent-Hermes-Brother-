...
connection values into Hercules, or create a minimal `ovcli.conf` when one does
not exist.
...
| `OPENVIKING_AGENT` | `hercules` | Hercules peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Hercules lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Hercules sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Hercules built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Hercules action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Hercules
...
file URIs, such as
`viking://user/peers/hercules/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/hercules/memories/preferences/mem_abc123.md`. Files
