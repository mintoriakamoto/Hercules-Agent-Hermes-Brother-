...
- Supermemory API key from [app.supermemory.ai/integrations?connect=hercules](http://app.supermemory.ai/integrations?connect=hercules)
...
Config file: `$HERCULES_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `hercules` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hercules-{identity}` → `hercules-coder`). |
...
All Supermemory API calls send `x-sm-source: hercules`, and document writes stamp
`metadata.sm_source: hercules`. This is a **functional routing key, not telemetry**:
it groups Hercules-written memories into a dedicated "Hercules" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Hercules profile:

```json
{
  "container_tag": "hercules-{identity}"
}
```

For a profile named `coder`, this resolves to `hercules-coder`. The default profile resolves to `hercules-default`. Without `{identity}`, all profiles share the same container.
