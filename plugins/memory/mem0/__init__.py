...
Configuration
-------------
Secret (lives in $HERCULES_HOME/.env or the environment):
...
Behavioral settings (live in $HERCULES_HOME/mem0.json, set via `hercules memory
setup`):
  ...
  agent_id           — Agent identifier (default: hercules)
...
_DEFAULT_USER_ID = "hercules-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $HERCULES_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "hercules"),
        "oss": {},
    }
    ...
    config_path = get_hercules_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hercules_home):
        """Write config to $HERCULES_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, hercules_home: str, config: dict) -> None:
        ...
        post_setup(hercules_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $HERCULES_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "hercules")
