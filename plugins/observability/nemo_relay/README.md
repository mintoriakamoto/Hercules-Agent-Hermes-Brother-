...
Optional Hercules observability plugin that maps Hercules observer hooks to
...
not replace Hercules Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Hercules emit NeMo Relay lifecycle events for
the work Hercules already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Hercules Agent can:

- Preserve Hercules execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
hercules plugins enable observability/nemo_relay
```

The `HERCULES_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `HERCULES_HOME` that the
agent run will use:

```bash
env HERCULES_HOME=/tmp/hercules-nemo-relay-test \
  hercules plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`HERCULES_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`HERCULES_HOME` is the Hercules profile/config home used by both
`hercules plugins enable ...` and the later `hercules chat ...` run. If unset,
Hercules uses the user's default home, usually `~/.hercules`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export HERCULES_HOME=/tmp/hercules-nemo-relay-test
hercules plugins enable observability/nemo_relay
hercules chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `hercules` command you run is built from the
...
uv run hercules plugins enable observability/nemo_relay
uv run hercules chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/hercules_agent-*.whl
...
The plugin can configure exporters directly from `HERCULES_NEMO_RELAY_*`
...
```bash
export HERCULES_NEMO_RELAY_ATOF_ENABLED=1
export HERCULES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export HERCULES_NEMO_RELAY_ATIF_ENABLED=1
export HERCULES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `HERCULES_NEMO_RELAY_ATOF_FILENAME`
- `HERCULES_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `HERCULES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `HERCULES_NEMO_RELAY_ATIF_AGENT_NAME`
- `HERCULES_NEMO_RELAY_ATIF_AGENT_VERSION`
- `HERCULES_NEMO_RELAY_ATIF_MODEL_NAME`
- `HERCULES_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Hercules at it:

```bash
export HERCULES_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `HERCULES_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`HERCULES_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`HERCULES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Hercules does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Hercules keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Hercules routes LLM and tool
...
For the full generic Hercules middleware contract, see
...
export HERCULES_HOME=/tmp/hercules-nemo-relay-docs/hercules-home
mkdir -p "$HERCULES_HOME"

cat > "$HERCULES_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Hercules session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HERCULES_NEMO_RELAY_ATOF_ENABLED=1
export HERCULES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hercules-nemo-relay-docs/subagent/atof
export HERCULES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HERCULES_NEMO_RELAY_ATOF_MODE=overwrite
export HERCULES_NEMO_RELAY_ATIF_ENABLED=1
export HERCULES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hercules-nemo-relay-docs/subagent/atif
export HERCULES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HERCULES_NEMO_RELAY_ATIF_AGENT_NAME='Hercules Agent E2E'
export HERCULES_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export HERCULES_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"hercules.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Hercules Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Hercules dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/hercules-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/hercules-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/hercules-nemo-relay-docs/workdir/beta.txt
cd /tmp/hercules-nemo-relay-docs/workdir

export HERCULES_NEMO_RELAY_ATOF_ENABLED=1
export HERCULES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hercules-nemo-relay-docs/parallel/atof
export HERCULES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HERCULES_NEMO_RELAY_ATOF_MODE=overwrite
export HERCULES_NEMO_RELAY_ATIF_ENABLED=1
export HERCULES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hercules-nemo-relay-docs/parallel/atif
export HERCULES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export HERCULES_NEMO_RELAY_ATIF_AGENT_NAME='Hercules Agent E2E'
export HERCULES_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Hercules sessions map to `agent` scopes.
- Hercules API request hooks map to `llm` scope start/end events.
- Hercules tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Hercules execution middleware to hand
...
Enable it for Hercules:

```bash
export HERCULES_NEMO_RELAY_PLUGINS_TOML=/tmp/hercules-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Hercules routes execution
through these boundaries:

```text
Hercules provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Hercules provider adapter next_call(...)

Hercules tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Hercules tool dispatcher next_call(...)
```
...
middleware for a local Hercules run. This path requires a NeMo Relay runtime that
...
```bash
export HERCULES_HOME=/tmp/hercules-middleware-test/hercules-home
mkdir -p "$HERCULES_HOME" /tmp/hercules-middleware-test/nemo-relay
...
cat > /tmp/hercules-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/hercules-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/hercules-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Hercules Middleware E2E"
agent_version = "local"
...
export HERCULES_NEMO_RELAY_PLUGINS_TOML=/tmp/hercules-middleware-test/nemo-relay/plugins.toml
