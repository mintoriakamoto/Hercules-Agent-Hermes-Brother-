...
* Never raises. Any I/O error (disk full, permission denied, missing
  HERCULES_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_hercules_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: HERCULES_HOME env var, then ~/.hercules
            home = os.environ.get("HERCULES_HOME") or os.path.expanduser("~/.hercules")
