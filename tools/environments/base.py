"""Base class for all Hercules execution environment backends.
...
# HERCULES_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {HERCULES_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_hercules_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__HERCULES_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Hercules backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/hercules-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/hercules-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../hercules-snap-*.sh: No such file or directory``
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("__hercules_ec=$?")
        # Restrict Hercules metadata files without changing the user's command
        ...
        parts.append("exit $__hercules_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"HERCULES_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via HERCULES_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __HERCULES_CWD_{session}__ marker from stdout output.
