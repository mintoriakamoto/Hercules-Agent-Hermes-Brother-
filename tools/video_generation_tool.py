...
provider** — turn on a backend by enabling a plugin (``hercules plugins
enable video_gen/<name>``) and selecting it in ``hercules tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/hercules-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `hercules tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `hercules tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `hercules tools` → "
...
# `hercules tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`hercules tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `hercules tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
