...
running under the dispatcher (env var ``HERCULES_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``hercules chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``hercules kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``hercules kanban complete …``
   inside the container, where ``hercules`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.hercules/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``hercules kanban …``), the dashboard
(``hercules dashboard``), and the slash command (``/kanban …``) — all
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("HERCULES_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("HERCULES_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("HERCULES_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``HERCULES_KANBAN_TASK`` set
    ...
    ``HERCULES_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``HERCULES_KANBAN_DB`` → ``HERCULES_KANBAN_BOARD`` env → current symlink
...
#   - No-op outside dispatcher-spawned worker context (no ``HERCULES_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``HERCULES_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``HERCULES_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``HERCULES_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("HERCULES_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("HERCULES_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("HERCULES_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set HERCULES_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # HERCULES_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``hercules-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("HERCULES_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("HERCULES_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets HERCULES_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("HERCULES_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (HERCULES_KANBAN_TASK set)
    ...
    # (kanban toolset, no HERCULES_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("HERCULES_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("HERCULES_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``hercules dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``HERCULES_SESSION_PLATFORM``
      and ``HERCULES_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``HERCULES_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # HERCULES_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # HERCULES_SESSION_ID is intentionally NOT a fallback here:
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to HERCULES_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: HERCULES_KANBAN_DB env → "
    "HERCULES_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to HERCULES_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
