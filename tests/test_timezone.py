"""
Tests for timezone support (hercules_time module + integration points).
...
import hercules_time


def _reset_hercules_time_cache():
    """Reset the hercules_time module cache (replacement for removed reset_cache)."""
    hercules_time._cached_tz = None
    hercules_time._cached_tz_name = None
    hercules_time._cache_resolved = False


# =========================================================================
# hercules_time.now() — core helper
# =========================================================================

class TestHerculesTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_hercules_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["HERCULES_TIMEZONE"] = "Asia/Kolkata"
        result = hercules_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["HERCULES_TIMEZONE"] = "UTC"
        result = hercules_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["HERCULES_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="hercules_time"):
            result = hercules_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["HERCULES_TIMEZONE"] = "UTC"
        _reset_hercules_time_cache()
        r1 = hercules_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["HERCULES_TIMEZONE"] = "Asia/Kolkata"
        _reset_hercules_time_cache()
        r2 = hercules_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["HERCULES_TIMEZONE"] = "Europe/London"
        tz = hercules_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["HERCULES_TIMEZONE"] = "Not/A/Timezone"
        tz = hercules_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When HERCULES_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) HERCULES_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("HERCULES_TIMEZONE=" + os.environ.get("HERCULES_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "HERCULES_TIMEZONE=NOT_SET" in result["output"], (
            "HERCULES_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When HERCULES_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("HERCULES_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=hercules_tz) which shifted
        absolute time when system-local tz != Hercules tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hercules_tz(self):
        """Already-aware datetimes should be normalized to Hercules tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Hercules tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Hercules tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Hercules is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Hercules tz
        ...
        # Use a Hercules timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _hercules_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["HERCULES_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hercules_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Hercules timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["HERCULES_TIMEZONE"] = "US/Eastern"
        _reset_hercules_time_cache()
