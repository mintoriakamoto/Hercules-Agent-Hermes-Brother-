...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "hercules"
        assert config.workspace_id == "hercules"
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="hercules-agent"
        ):
            result = config.resolve_session_name("/home/user/hercules-agent/subdir")
...
class TestResolveConfigPath:
    def test_prefers_hercules_home_when_exists(self, tmp_path):
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: HERCULES_HOME points at ~/.hercules/profiles/<name>, so
        # _get_default_hercules_home() must resolve back to ~/.hercules — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".hercules"
        ...
        assert _get_default_hercules_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_hercules_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "hercules_coder"
        assert profile_host_key("default") == "hercules"

    def test_default_returns_hercules(self):
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HERCULES_HONCHO_HOST": "hercules.coder"}):
            assert resolve_active_host() == "hercules.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            saved = sys.modules.get("hercules_cli.profiles")
            sys.modules["hercules_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "hercules"
            finally:
                if saved is not None:
                    sys.modules["hercules_cli.profiles"] = saved
                else:
                    sys.modules.pop("hercules_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="hercules_coder")
        assert config.host == "hercules_coder"
        assert config.workspace_id == "hercules"  # shared workspace
        assert config.ai_peer == "hercules_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="hercules")
        assert config.host == "hercules"
        assert config.workspace_id == "hercules"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        assert config.host == "hercules_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "hercules_dreamer"
