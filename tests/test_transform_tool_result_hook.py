...
def _run_handle_function_call(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        ...
        monkeypatch.setattr("hercules_cli.plugins.invoke_hook", invoke_hook)
...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("HERCULES_HOME", "/tmp/hercules_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from HERCULES_HOME and verify it rewrites results."""
    import yaml
