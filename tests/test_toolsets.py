...
class TestToolsetConsistency:
    ...
    def test_hercules_platforms_share_core_tools(self):
        """All hercules-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on hercules-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["hercules-cli", "hercules-telegram", "hercules-discord", "hercules-whatsapp", "hercules-slack", "hercules-signal", "hercules-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hercules_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hercules-whatsapp")

    def test_hercules_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hercules-api-server")
