...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.hercules/hercules-agent/utils.py)
...
2. ``hercules update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``hercules_cli/main.py`` (the ``hercules update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``hercules_constants`` after the
