...
Uses a throwaway HERCULES_HOME so it never touches ~/.hercules. No real names.
"""
...
def _fresh_home():
    """Point HERCULES_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hercules_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["HERCULES_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER HERCULES_HOME is set.
...
# a different get_hercules_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"HERCULES_HOME (throwaway): {HOME}")
