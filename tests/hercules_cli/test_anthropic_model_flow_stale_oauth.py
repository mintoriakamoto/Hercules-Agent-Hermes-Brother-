"""Tests for Bug #12905 fix — stale OAuth token detection in hercules model flow.

Bug 3: `hercules model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.hercules/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("hercules_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from hercules_cli.main import _model_flow_anthropic
        cfg = {}
