"""Tests for the stale-dashboard handling run at the end of ``hercules update``.

``hercules update`` detects ``hercules dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``hercules_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``hercules_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from hercules_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"hercules_cli.main.X\")``
    ...
    live = sys.modules.get("hercules_cli.main")
    if live is None:
        live = importlib.import_module("hercules_cli.main")
...
class TestFindStaleDashboardPids:
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "hercules dashboard --port 9119") + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("hercules_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
...
class TestWindowsWmicEncoding:
    ...
    `hercules update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m hercules_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
