"""Tests for hercules_cli.gateway."""
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /hercules-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``HERCULES_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``HERCULES_HOME`` value, which
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "hercules"
    ...
    monkeypatch.delenv("HERCULES_ALLOW_ROOT_GATEWAY", raising=False)
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Hercules gateway as root" in out
    assert "/opt/hercules/docker/entrypoint.sh" in out
...
def test_run_gateway_root_guard_has_escape_hatch(monkeypatch):
    ...
    monkeypatch.setattr(gateway.os, "geteuid", lambda: 0)
    monkeypatch.setattr(gateway, "_is_official_docker_checkout", lambda: True)
    monkeypatch.setenv("HERCULES_ALLOW_ROOT_GATEWAY", "1")
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("HERCULES_S6_SUPERVISED_CHILD", raising=False)
...
def test_run_gateway_refuses_when_service_supervising(monkeypatch, capsys):
    ...
    assert "hercules gateway restart" in out
    assert "--force" in out
...
def test_run_gateway_refuses_existing_process_before_importing_gateway_run(monkeypatch, capsys):
    ...
    assert "hercules gateway run --replace" in out
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("hercules_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("hercules_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_running_under_gateway_supervisor_markers(monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "org.nousresearch.hercules.gateway")
...
def test_run_gateway_windows_foreground_keeps_ctrl_c_enabled(monkeypatch):
    ...
    monkeypatch.delenv("HERCULES_GATEWAY_DETACHED", raising=False)
...
def test_run_gateway_windows_detached_absorbs_console_controls(monkeypatch):
    ...
    monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(gateway.sys, "stdin", _TTY())
    monkeypatch.setenv("HERCULES_GATEWAY_DETACHED", "1")
    monkeypatch.setattr(gateway.signal, "signal", fake_signal)
    monkeypatch.setattr(gateway.asyncio, "run", lambda coro: True)
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    A Telegram-hosted agent may run `hercules gateway restart` via the terminal
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "hercules-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test HERCULES_HOME (a tmp dir), which the temp-home write
...
def test_systemd_install_can_skip_enable_on_startup(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test HERCULES_HOME) stays out of the way.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "hercules-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "hercules-gateway.service"
    system_unit = tmp_path / "system" / "hercules-gateway.service"
    ...
    assert "hercules gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_non_root_never_offers_system(monkeypatch, capsys):
    ...
    # handed a `sudo hercules …` self-elevation recipe.
    captured = {}
    ...
    assert "sudo hercules" not in out
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_hercules_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_hercules_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "hercules-gateway.service"
...
def test_scan_gateway_pids_detects_windows_hercules_exe_case_variants(monkeypatch):
    monkeypatch.setattr(gateway, "is_windows", lambda: True)
    ...
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["wmic.exe", "process", "get", "ProcessId,CommandLine"]:
            return SimpleNamespace(
                returncode=0,
                stdout=(
                    "CommandLine=C:\\Program Files\\Hercules\\Hercules.EXE gateway run --replace\n"
                    "ProcessId=2468\n\n"
                ),
                stderr="",
            )
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "hercules_cli.gateway"
