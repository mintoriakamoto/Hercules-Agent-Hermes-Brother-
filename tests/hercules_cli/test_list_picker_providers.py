...
import pytest
from hercules_cli import model_switch
...
def test_openrouter_models_replaced_with_live_catalog(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("hercules_cli.models.fetch_openrouter_models",
...
def test_openrouter_falls_back_to_base_models_on_fetch_failure(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("hercules_cli.models.fetch_openrouter_models", _raise)
...
def test_include_moa_adds_virtual_provider_with_named_presets(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("hercules_cli.config.load_config", lambda: moa_config)
    monkeypatch.setattr("hercules_cli.models.fetch_openrouter_models",
                        lambda *a, **kw: pytest.fail("should not be called"))
