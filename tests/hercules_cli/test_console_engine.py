...
from hercules_cli.console_engine import HerculesConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/hercules-console-test.zip",
    "import /tmp/hercules-console-test.zip",
...
def test_console_parses_bare_and_hercules_prefixed_commands(_isolate_hercules_home):
    engine = HerculesConsoleEngine()

    bare = engine.execute("config path")
    prefixed = engine.execute("hercules config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    _isolate_hercules_home,
):
    import hercules_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'hercules doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'hercules setup' to configure\x1b[0m")
        print()

    monkeypatch.setattr(status_mod, "show_status", fake_show_status)

    result = HerculesConsoleEngine().execute("status")

    assert result.status == "ok"
    assert "Sessions" in result.output
    assert "Active: 3 session(s)" in result.output
    assert "hercules doctor" not in result.output
    assert "hercules setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'hercules doctor' for detailed diagnostics"))
        print(osc_link("  Run 'hercules setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    ...
    assert "Run `hercules skills list`" not in help_text
    assert "Run `hercules tools list`" not in help_text
...
def test_console_help_for_command_uses_cli_summary():
    help_text = HerculesConsoleEngine().help_text("skills list")

    assert help_text == "skills list\nList installed skills"


def test_console_registry_covers_non_admin_cli_surface():
    registered = set(HerculesConsoleEngine().commands)
...
def test_hosted_console_rejects_local_only_or_dangerous_commands(line):
    result = HerculesConsoleEngine(context="hosted").execute(line)
...
def test_console_rejects_destructive_and_shell_like_commands(line):
    result = HerculesConsoleEngine().execute(line)
...
def test_help_lists_supported_commands_and_not_full_cli():
    result = HerculesConsoleEngine().execute("help")
...
def test_config_set_requires_confirmation_then_writes(_isolate_hercules_home):
    engine = HerculesConsoleEngine()
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_hercules_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_hercules_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_hercules_home):
    ...
    assert code == 0
    assert "Hercules Console" in stdout.getvalue()
    assert "hercules>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_repl_refuses_non_interactive_confirmation(_isolate_hercules_home):
...
def test_main_console_subcommand_smoke(_isolate_hercules_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "hercules_cli.main", "console"],
    ...
    assert "Hercules Console" in result.stdout
