...
the SAME provider universe as ``hercules model`` (the CLI/TUI picker).
...
def test_every_hercules_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `hercules model` universe."""
    ...
    assert not missing, (
        "providers shown in `hercules model` but not configurable in the desktop "
