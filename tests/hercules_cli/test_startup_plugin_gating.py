...
``hercules_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``hercules --help``, ``hercules version``,
``hercules logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``hercules --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from hercules_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["hercules", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["hercules", "-w", "--tui"], None),
        (["hercules", "version"], "version"),
        (["hercules", "--tui", "chat"], "chat"),
        (["hercules", "-w", "logs"], "logs"),
        (["hercules", "chat", "hello world"], "chat"),
        (["hercules", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["hercules", "-m", "gpt5", "chat"], "chat"),
        (["hercules", "--model", "gpt5", "chat", "hi"], "chat"),
        ...
        (["hercules", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``hercules help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"hercules_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
