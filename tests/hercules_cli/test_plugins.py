"""Tests for the Hercules plugin system (hercules_cli.plugins)."""
...
from hercules_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<hercules_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<hercules_home>/plugins/``; we derive
    ``<hercules_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <HERCULES_HOME>/config.yaml.
        # Config is always read from HERCULES_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        hercules_home_str = os.environ.get("HERCULES_HOME")
        if hercules_home_str:
            hercules_home = Path(hercules_home_str)
        else:
            hercules_home = base.parent
        hercules_home.mkdir(parents=True, exist_ok=True)
        cfg_path = hercules_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.hercules/plugins/ are discovered."""
        plugins_dir = tmp_path / "hercules_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("HERCULES_HOME", str(tmp_path / "hercules_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".hercules" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        hercules_home = tmp_path / "hercules_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under hercules_plugins.<name>."""
        plugins_dir = tmp_path / "hercules_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("HERCULES_HOME", str(tmp_path / "hercules_test"))

        # Clean up any prior namespace module
        sys.modules.pop("hercules_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "hercules_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="hercules_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "hercules_cli.plugins.invoke_hook",
...
class TestPreToolCallDirective:
    ...
    def test_approve_directive_returned(self, monkeypatch):
        from hercules_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "hercules_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from hercules_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "hercules_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("hercules_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from hercules_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from hercules_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("hercules_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import hercules_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `hercules plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("hercules_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / "hercules_test"
        plugins_dir = hercules_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "hercules_test" / "plugins"
        # Set HERCULES_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("hercules_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("hercules_cli.plugins.PluginContext.dispatch_tool.__module__", "hercules_cli.plugins"):
...
class TestPluginDebugLogging:
    """HERCULES_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("HERCULES_PLUGINS_DEBUG", raising=False)
        from hercules_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With HERCULES_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("HERCULES_PLUGINS_DEBUG", "1")
        from hercules_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from HERCULES_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """HERCULES_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """HERCULES_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".hercules" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERCULES_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".hercules" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERCULES_HOME", str(prof))
        ctx = self._ctx()
