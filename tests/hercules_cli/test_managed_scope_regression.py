...
def hercules_home(tmp_path, monkeypatch):
    ...
    # /etc/hercules on the dev/CI box can't influence the test.
    monkeypatch.setenv("HERCULES_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(hercules_home, monkeypatch):
    from hercules_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(hercules_home, monkeypatch):
    from hercules_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(hercules_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_hercules_dotenv(hercules_home=str(home))
