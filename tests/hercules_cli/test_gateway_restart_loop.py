...
Covers:
- Defense 1: gateway stop/restart refuse when _HERCULES_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.hercules.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.hercules.gateway.plist",
        "launchctl stop ai.hercules.gateway",
        "systemctl restart hercules-gateway",
        "systemctl stop hercules-gateway.service",
    ...
    @pytest.mark.parametrize("text", [
        "kill hercules gateway process",
        "pkill -f hercules.*gateway",
        "pkill -f gateway.*hercules",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "hercules cron list",
        "hercules update",
        "hercules config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `hercules gateway start` is benign — starting a gateway from inside a
        ...
        "hercules gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway hercules
        # services must not be falsely blocked (the old `.*hercules` matched any
        # hercules token).
        "launchctl unload ai.hercules.update-checker.plist",
        "launchctl restart ai.hercules.daemon",
        "systemctl restart hercules-meta.service",
        "systemctl restart hercules-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hercules_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade hercules then run hercules gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.hercules.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_hercules_gateway_once.sh). The script must live under
        # HERCULES_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("HERCULES_HOME", str(tmp_path / ".hercules"))
        scripts_dir = tmp_path / ".hercules" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify hercules gateway stop/restart refuse when _HERCULES_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_HERCULES_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _HERCULES_GATEWAY=1.

    Issue #37453: systemctl --user restart hercules-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart hercules-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-hercules systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _HERCULES_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart hercules-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart hercules-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run hercules gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("hercules gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfehercules gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under HERCULES_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.hercules.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    hercules_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run hercules gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_HOME", str(tmp_path / ".hercules"))
        (tmp_path / ".hercules").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run hercules gateway restart nightly",
        ))
