...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (HERCULES_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via HERCULES_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in HERCULES_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from hercules_cli.providers import HERCULES_OVERLAYS
        assert "bedrock" in HERCULES_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from hercules_cli.providers import HERCULES_OVERLAYS
        assert HERCULES_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from hercules_cli.providers import HERCULES_OVERLAYS
        assert HERCULES_OVERLAYS["bedrock"].auth_type == "aws_sdk"
