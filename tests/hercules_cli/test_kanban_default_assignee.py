...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh HERCULES_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("HERCULES_HOME", test_home)
    # Force-reimport so the fresh HERCULES_HOME is picked up.
    ...
    # gives each test its own monkeypatched HERCULES_HOME so no cross-test
    # contamination.
...
def test_dry_run_with_default_assignee_reports_without_mutating(isolated_kanban_home):
    ...
    `hercules kanban dispatch --dry-run` see the routing decision before
    committing."""
    kb, _home = isolated_kanban_home
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
...
def test_dispatch_result_has_auto_assigned_default_field():
    ...
    from hercules_cli.kanban_db import DispatchResult
    r = DispatchResult()
