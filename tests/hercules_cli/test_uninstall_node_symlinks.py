"""Tests for hercules_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $HERCULES_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Hercules-managed node dir.
"""
...
def _make_hercules_node(hercules_home: Path) -> Path:
    """Create a fake $HERCULES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = hercules_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_hercules_node(fake_home):
    hercules_home = fake_home / ".hercules"
    node_bin = _make_hercules_node(hercules_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(hercules_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    hercules_home = fake_home / ".hercules"
    _make_hercules_node(hercules_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(hercules_home) == []


def test_removes_dangling_symlink_into_hercules_node(fake_home):
    """A link into the Hercules node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Hercules links that exist; ignores the ones that don't."""
    hercules_home = fake_home / ".hercules"
    node_bin = _make_hercules_node(hercules_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Hercules-managed; node is a real user binary.
