"""Tests for IRC gateway configuration via `hercules setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Hercules install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Hercules install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#hercules")
            monkeypatch.setenv("IRC_NICKNAME", "hercules-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `hercules setup gateway` experience with IRC present."""
