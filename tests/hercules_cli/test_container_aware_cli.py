...
def container_env(tmp_path, monkeypatch):
    """Set up a fake HERCULES_HOME with .container-mode file."""
    hercules_home = tmp_path / ".hercules"
    hercules_home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
    monkeypatch.delenv("HERCULES_DEV", raising=False)

    container_mode = hercules_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=hercules-agent\n"
        "exec_user=hercules\n"
        "hercules_bin=/data/current-package/bin/hercules\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("hercules_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "hercules-agent"
    assert info["exec_user"] == "hercules"
    assert info["hercules_bin"] == "/data/current-package/bin/hercules"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("hercules_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_hercules_dev(container_env, monkeypatch):
    """Returns None when HERCULES_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("HERCULES_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hercules_dev_zero(container_env, monkeypatch):
    """HERCULES_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("HERCULES_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        hercules_home = Path(tmpdir) / ".hercules"
        hercules_home.mkdir()
        (hercules_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("hercules_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_hercules_home": lambda: hercules_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERCULES_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=hercules-custom\n"
        "exec_user=myuser\n"
        "hercules_bin=/opt/hercules/bin/hercules\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "hercules-custom"
    assert info["exec_user"] == "myuser"
    assert info["hercules_bin"] == "/opt/hercules/bin/hercules"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from hercules_cli.main import _exec_in_container
