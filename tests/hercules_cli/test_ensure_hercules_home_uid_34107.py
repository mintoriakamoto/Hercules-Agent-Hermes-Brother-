"""Regression tests for #34107 — Docker UID/GID handling in ensure_hercules_home.

When Hercules runs in Docker with ``HERCULES_UID=1000`` / ``HERCULES_GID=911``,
the entrypoint chowns the top-level ``HERCULES_HOME`` once at startup. But
...
The fix is a ``_chown_to_hercules_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_hercules_uid_gid
# ---------------------------------------------------------------------------


class TestResolveHerculesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("HERCULES_UID", "1000")
        monkeypatch.setenv("HERCULES_GID", "911")
        from hercules_cli.config import _resolve_hercules_uid_gid
        uid, gid = _resolve_hercules_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("HERCULES_UID", raising=False)
        monkeypatch.delenv("HERCULES_GID", raising=False)
        from hercules_cli.config import _resolve_hercules_uid_gid
        uid, gid = _resolve_hercules_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("HERCULES_UID", "not-a-number")
        monkeypatch.setenv("HERCULES_GID", "911")
        from hercules_cli.config import _resolve_hercules_uid_gid
        uid, gid = _resolve_hercules_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("HERCULES_UID", "")
        monkeypatch.setenv("HERCULES_GID", "")
        from hercules_cli.config import _resolve_hercules_uid_gid
        uid, gid = _resolve_hercules_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_hercules_uid
# ---------------------------------------------------------------------------


class TestChownToHerculesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERCULES_UID", "1000")
        monkeypatch.setenv("HERCULES_GID", "911")
        from hercules_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_hercules_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_hercules_uid(d)  # must not raise
