...
hercules_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `hercules config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_hercules_home = get_hercules_home()`` at import time (module
    singleton), so monkeypatching HERCULES_HOME after import doesn't move it.
    ...
    this only matters for tests that swap HERCULES_HOME.)
    """
    import cli

    cli._hercules_home = home
