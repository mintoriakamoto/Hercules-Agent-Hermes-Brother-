"""Tests for hercules_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("hercules_")
        assert name.endswith("_bot")
        assert len(name) == len("hercules_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("hercules_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "hercules_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="Hercules & Friends",
        )
        assert "Hercules+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "hercules_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_abcdefghijklmnop_bot?name=Hercules+Agent",
            "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_abcdefghijklmnop_bot?name=Hercules+Agent",
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="hercules_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/HerculesSetupBot/hercules_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/HerculesSetupBot/hercules_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "hercules_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
    ...
    def test_eventual_success(self):
        ...
        with patch("hercules_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("hercules_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from hercules_cli.setup import _setup_telegram_auto
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        seen = {}
        profile_home = tmp_path / ".hercules" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_hercules_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "hercules_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from hercules_cli.setup import _profile_name_from_hercules_home

        assert (
            _profile_name_from_hercules_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\hercules\profiles\oracle")
            )
            == "oracle"
        )
