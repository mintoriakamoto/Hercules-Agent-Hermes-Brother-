"""Dashboard Hercules Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_hercules_home):
...
def test_console_ws_uses_hosted_context_for_opt_data_policy(console_client, monkeypatch):
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert "hosted Hercules Console" in error["message"]


def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from hercules_cli.console_engine import ConsoleResult, HerculesConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.5)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(HerculesConsoleEngine, "execute", slow_execute)
