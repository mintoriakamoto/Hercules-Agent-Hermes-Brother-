"""Tests for the `hercules memory reset` CLI command.
...
- Profile-scoped reset (uses HERCULES_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake HERCULES_HOME with memory files."""
    hercules_home = tmp_path / ".hercules"
    memories = hercules_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("HERCULES_HOME", str(hercules_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHercules repo is at ~/.hercules/hercules-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `hercules memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        (hercules_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's HERCULES_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_hercules_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
