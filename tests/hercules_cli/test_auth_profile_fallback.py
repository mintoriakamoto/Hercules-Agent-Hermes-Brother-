...
When ``HERCULES_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.hercules/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.hercules            (has its own auth.json fixture)
    * Profile     -> tmp_path/.hercules/profiles/coder   (active, HERCULES_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Hercules is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from hercules_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    hercules_home = tmp_path / "classic"
    hercules_home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(hercules_home))

    _write(hercules_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (HERCULES_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.hercules as the "real home" — which is NOT equal
    # to the HERCULES_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    hercules_home = tmp_path / "classic"
    ...
    _write(hercules_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from hercules_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: HERCULES_HOME is set to a custom path that is NOT under
    # ~/.hercules/profiles/ — get_default_hercules_root() returns HERCULES_HOME
