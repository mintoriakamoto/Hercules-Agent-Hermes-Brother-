...
* ``hercules_cli.providers.determine_api_mode`` correctly mapped
  ``api.anthropic.com`` to ``anthropic_messages``.
* ``hercules_cli.runtime_provider._detect_api_mode_for_url`` did NOT, so
...
class TestExplicitRuntimeForAnthropic:
    ...
    Exercised whenever the user (or a Hercules subcommand) passes an
...
class TestPoolEntryForAnthropic:
    ...
    has added an OAuth credential via ``hercules auth add anthropic
