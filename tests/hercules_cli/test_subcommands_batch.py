...
#
# `hercules login` is a removed command; its handler (`login_command` in
# `hercules_cli/auth.py`) prints a deprecation notice pointing at `hercules auth` /
# `hercules model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `hercules --help`.
