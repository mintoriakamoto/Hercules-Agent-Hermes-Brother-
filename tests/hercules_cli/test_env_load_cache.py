...
The cache exists to keep `hercules tools` → "All Platforms" fast: every
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from hercules_cli import config as config_mod
    from hercules_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_hercules_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "hercules-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("hercules_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
