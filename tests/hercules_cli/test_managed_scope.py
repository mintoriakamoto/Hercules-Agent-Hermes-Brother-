"""Unit tests for hercules_cli.managed_scope (resolver + loaders + key helpers)."""
import textwrap
...
def test_get_managed_dir_absent_override_returns_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERCULES_MANAGED_DIR", str(tmp_path / "nope"))
...
def test_get_managed_dir_empty_override_falls_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERCULES_MANAGED_DIR", "   ")  # whitespace = unset
    # Under pytest the /etc/hercules default is ignored, so this is None; the
...
def test_get_managed_dir_default_ignored_under_pytest(monkeypatch):
    ...
    monkeypatch.delenv("HERCULES_MANAGED_DIR", raising=False)
    assert managed_scope.get_managed_dir() is None
...
def test_managed_dir_env_scrubbed_by_default():
    """conftest must scrub HERCULES_MANAGED_DIR so a dev-shell value can't leak in."""
    import os

    assert "HERCULES_MANAGED_DIR" not in os.environ
