...
hand-rolled parsers — ``hercules_cli.config.load_env``,
``hercules_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from hercules_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("hercules_cli.config.get_env_path", return_value=env_path):
            env = load_env()
    finally:
        invalidate_env_cache()
...
def test_skills_tool_load_env_strips_export_prefix(tmp_path, monkeypatch):
    ...
    # skills_tool.load_env reads get_hercules_home()/.env directly.
    import importlib

    import tools.skills_tool as skills_tool

    importlib.reload(skills_tool)
    with patch.object(skills_tool, "get_hercules_home", return_value=tmp_path):
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import hercules_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from HERCULES_HOME (set above) → tmp_path/.env, so
    # no patching is needed. Re-clear os.environ provider keys that
    # load_hercules_dotenv may have populated at import/reload time, forcing the
