"""Tests for hercules_cli/webhook.py — webhook subscription CLI."""
...
from hercules_cli.webhook import (
    webhook_command,
    _load_subscriptions,
    _save_subscriptions,
    _subscriptions_path,
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("hercules_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "hercules gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "hercules_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "hercules_cli.webhook._is_webhook_enabled",
