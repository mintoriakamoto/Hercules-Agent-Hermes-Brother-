...
The Python launcher (`hercules --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Hercules session
...
from hercules_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hercules", "--tui", "-c", "abc"])
        monkeypatch.delenv("HERCULES_TUI", raising=False)
        monkeypatch.delenv("HERCULES_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_hercules_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hercules"])
        monkeypatch.setenv("HERCULES_TUI", "1")
        monkeypatch.delenv("HERCULES_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("HERCULES_TUI", raising=False)
        monkeypatch.setenv("HERCULES_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `hercules --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
