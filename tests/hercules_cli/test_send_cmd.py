"""Tests for the ``hercules send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``hercules_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from hercules_cli import send_cmd
...
def test_load_hercules_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``hercules send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``hercules config set``) lives
    ...
    hercules_home.mkdir()
    (hercules_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_hercules_home() to re-resolve under the patched env.
...
def test_load_hercules_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    hercules_home = tmp_path / ".hercules"
    hercules_home.mkdir()
    (hercules_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_hercules_env_handles_missing_files(tmp_path, monkeypatch):
