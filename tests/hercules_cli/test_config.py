"""Tests for hercules_cli configuration management."""
...
class TestGetHerculesHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".hercules"

    def test_env_override(self):
        with patch.dict(os.environ, {"HERCULES_HOME": "/custom/path"}):
            home = get_hercules_home()
            assert home == Path("/custom/path")


class TestEnsureHerculesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from hercules_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from hercules_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``hercules logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"HERCULES_HOME": str(tmp_path)}):
            ...
            # stands out at hercules startup before logging is configured)
            captured = capsys.readouterr()
            assert "hercules config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERCULES_HOME": str(tmp_path)}):
            ...
            assert "hercules config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERCULES_HOME": str(tmp_path)}):
            ...
            assert "hercules config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERCULES_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """HERCULES_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `hercules setup`) lets a
        ...
        via config.yaml; HERCULES_MAX_ITERATIONS remains a read-only backward-compat
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `hercules setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``HERCULES_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Hercules
    process on next startup via the dotenv → ``os.environ`` chain in
    ``hercules_cli/env_loader.py``.
    ...
    def _hercules_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "HERCULES_SPOTIFY_CLIENT_ID",
            "HERCULES_QWEN_BASE_URL",
            "HERCULES_MAX_ITERATIONS",
        ],
    )
    def test_hercules_integration_keys_still_writable(self, allowed_key):
        """``HERCULES_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``HERCULES_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``HERCULES_*``) keep working. The denylist
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"HERCULES_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: hercules\n",
            )
