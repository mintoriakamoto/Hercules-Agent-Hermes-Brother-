"""Tests for the per-profile Projects store (hercules_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Hercules Agent", folders=["/tmp/hercules"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "hercules-agent"
    assert proj.name == "Hercules Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/hercules"
    assert [f.path for f in proj.folders] == ["/tmp/hercules"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "hercules-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Hercules Agent")
    pdb.create_project(conn, name="Hercules Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["hercules-agent", "hercules-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' HERCULES_HOME.
...
def test_db_path_under_hercules_home():
    # Resolves under HERCULES_HOME (set by the autouse isolation fixture).
