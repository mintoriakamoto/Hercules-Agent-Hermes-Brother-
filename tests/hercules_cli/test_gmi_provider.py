...
from hercules_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from hercules_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # hercules model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in HERCULES_OVERLAYS
        overlay = HERCULES_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from hercules_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a HerculesAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("HerculesAgent/")

    def test_gmi_profile_declares_hercules_user_agent(self):
        """The GMI plugin sets a HerculesAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("HerculesAgent/"), (
            f"expected GMI profile User-Agent to start with 'HerculesAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "hercules_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["hercules", "chat", "--provider", "gmi"])

        from hercules_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("hercules_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        config = yaml.safe_load((get_hercules_home() / "config.yaml").read_text()) or {}
