...
`HERCULES_REDACT_SECRETS` at module-import time. `hercules_cli/main.py` at
...
in .env), the toggle was silently ignored in both `hercules chat` and
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports hercules_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("HERCULES_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import hercules_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('HERCULES_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["HERCULES_HOME"] = str(hercules_home)
    env.pop("HERCULES_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env HERCULES_REDACT_SECRETS takes precedence over config.yaml."""
    hercules_home = tmp_path / ".hercules"
    hercules_home.mkdir()
    (hercules_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hercules_home / ".env").write_text("HERCULES_REDACT_SECRETS=true\n")
