...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    HERCULES_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_hercules_argv", lambda: ["hercules"])
    ...
    assert pid == 4242
    assert captured["env"]["HERCULES_HOME"] == str(profile)
    assert captured["env"]["HERCULES_KANBAN_TASK"] == "t_spawn_tools"
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited HERCULES_TUI=1 (or a TUI-default
    ...
    flag) and strips HERCULES_TUI from the child env."""
    root = tmp_path / ".hercules"
    ...
    assert "HERCULES_TUI" not in captured["env"]
