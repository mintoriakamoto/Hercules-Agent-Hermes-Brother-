"""Tests for hercules backup and import commands."""
...
def _make_hercules_tree(root: Path) -> None:
    ...
    (root / "hercules_state.db").write_bytes(b"fake-state")
    ...
    (root / "hercules-agent").mkdir(exist_ok=True)
    (root / "hercules-agent" / "run_agent.py").write_text("# big file\n")
    (root / "hercules-agent" / ".git").mkdir()
    (root / "hercules-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_hercules_root needs this
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = hercules_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, hercules_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_hercules_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include hercules-agent/ directory."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        _make_hercules_tree(hercules_home)
    ...
    def test_includes_nested_hercules_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../hercules-agent/ but NOT root hercules-agent/."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        _make_hercules_tree(hercules_home)

        # Add a nested hercules-agent directory inside skills (like the real layout)
        nested = hercules_home / "skills" / "autonomous-ai-agents" / "hercules-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Hercules Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root hercules-agent must be excluded
            root_agent = [n for n in names if n.startswith("hercules-agent/")]
            assert root_agent == [], f"root hercules-agent leaked: {root_agent}"
            # Nested skill hercules-agent must be included
            assert "skills/autonomous-ai-agents/hercules-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/hercules-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/hercules-backup-*.zip."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("hercules-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside HERCULES_HOME."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        _make_hercules_tree(hercules_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(hercules_home / "skills" / "outside-link.txt", outside)
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into hercules home."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_hercules_prefix(self, tmp_path, monkeypatch):
        """Import strips .hercules/ prefix if all entries share it."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (hercules_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_hercules_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a hercules backup."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside hercules home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        assert (hercules_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (hercules_home / "config.yaml").exists()
        assert not (hercules_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        (hercules_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (hercules_home / "gateway.pid").write_text("4242")
        ...
        assert (hercules_home / "gateway.pid").read_text() == "4242"
        assert (hercules_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (hercules_home / "cron.pid").exists()
        assert not (hercules_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (hercules_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (hercules_home / "config.yaml").read_text() == "existing: true\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".hercules"
        src_home.mkdir(parents=True)
        _make_hercules_tree(src_home)

        monkeypatch.setenv("HERCULES_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from hercules_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".hercules"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("HERCULES_HOME", str(dst_home))
        ...
        # hercules-agent should NOT be present
        assert not (dst_home / "hercules-agent").exists()
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without hercules markers fails validation."""
        import io
        from hercules_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hercules(self):
        """Detects .hercules/ prefix wrapping all entries."""
        import io
        from hercules_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".hercules/config.yaml", "test")
            zf.writestr(".hercules/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".hercules/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".hercules/", "")
            zf.writestr(".hercules/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_hercules_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "nonexistent" / ".hercules"
        monkeypatch.setenv("HERCULES_HOME", str(fake_home))
    ...
    def test_empty_hercules_home(self, tmp_path, monkeypatch):
        """Backup handles empty hercules home (no files to back up)."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        # Only excluded dirs, no actual files
        (hercules_home / "__pycache__").mkdir()
        (hercules_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = hercules_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = hercules_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_hercules(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside hercules root."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside hercules home
        out_zip = hercules_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = hercules_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (hercules_home / "config.yaml").exists()
        assert (hercules_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (hercules_home / "profiles" / "coder" / "config.yaml").exists()
        assert (hercules_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "hercules -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    @pytest.fixture
    def hercules_home(self, tmp_path):
        """Create a fake HERCULES_HOME with critical state files."""
    ...
    def test_list_snapshots(self, hercules_home):
        ...
        snaps = list_quick_snapshots(hercules_home=hercules_home)
        assert len(snaps) == 2
    ...
    def test_restore_config(self, hercules_home):
        ...
        assert "anthropic" in (hercules_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, hercules_home=hercules_home)
        assert result is True
        assert "openrouter" in (hercules_home / "config.yaml").read_text()
    ...
    def test_restore_state_db(self, hercules_home):
        ...
        restore_quick_snapshot(snap_id, hercules_home=hercules_home)
    ...
    def test_manual_prune(self, hercules_home):
        ...
        assert deleted == 7
        assert len(list_quick_snapshots(hercules_home=hercules_home)) == 3

    def test_snapshot_includes_pairing_directories(self, hercules_home):
        ...
        (hercules_home / "platforms" / "pairing").mkdir(parents=True)
        (hercules_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (hercules_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (hercules_home / "pairing").mkdir()
        (hercules_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (hercules_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, hercules_home):
        ...
        assert restore_quick_snapshot(snap_id, hercules_home=hercules_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, hercules_home):
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, hercules_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, hercules_home):
        ...
        entry rather than restoring outside HERCULES_HOME."""
        from hercules_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        escape_dst = hercules_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped HERCULES_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_restore_recreates_emptied_projects_db(self, hercules_home):
        ...
        conn = sqlite3.connect(str(hercules_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, hercules_home):
        ...
        board_dir = hercules_home / "kanban" / "boards" / "work"
    ...
    def test_additional_per_profile_dbs_round_trip(self, hercules_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(hercules_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, hercules_home):
        ...
        board = hercules_home / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, hercules_home, monkeypatch):
        ...
        import hercules_cli.backup as bk
        from hercules_cli.backup import create_quick_snapshot
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``hercules update``
    runs before touching anything."""

    @pytest.fixture
    def hercules_home(self, tmp_path):
        root = tmp_path / ".hercules"
        root.mkdir()
        _make_hercules_tree(root)
        return root
    ...
    def test_creates_backup_under_backups_dir(self, hercules_home):
        ...
        assert out.exists()
        assert out.parent == hercules_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, hercules_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # hercules-agent repo excluded
        assert not any(n.startswith("hercules-agent/") for n in names)
    ...
    def test_does_not_recurse_into_prior_backups(self, hercules_home):
        ...
        out2 = create_pre_update_backup(hercules_home=hercules_home)
    ...
    def test_rotation_keeps_only_n(self, hercules_home):
    ...
    def test_rotation_preserves_manual_files(self, hercules_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from hercules_cli.backup import create_pre_update_backup

        (hercules_home / "backups").mkdir(exist_ok=True)
        manual = hercules_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(hercules_home=hercules_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, hercules_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, hercules_home):
        ...
        first = create_pre_update_backup(hercules_home=hercules_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(hercules_home=hercules_home, keep=5)
        _t.sleep(1.05)
    ...
    def test_skips_symlinked_files(self, hercules_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside HERCULES_HOME."""
        from hercules_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def hercules_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".hercules"
        root.mkdir()
        _make_hercules_tree(root)
        # Point HERCULES_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("HERCULES_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for hercules_cli.config + hercules_constants so they pick up HERCULES_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("hercules_cli.config") or mod == "hercules_constants":
    ...
    def test_backup_flag_creates_backup(self, hercules_home, capsys):
        ...
        assert "hercules import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((hercules_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, hercules_home, capsys):
        """With the default (``pre_update_backup: false``), ``hercules update``
    ...
    def test_no_backup_flag_skips(self, hercules_home, capsys):
        ...
        # No backup written
        assert not (hercules_home / "backups").exists() or not list(
            (hercules_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, hercules_home, capsys):
    ...
    def test_config_disabled_is_silent(self, hercules_home, capsys):
...
class TestPreMigrationBackup:
    ...
    ``hercules claw migrate`` runs before mutating ~/.hercules/."""
    ...
    def test_creates_backup_under_backups_dir(self, hercules_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `hercules
        # import` and the update-backup listing both pick them up.
        assert out.parent == hercules_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, hercules_home):
    ...
    def test_rotation_keeps_only_n(self, hercules_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(hercules_home=hercules_home, keep=3)
            if out is not None:
                created.append(out)
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_restores_when_emptied_after_migration(self, tmp_path):
        ...
        snap_id = self._make_snapshot(hercules_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, hercules_home=hercules_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = hercules_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, hercules_home=hercules_home) is None
        assert restore_cron_jobs_if_emptied("", hercules_home=hercules_home) is None
...
class TestMemoryProviderExternalPaths:
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        self._make_min_tree(hercules_home)
        # External provider state living OUTSIDE HERCULES_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        ...
        dst_home.mkdir()
        hercules_home = dst_home / ".hercules"
        hercules_home.mkdir()
        ...
        # External state did NOT leak into HERCULES_HOME.
        assert not (hercules_home / "_external").exists()
