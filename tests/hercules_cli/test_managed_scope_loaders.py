...
hercules_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERCULES_MANAGED_DIR", str(managed))
...
def test_gateway_run_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(gr, "_hercules_home", home, raising=False)
...
def test_gateway_config_loader_honors_managed(homes, monkeypatch):
    ...
    # load_gateway_config resolves home via get_hercules_home() (HERCULES_HOME env).
...
def test_tui_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(ts, "_hercules_home", home, raising=False)
    monkeypatch.setattr(ts, "_cfg_cache", None, raising=False)
    monkeypatch.setattr(ts, "_cfg_mtime", None, raising=False)
    monkeypatch.setattr(ts, "get_hercules_home_override", lambda: None, raising=False)
    cfg = ts._load_cfg()
...
def test_logging_config_honors_managed(homes, monkeypatch):
    ...
    level, _max, _bk = hercules_logging._read_logging_config()
    assert level == "DEBUG"


def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # hercules_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("HERCULES_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import hercules_time

    assert hercules_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (HERCULES_TIMEZONE, HERCULES_REDACT_SECRETS, HERCULES_MAX_ITERATIONS,
    ...
    # writing HERCULES_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
