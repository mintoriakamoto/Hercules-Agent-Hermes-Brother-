...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``hercules_session_rt`` cookie (contract V1: no RT to persist).
...
class TestApi401Envelope:
    ...
    def test_login_url_drops_next_for_deep_api_path(self, gated_app):
        ...
        from ``next=`` entirely; the SPA's own ``hercules.lastLocation``
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    the browser deletes ``hercules_session_at`` the instant the token lapses,
    while ``hercules_session_rt`` lives for 30 days. From that moment the
    ...
    def _build_rt_only_app(self):
        ...
        """
        import time as _t
        from tests.hercules_cli.conftest_dashboard_auth import _sign
...
class TestAutoSsoRedirect:
    ...
    from hercules_cli.dashboard_auth.cookies import SSO_ATTEMPT_COOKIE
    ...
    def test_multiple_providers_render_chooser_not_auto_sso(self, gated_app):
        ...
        from tests.hercules_cli.conftest_dashboard_auth import StubAuthProvider
        from hercules_cli.dashboard_auth import register_provider
...
class TestNextSameOriginValidation:
    ...
    def test_safe_next_validator_accepts_same_origin(self):
        from hercules_cli.dashboard_auth.middleware import _safe_next_target
...
class TestValidatePostLoginTarget:
    ...
    def test_accepts_same_origin_paths(self):
        from hercules_cli.dashboard_auth.routes import _validate_post_login_target
