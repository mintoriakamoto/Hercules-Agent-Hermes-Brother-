...
import pytest
from hercules_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from hercules_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("hercules_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `hercules setup` should reach setup.py, which prints headless guidance."""
        from hercules_cli.main import cmd_setup
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `hercules` should not prompt for input when no provider exists and stdin is headless."""
        from hercules_cli.main import cmd_chat
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`hercules setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["hercules", "setup", "tts"])
