...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("HERCULES_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("HERCULES_DASHBOARD_FILES_ROOT", raising=False)
...
def test_gated_local_mode_still_defaults_to_home(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("HERCULES_HOME", str(home / ".hercules"))
...
def test_hosted_policy_locks_to_opt_data(monkeypatch):
    monkeypatch.delenv("HERCULES_DASHBOARD_FILES_ROOT", raising=False)
    monkeypatch.setenv("HERCULES_HOME", "/opt/data")
    client, prev_auth_required, prev_bound_host = _client_with_app_state()
    try:
        request = SimpleNamespace(
            app=web_server.app,
...
def test_other_credential_store_basenames_blocked(forced_files_client):
    ...
    point the managed root at HERCULES_HOME itself (#57505), which contains
