"""Tests for the top-level `./hercules` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hercules` should use `hercules_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hercules"
    called = []

    fake_main_module = types.ModuleType("hercules_cli.main")

    def fake_main():
        called.append("hercules_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "hercules_cli.main", fake_main_module)
    ...
    assert called == ["hercules_cli.main"]
