...
- Bare `hercules setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `hercules setup --quick` runs the narrower "fill in missing items" flow.
- `hercules setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("hercules_cli.setup.ensure_hercules_home", {}),
        ("hercules_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("hercules_cli.config.is_managed", {"return_value": False}),
        ("hercules_cli.setup.load_config", {"return_value": {}}),
        ("hercules_cli.setup.save_config", {}),
        ("hercules_cli.setup.get_env_value", {"return_value": None}),
        ("hercules_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("hercules_cli.setup._print_setup_summary", {}),
        ("hercules_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("hercules_cli.setup.load_config", {"return_value": {}}),
        ("hercules_cli.setup.save_config", {}),
        ("hercules_cli.auth.get_active_provider", {"return_value": None}),
        ("hercules_cli.setup.get_env_value", {"return_value": None}),
        ("hercules_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `hercules setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="hercules_cli.setup.prompt_choice",
                quick="hercules_cli.setup._run_quick_setup",
                model="hercules_cli.setup.setup_model_provider",
                terminal="hercules_cli.setup.setup_terminal_backend",
                agent="hercules_cli.setup.setup_agent_settings",
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`hercules setup --reconfigure` behaves the same as bare `hercules setup`."""
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "hercules_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["hercules", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["hercules", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["hercules", "setup"])
        try:
            main()
        except SystemExit:
            pass
