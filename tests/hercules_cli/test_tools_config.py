...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "hercules_cli.tools_config.get_nous_subscription_features",
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("hercules_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "hercules_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "hercules_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "hercules_cli.nous_subscription.get_nous_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "hercules_cli.tools_config._configure_toolset",
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "hercules_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "hercules_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
