"""Regression tests for #60328: --yolo must set HERCULES_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing HERCULES_YOLO_MODE at the
    ...
    from hercules_cli.main import main as cli_main
...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """hercules chat --yolo must also set HERCULES_YOLO_MODE before
    _prepare_agent_startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["hercules", "chat", "--yolo"]
    )
    assert result == "1", (
        "HERCULES_YOLO_MODE was not '1' when _prepare_agent_startup was "
