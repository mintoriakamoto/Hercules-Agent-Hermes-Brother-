...
Before the fix, ``hercules whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``hercules gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    hercules = home / ".hercules"
    hercules.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("HERCULES_HOME", str(hercules))
...
def _env_value(hercules_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from hercules_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("hercules_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``hercules whatsapp`` with an existing paired session and
