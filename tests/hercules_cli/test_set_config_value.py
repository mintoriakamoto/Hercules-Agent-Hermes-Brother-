...
from hercules_cli.config import set_config_value, config_command


@pytest.fixture(autouse=True)
def _isolated_hercules_home(tmp_path):
    """Point HERCULES_HOME at a temp dir so tests never touch real config."""
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_hercules_home):
        ...
        assert key not in _read_config(_isolated_hercules_home)
...
class TestCatchAllPatterns:
    ...
    def test_case_insensitive(self, _isolated_hercules_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_hercules_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_hercules_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_hercules_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_hercules_home)

    def test_nested_key(self, _isolated_hercules_home):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_hercules_home)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_hercules_home)

    def test_terminal_image_goes_to_config(self, _isolated_hercules_home):
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_hercules_home):
    ...
    def test_empty_string_routes_to_config(self, _isolated_hercules_home):
    ...
    def test_config_command_accepts_empty_string(self, _isolated_hercules_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_set_echo_masks_secret_value(self, _isolated_hercules_home, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_hercules_home, capsys):
