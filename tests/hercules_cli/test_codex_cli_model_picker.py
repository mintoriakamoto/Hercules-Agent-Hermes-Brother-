...
Covers:
 - Normal path (tokens already in Hercules auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Hercules
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `hercules auth openai-codex`. The old
...
@pytest.fixture()
def hercules_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Hercules auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(hercules_auth_only_env):
    """openai-codex appears when tokens are already in Hercules auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Hercules
    auth store."""
    hercules_home = tmp_path / ".hercules"
    hercules_home.mkdir()
