...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "hercules_cli.models.fetch_nous_recommended_models",
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("hercules_cli.models.fetch_nous_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestCheckNousFreeTierCache:
    ...
    @patch("hercules_cli.nous_account.get_nous_portal_account_info")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from hercules_cli.models import fetch_nous_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("hercules_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
    ...
    def test_get_aux_model_returns_vision_recommendation(self):
        from hercules_cli.models import get_nous_recommended_aux_model
        with patch(
            "hercules_cli.models.fetch_nous_recommended_models",
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("hercules_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        ...
        with (
            patch("hercules_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("hercules_cli.models.check_nous_free_tier", return_value=True),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("hercules_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("hercules_cli.models.check_nous_free_tier", return_value=False),
        ):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("hercules_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("hercules_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "paid-model"
