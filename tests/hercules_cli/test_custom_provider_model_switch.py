"""Tests that `hercules model` always shows the model selection menu for custom
...
def config_home(tmp_path, monkeypatch):
    """Isolated HERCULES_HOME with a minimal config."""
...
class TestCustomProviderModelSwitch:
    ...
    def test_custom_endpoint_switch_prunes_stale_model_config_pool_entry(
        ...
        from hercules_cli.main import _model_flow_custom
    ...
    def test_bare_custom_current_provider_matches_env_base_url_before_first_fallback(
        ...
        """`hercules model` must mark the custom provider matching model.base_url
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        config_path.write_text(
            ...
            "    key_env: HERCULES_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
            "custom_providers: []\n"
        )
        monkeypatch.setenv("HERCULES_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "HERCULES_CRS_HENKEE_KEY"
        ...
        assert "${HERCULES_CRS_HENKEE_KEY}" not in saved_text
    ...
    def test_key_env_providers_dict_preserves_existing_api_key(
        ...
        config_path.write_text(
            ...
            "    name: CRS Henkee\n"
            "    base_url: http://127.0.0.1:3000/api/v1\n"
            "    api_key: ${HERCULES_CRS_HENKEE_KEY}\n"
            "    key_env: HERCULES_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
        ...
        assert entry["api_key"] == "${HERCULES_CRS_HENKEE_KEY}"
        assert "cr_live_secret_xyz" not in saved_text


class TestCustomProviderDiscoverModels:
    """#18726: honor ``discover_models: false`` in the terminal ``hercules model``
