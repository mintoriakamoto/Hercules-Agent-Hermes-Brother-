"""Tests for hercules_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Hercules entry point
(hercules, hercules-agent, hercules-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hercules entry point imports hercules_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported hercules_bootstrap module.
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Hercules CLI
...
class TestEntryPointsImportBootstrap:
    """Every Hercules entry point must import hercules_bootstrap as its
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import hercules_bootstrap' and that
        ...
        half-finished ``hercules update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hercules_bootstrap`` as a top-level module) leaves hercules
        ...
        assert first_import_name == "hercules_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'hercules_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import hercules_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Hercules's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("HERCULES_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["HERCULES_PYTHON_SRC_ROOT"] = env
            elif "HERCULES_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["HERCULES_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hercules")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HERCULES_PYTHON_SRC_ROOT", None)
            else:
                os.environ["HERCULES_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/hercules", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Hercules root.  The guard must relocate Hercules to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/hercules"])
        assert result[0] == "/opt/hercules"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Hercules root.
        assert result.index("/opt/hercules") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/hercules", "/opt/hercules", ""])
        assert result.count("/opt/hercules") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HERCULES_PYTHON_SRC_ROOT"] = "/env/hercules"
            hb.harden_import_path()
            assert sys.path[0] == "/env/hercules"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HERCULES_PYTHON_SRC_ROOT", None)
