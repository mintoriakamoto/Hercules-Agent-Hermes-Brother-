...
closure the PR changed, against a real temp ``HERCULES_HOME``.
"""
...
def _fake_switch_result():
    ...
    from hercules_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "hercules_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_hercules_home() / config.yaml`` — point it here.
    monkeypatch.setattr("hercules_constants.get_hercules_home", lambda: hercules_home)
    monkeypatch.setattr("hercules_cli.config.get_hercules_home", lambda: hercules_home)
    return cfg_path
