...
- The original implementation hardcoded an allow-list of known gateway
  ...
  sources (``acp``, ``webhook``, user-defined ``HERCULES_SESSION_SOURCE``
  ...
  but exist in .hercules/sessions."
...
def test_session_list_surfaces_all_user_facing_sources(monkeypatch):
    ...
    assert "custom-1" in ids, "custom HERCULES_SESSION_SOURCE values were being hidden"
