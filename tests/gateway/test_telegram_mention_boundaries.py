...
substrings like 'foo@hercules_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="hercules_bot")
    return adapter
...
class TestRealMentionsAreDetected:
    ...
    def test_mention_mid_sentence(self):
        adapter = _make_adapter()
        text = "hey @hercules_bot, can you help?"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_at_end_of_message(self):
        adapter = _make_adapter()
        text = "thanks for looking @hercules_bot"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_in_caption(self):
        adapter = _make_adapter()
        caption = "photo for @hercules_bot"
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@hercules_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@hercules_bot.example")
        assert adapter._message_mentions_bot(msg) is False

    def test_hostname_substring(self):
        adapter = _make_adapter()
        msg = _message(text="contact user@hercules_bot.domain.com")
        assert adapter._message_mentions_bot(msg) is False

    def test_superstring_username(self):
        """`@hercules_botx` is a different username; Telegram would emit a mention
        entity for `@hercules_botx`, not `@hercules_bot`."""
        adapter = _make_adapter()
        msg = _message(text="@hercules_botx hello")
        assert adapter._message_mentions_bot(msg) is False

    def test_underscore_suffix_substring(self):
        adapter = _make_adapter()
        msg = _message(text="see @hercules_bot_admin for help")
        assert adapter._message_mentions_bot(msg) is False

    def test_substring_inside_url_without_entity(self):
        """@handle inside a URL produces a URL entity, not a MENTION entity."""
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@hercules_bot for details")
        assert adapter._message_mentions_bot(msg) is False

    def test_substring_inside_code_block_without_entity(self):
        """Telegram doesn't emit mention entities inside code/pre entities."""
        adapter = _make_adapter()
        msg = _message(text="use the string `@hercules_bot` in config")
        assert adapter._message_mentions_bot(msg) is False
    ...
    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@hercules_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestEntityEdgeCases:
    ...
    def test_malformed_entity_with_negative_offset(self):
        adapter = _make_adapter()
        msg = _message(text="@hercules_bot hi",
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @HERCULES_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@HERCULES_BOT")])
        assert adapter._message_mentions_bot(msg) is True
