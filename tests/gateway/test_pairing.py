...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_hercules_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <HERCULES_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``hercules pairing`` CLI continues to work without a profile context."""
        from hercules_constants import get_hercules_home
        monkeypatch.setattr("hercules_constants.get_hercules_home", lambda: tmp_path)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <HERCULES_HOME>/profiles/yangyang/pairing/."""
        from hercules_constants import get_hercules_home
        monkeypatch.setattr("hercules_constants.get_hercules_home", lambda: tmp_path)
