...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.hercules."""
    monkeypatch.setattr(
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@hercules`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@hercules"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@hercules")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /hercules subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /hercules btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_hercules_prefix_still_works(self, adapter):
        """Backward compat: /hercules btw foo must still route to /btw foo.

        Old workspace manifests only declared /hercules as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_hercules_freeform_question(self, adapter):
        """/hercules <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/hercules",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_hercules_slash_stashes_context(self, adapter):
        """Legacy /hercules <subcommand> also stashes context."""
        command = {
            "command": "/hercules",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_hercules_question_does_not_stash_context(self, adapter):
        """Free-form /hercules <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/hercules",
