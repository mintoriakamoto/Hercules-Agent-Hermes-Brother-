...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="nas-token"):
    ...
    monkeypatch.setattr("hercules_cli.auth.resolve_nous_access_token", lambda: token)
...
def test_provisions_on_nas_host_that_is_NOT_is_managed(monkeypatch):
    """Regression: a NAS-hosted Fly agent sets neither HERCULES_MANAGED nor a
    ...
    monkeypatch.setattr("hercules_cli.config.is_managed", lambda: False)
    _arm(monkeypatch)
    captured: dict = {}
...
def test_no_nas_token_is_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr("hercules_cli.auth.resolve_nous_access_token", _boom)
