...
def _mention_entity(text, mention="@hercules_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@hercules_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@hercules_bot",
            entities=[_bot_command_entity("/new@hercules_bot", "/new@hercules_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@hercules_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @hercules_bot", entities=[_mention_entity("hi @hercules_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@hercules_bot",
            entities=[_bot_command_entity("/status@hercules_bot", "/status@hercules_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="hercules_bot")

    assert adapter._should_process_message(_group_message("email ops@hercules_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@hercules_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @hercules_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @hercules_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @hercules_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @hercules_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @hercules_bot")])
    ) is False
...
def test_config_bridges_telegram_group_settings(monkeypatch, tmp_path):
    ...
    # from a developer's real ~/.hercules/.env, defeating the env-over-YAML bridge
...
def test_config_bridges_telegram_user_allowlists(monkeypatch, tmp_path):
    ...
    # ~/.hercules/.env, which would defeat the env-over-YAML bridge here.
    tg_cfg = config.platforms.get(Platform.TELEGRAM)
    assert tg_cfg is not None
    assert tg_cfg.extra.get("group_allowed_chats") == ["-100"]
