...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.hercules/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.hercules/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("HERCULES_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("HERCULES_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("HERCULES_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HERCULES_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("HERCULES_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("HERCULES_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HERCULES_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("HERCULES_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("HERCULES_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.hercules/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_hercules_credentials(self, tmp_path, monkeypatch):
        """~/.hercules/.env and ~/.hercules/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        hercules_dir = fake_home / ".hercules"
        hercules_dir.mkdir(parents=True)
        env_file = hercules_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._HERCULES_HOME",
            hercules_dir,
        )
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.hercules/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        hercules_dir = fake_home / ".hercules"
        (hercules_dir / "mcp-tokens").mkdir(parents=True)
        secret = hercules_dir / rel
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._HERCULES_ROOT",
            hercules_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(secret)) is None

    def test_denylist_blocks_hercules_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        hercules_dir = fake_home / ".hercules"
        hercules_dir.mkdir(parents=True)
        config_file = hercules_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_hercules_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Hercules root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".hercules" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        hercules_root = fake_home / ".hercules"
        config_file = hercules_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the HERCULES_HOME root (google_token.json)
        ...
        hercules_dir = fake_home / ".hercules"
        hercules_dir.mkdir(parents=True)
        token = hercules_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._HERCULES_HOME", hercules_dir)
        monkeypatch.setattr("gateway.platforms.base._HERCULES_ROOT", hercules_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.hercules/pairing/ (platform pairing tokens) are
    ...
    def test_hercules_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        hercules_dir = fake_home / ".hercules"
        cache_dir = hercules_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_hercules_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.hercules
        ...
        hercules_dir = fake_home / ".hercules"
        hercules_dir.mkdir(parents=True)
        artifact = hercules_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting HERCULES_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``HERCULES_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("HERCULES_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_hercules_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.hercules/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose HERCULES_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.hercules) emits
        ...
        hercules_root = denied_root / ".hercules"
        prof_cache = hercules_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._HERCULES_ROOT", hercules_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        hercules_root = denied_root / ".hercules"
        prof_dir = hercules_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"HERCULES_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERCULES_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"HERCULES_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "HERCULES_HUMAN_DELAY_MODE": "natural",
            "HERCULES_HUMAN_DELAY_MIN_MS": "oops",
            "HERCULES_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "HERCULES_HUMAN_DELAY_MODE": "custom",
            "HERCULES_HUMAN_DELAY_MIN_MS": "100",
            "HERCULES_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"HERCULES_MEDIA_DELIVERY_STRICT": "1",
                                     "HERCULES_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Hercules
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../hercules/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.hercules/cache/media/sensitive_host_path_abc123.bin"
