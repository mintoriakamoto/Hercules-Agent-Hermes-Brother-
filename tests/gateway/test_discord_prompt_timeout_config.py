...
def _patch_config(monkeypatch, cfg):
    """Stub ``hercules_cli.config.read_raw_config`` to return ``cfg``."""
    import hercules_cli.config
    monkeypatch.setattr(hercules_cli.config, "read_raw_config", lambda: cfg)
...
def test_config_read_exception_falls_back_to_default(monkeypatch):
    ...
    monkeypatch.setattr(hercules_cli.config, "read_raw_config", _boom)
    assert _read_discord_prompt_timeout() == _DISCORD_PROMPT_TIMEOUT_DEFAULT
