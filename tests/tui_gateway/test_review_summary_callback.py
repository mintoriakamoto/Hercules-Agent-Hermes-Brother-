    ...
    with patch.dict(
        "sys.modules",
        {
            "hercules_constants": MagicMock(
                get_hercules_home=MagicMock(return_value="/tmp/hercules_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'hercules-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'hercules-release' patched"
    }
