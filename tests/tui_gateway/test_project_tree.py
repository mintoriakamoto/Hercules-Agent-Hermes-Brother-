...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="hercules/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is HERCULES_HOME (config/state) must not spawn a
    # phantom project; it falls through to flat Recents (unscoped). A real repo
    # alongside it still groups normally.
    resolve = _resolver(
        {
            "/home/me/.hercules": ("/home/me/.hercules", "/home/me/.hercules"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.hercules", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.hercules"
...
def test_junk_root_is_dropped_from_the_discovered_tier():
    discovered = [{"root": "/home/me/.hercules", "label": ".hercules", "sessions": 0, "last_active": 9}]

    tree = pt.build_tree([], [], discovered, resolve=None, is_junk_root=lambda r: r == "/home/me/.hercules")
...
def test_non_git_cwd_can_group_inside_a_junk_repo_subtree():
    ...
    workspace = _session("/home/test/.hercules/workspaces/notes")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_root=lambda path: path.startswith("/home/test/.hercules"),
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.hercules"},
    )

    assert [p["id"] for p in tree["projects"]] == ["/home/test/.hercules/workspaces/notes"]
    assert tree["scoped_session_ids"] == [workspace["id"]]


def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.hercules")
