...
The desktop app's chat panel uses ``hercules serve`` (the ``tui_gateway``
...
``tui_gateway`` session-creation time:
  ...
  HERCULES_DESKTOP=1, HERCULES_DESKTOP_TERMINAL unset  -> platform="desktop"
  HERCULES_DESKTOP=1, HERCULES_DESKTOP_TERMINAL=1     -> platform="tui"  (embedded pane)
...
class TestResolveSessionPlatform:
    ...
    def test_truthy_variants_recognized(self, clean_env, val):
        clean_env.setenv("HERCULES_DESKTOP", val)
        _srv = _reload_resolver()
        assert _srv._resolve_session_platform() == "desktop"
    ...
    def test_embedded_terminal_overrides_desktop_when_both_set(self, clean_env):
        ...
        clean_env.setenv("HERCULES_DESKTOP", "1")
        clean_env.setenv("HERCULES_DESKTOP_TERMINAL", "true")
        _srv = _reload_resolver()
        assert _srv._resolve_session_platform() == "tui"
