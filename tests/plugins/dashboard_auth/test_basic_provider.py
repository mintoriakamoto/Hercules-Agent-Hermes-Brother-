...
def _clear_basic_env(monkeypatch):
    for var in (
        ...
        "HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH",
...
class TestRegister:
    ...
    def test_skips_when_username_but_no_password(self, basic, monkeypatch):
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_USERNAME", "admin")
        monkeypatch.setattr(basic, "_load_config_basic_auth_section", lambda: {})
        ctx = MagicMock()
        basic.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
    ...
    def test_registers_with_env_plaintext_password(self, basic, monkeypatch):
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_USERNAME", "admin")
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD", "hunter2")
        monkeypatch.setattr(basic, "_load_config_basic_auth_section", lambda: {})
        ctx = MagicMock()
        basic.register(ctx)
    ...
    def test_env_password_overrides_config(self, basic, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD", "env-pw")
        ctx = MagicMock()
        basic.register(ctx)
        provider = ctx.register_dashboard_auth_provider.call_args.args[0]
        # env password works ...
    ...
    def test_explicit_secret_makes_sessions_portable(self, basic, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_USERNAME", "admin")
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD", "hunter2")
        monkeypatch.setenv("HERCULES_DASHBOARD_BASIC_AUTH_SECRET", shared)
