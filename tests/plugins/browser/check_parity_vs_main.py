...
Run from the PR worktree:
    ...
    cd ~/.hercules/hercules-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.hercules/hercules-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.hercules/hercules-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a hercules-agent checkout"
)
...
# Isolated HERCULES_HOME for the config write.
