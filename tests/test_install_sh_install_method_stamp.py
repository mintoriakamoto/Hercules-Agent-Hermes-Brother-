...
($INSTALL_DIR), not into the shared $HERCULES_HOME.

Background (shared-$HERCULES_HOME bug)
------------------------------------
$HERCULES_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.hercules:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$HERCULES_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``hercules update``).
...
``~/.hercules/hercules-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$HERCULES_HOME/\.install_method"', text), (
        "install.sh must not stamp $HERCULES_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `hercules update`"
    )
