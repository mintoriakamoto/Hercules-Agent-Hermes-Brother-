...
from cli import HerculesCLI, _build_compact_banner, _rich_text_from_ansi
from hercules_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("hercules_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hercules_branding(self):
        set_active_skin("default")
        ...
        assert "NOUS HERCULES" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_hercules(self):
        ...
        assert "NOUS HERCULES" not in banner
