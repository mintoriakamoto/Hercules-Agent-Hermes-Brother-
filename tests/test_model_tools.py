...
class TestHandleFunctionCall:
    ...
    def test_tool_hooks_receive_session_and_tool_call_ids(self):
        with (
            ...
            patch("hercules_cli.plugins.has_hook", return_value=True),
            patch("hercules_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("hercules_cli.plugins.has_hook", return_value=False),
            patch("hercules_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.plugins.invoke_middleware", fake_invoke_middleware)
        ...
        monkeypatch.setattr("hercules_cli.plugins.has_hook", lambda name: True)
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("hercules_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_run_agent_pattern_fires_pre_tool_call_exactly_once(self, monkeypatch):
        ...
        """
        from hercules_cli.plugins import get_pre_tool_call_block_message
...
class TestDisabledToolsetsPlatformBundle:
    """Regression test for #33924: disabling a platform bundle (hercules-*)
    must not remove core tools from other enabled toolsets."""

    def test_disabling_platform_bundle_preserves_core_tools(self):
        """Disabling hercules-yuanbao should not strip core tools from hercules-telegram."""
        from model_tools import get_tool_definitions

        tools_telegram = get_tool_definitions(
            enabled_toolsets=["hercules-telegram"],
            quiet_mode=True,
        )
        tools_telegram_no_yuanbao = get_tool_definitions(
            enabled_toolsets=["hercules-telegram"],
            disabled_toolsets=["hercules-yuanbao"],
            quiet_mode=True,
        )
        ...
        assert names_telegram == names_no_yuanbao, (
            f"Tools lost after disabling hercules-yuanbao: "
            f"{names_telegram - names_no_yuanbao}"
        )

    def test_disabling_platform_bundle_removes_own_tools(self):
        """Disabling hercules-discord should remove discord-specific tools."""
        from model_tools import get_tool_definitions

        tools = get_tool_definitions(
            enabled_toolsets=["hercules-discord"],
            disabled_toolsets=["hercules-discord"],
            quiet_mode=True,
        )
    ...
    def test_disabling_non_platform_toolset_still_works(self):
        """Disabling a regular (non-hercules-) toolset still subtracts all tools."""
        from model_tools import get_tool_definitions
    ...
    def test_disabling_bundle_removes_platform_tools_but_keeps_core(self):
        """Disabling hercules-discord (when enabled) removes discord/discord_admin
        from the resolved delta but keeps core tools — via bundle_non_core_tools."""
        from toolsets import bundle_non_core_tools, _HERCULES_CORE_TOOLS

        delta = bundle_non_core_tools("hercules-yuanbao")
        # The delta is the bundle's platform-specific tools, NOT core.
        assert "yb_send_dm" in delta
        assert not (delta & set(_HERCULES_CORE_TOOLS)), "core tools must not be in the removal delta"
    ...
    def test_bundle_non_core_tools_unknown_falls_back(self):
        ...
        assert bundle_non_core_tools("hercules-does-not-exist") == set()
...
class TestDisabledToolsetsPostureToolset:
    ...
    not own -- same non-core-delta subtraction as hercules-* bundles (#33924) --
