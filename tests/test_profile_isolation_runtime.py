...
``_HERCULES_HOME_OVERRIDE`` ContextVar, not the process environment.  State that
...
with real temp HERCULES_HOME directories (no mocks) and assert the *active*
...
def two_profiles(tmp_path):
    """Two distinct profile HERCULES_HOME dirs with the dir skeleton created."""
...
class TestRichSentStorePathResolution:
    ...
    def test_store_path_follows_override(self, two_profiles, monkeypatch):
        prof_a, prof_b = two_profiles
        # Ensure no ambient HERCULES_HOME env masks the test.
...
class TestThreadContextPropagation:
    ...
    def test_run_async_worker_preserves_override(self, two_profiles):
        ...
        leaks, every async tool that resolves get_hercules_home() leaks.
        """
        import asyncio
