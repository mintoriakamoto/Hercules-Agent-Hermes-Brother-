"""Tests for hercules_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
from hercules_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hercules_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("hercules_state.sqlite3.connect", real_connect)
    ...
    def test_base_fts_rebuilds_after_trigger_repair_without_trigram(
        ...
        monkeypatch.setattr("hercules_state.sqlite3.connect", connect_without_trigram)
        restored = SessionDB(db_path=db_path)
        try:
            assert restored._fts_enabled is True
...
class TestFTS5Search:
    ...
    def test_sanitize_fts5_query_runtime_is_bounded(self):
        ...
        from hercules_state import MAX_FTS5_QUERY_CHARS, SessionDB
...
class TestSchemaInit:
    ...
    def test_schema_version(self, db):
        from hercules_state import SCHEMA_VERSION
    ...
    def test_topic_mode_schema_is_not_auto_migrated_on_open(self, tmp_path):
        ...
        The gateway must remain rollback-safe: simply upgrading Hercules and starting
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hercules_state.sqlite3.connect", connect_with_trace)
        migrated_db = SessionDB(db_path=db_path)
        try:
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from hercules_state import SCHEMA_SQL
...
class TestConcurrentWriteSafety:
    ...
    def test_sqlite_timeout_is_at_least_30s(self, db):
        ...
        import inspect
        from hercules_state import SessionDB as _SessionDB
...
class TestApplyWalProbe:
    ...
    def test_macos_checkpoint_fullsync_barrier_applied(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hercules_state.sys, "platform", "darwin")
    ...
    def test_checkpoint_fullsync_barrier_skipped_off_darwin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hercules_state.sys, "platform", "linux")
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(hercules_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(hercules_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(hercules_state.time, "time", lambda: 1016.0)
