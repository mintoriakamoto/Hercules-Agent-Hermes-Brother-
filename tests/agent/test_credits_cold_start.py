...
HERCULES_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_dev_fixtures_drive_cold_start():
    """Every HERCULES_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["HERCULES_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["HERCULES_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("HERCULES_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("HERCULES_DEV_CREDITS", None)
...
def _seed(agent, fixture):
    ...
    os.environ["HERCULES_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["HERCULES_DEV_CREDITS_FIXTURE"] = fixture
    try:
