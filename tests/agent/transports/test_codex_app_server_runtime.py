...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Hercules passes a narrow app-server config override
        ...
        monkeypatch.setenv(
            "HERCULES_KANBAN_DB",
            "/users/alice/.hercules/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.hercules/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through hercules_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Hercules secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "HERCULES_DASHBOARD_SESSION_TOKEN": "dash-secret",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through hercules_subprocess_env must not
