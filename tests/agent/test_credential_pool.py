...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "hercules_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    assert borrowed["source"] == "systemd://hercules/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "hercules" / "auth.json").read_text())["credential_pool"]["nous"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.hercules/.env when
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.hercules/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (hercules_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_nous_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("HERCULES_HOME", str(tmp_path / "hercules"))
    monkeypatch.setenv("HERCULES_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `hercules setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Hercules PKCE
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `hercules_pkce` into auth.json) and later switches to the API key at
    `hercules setup` would still have those OAuth entries dormant on disk.
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `hercules setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `hercules auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `hercules model` to reauth, but the pool
    ...
    # Simulate `hercules model` / `hercules auth` refreshing the tokens.
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    final = json.loads((tmp_path / "hercules" / "auth.json").read_text())
