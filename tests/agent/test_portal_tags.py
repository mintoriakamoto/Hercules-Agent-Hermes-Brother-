...
def test_hercules_client_tag_includes_current_version():
    """The client tag must reflect hercules_cli.__version__ verbatim."""
    from hercules_cli import __version__
    from agent.portal_tags import hercules_client_tag

    assert hercules_client_tag() == f"client=hercules-client-v{__version__}"


def test_hercules_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import hercules_client_tag

    tag = hercules_client_tag()
    assert tag.startswith("client=hercules-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import hercules_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=hercules-agent" in tags
    assert hercules_client_tag() in tags
    assert len(tags) == 2
