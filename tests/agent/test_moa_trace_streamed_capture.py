...
path via ``hercules chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp HERCULES_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point HERCULES_HOME at a temp dir and turn moa.save_traces on."""
    hercules_home = tmp_path / ".hercules"
    hercules_home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(hercules_home))

    # save_moa_turn reads config via hercules_cli.config.load_config; stub it to
    ...
    return hercules_home / "moa-traces"
