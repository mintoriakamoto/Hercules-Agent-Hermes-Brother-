...
engine plugins (e.g. hercules-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``hercules plugins`` picker."""
    ...
    monkeypatch.setattr(
        "hercules_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "hercules_cli.plugins.get_plugin_context_engine",
