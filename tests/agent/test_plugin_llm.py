...
""",
        ...
        from hercules_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        from hercules_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from hercules_cli.plugins import PluginContext, PluginManifest, PluginManager
