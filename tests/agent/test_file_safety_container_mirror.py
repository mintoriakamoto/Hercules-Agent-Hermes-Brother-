...
``…/sandboxes/<backend>/<task>/home/.hercules/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.hercules/…``.
...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.hercules/… must not be flagged."""
        from agent.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.hercules/profiles/group1/SOUL.md") is None
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.hercules/profiles/group1/SOUL.md",
            mirror_prefix="/root/.hercules",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.hercules")
    ...
    def test_catches_authoritative_profile_files(self, inner):
        ...
        result = classify_container_mirror_target(
            f"/root/.hercules/{inner}",
            mirror_prefix="/root/.hercules",
        )
        assert result is not None
        assert result["inner_path"] == inner

    def test_non_hercules_path_not_flagged(self):
        """/root/workspace/… is not .hercules state and must not be blocked."""
        from agent.file_safety import classify_container_mirror_target
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.hercules/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.hercules") is not None
