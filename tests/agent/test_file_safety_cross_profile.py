"""Tests for the cross-Hercules-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Hercules profile's
...
Reference: May 2026 incident — a hercules-security profile session
accidentally edited skills under both ~/.hercules/profiles/hercules-security/skills/
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Hercules root with two profiles, monkeypatch the
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hercules, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_hercules_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hercules, monkeypatch):
        ...
        assert result["active_profile"] == "hercules-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_hercules, monkeypatch):
        ...
        assert result["target_profile"] == "hercules-security"
...
class TestGetCrossProfileWarning:
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_hercules, monkeypatch):
        ...
        assert "hercules-security" in warn
