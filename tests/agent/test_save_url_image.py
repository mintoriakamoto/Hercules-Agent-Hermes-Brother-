...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate HERCULES_HOME under tmp_path."""
    monkeypatch.setenv("HERCULES_HOME", str(tmp_path / ".hercules"))
    (tmp_path / ".hercules").mkdir()

    # Force the constants/image cache helpers to re-read HERCULES_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("hercules_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hercules_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under HERCULES_HOME — gateway cleanup
