...
class TestResolveVisionMainFirst:
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "hercules_cli.copilot_auth.copilot_request_headers",
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("openrouter", None, None, None, None),  # explicit override
        ), patch(
            "agent.auxiliary_client._resolve_strict_vision_backend"
        ) as mock_strict:
            mock_strict.return_value = (MagicMock(), "openrouter-default-model")

            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        # Explicit "openrouter" override → uses strict backend, NOT main model path
        assert provider == "openrouter"
        mock_strict.assert_called_once_with("openrouter", None)
