...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.config.load_config", default_load_config)
    ...
    def test_configured_context_file_max_chars_controls_truncation(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.config.load_config", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("hercules_cli.config.load_config", lambda: {})
...
class TestBuildNousSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "hercules_cli.nous_subscription.get_nous_subscription_features",
...
class TestBuildContextFilesPrompt:
    ...
    def test_loads_soul_md_from_hercules_home_only(self, tmp_path, monkeypatch):
    ...
    # --- .hercules.md / HERCULES.md discovery ---

    def test_loads_hercules_md(self, tmp_path):
        (tmp_path / ".hercules.md").write_text("Use pytest for testing.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "pytest for testing" in result
        assert "Project Context" in result

    def test_loads_hercules_md_uppercase(self, tmp_path):
        (tmp_path / "HERCULES.md").write_text("Always use type hints.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "type hints" in result

    def test_hercules_md_lowercase_takes_priority(self, tmp_path):
        (tmp_path / ".hercules.md").write_text("From dotfile.")
        (tmp_path / "HERCULES.md").write_text("From uppercase.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "From dotfile" in result
        assert "From uppercase" not in result

    def test_hercules_md_parent_dir_discovery(self, tmp_path):
        """Walks parent dirs up to git root."""
        # Simulate a git repo root
        (tmp_path / ".git").mkdir()
        (tmp_path / ".hercules.md").write_text("Root project rules.")
        sub = tmp_path / "src" / "components"
        sub.mkdir(parents=True)
        result = build_context_files_prompt(cwd=str(sub))
        assert "Root project rules" in result

    def test_hercules_md_stops_at_git_root(self, tmp_path):
        """Should NOT walk past the git root."""
        # Parent has .hercules.md but child is the git root
        (tmp_path / ".hercules.md").write_text("Parent rules.")
        child = tmp_path / "repo"
        child.mkdir()
        (child / ".git").mkdir()
    ...
    def test_hercules_md_strips_yaml_frontmatter(self, tmp_path):
        content = "---\nmodel: claude-sonnet-4-20250514\ntools:\n  disabled: [tts]\n---\n\n# My Project\n\nUse Ruff for linting."
        (tmp_path / ".hercules.md").write_text(content)
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Ruff for linting" in result
    ...
    def test_hercules_md_beats_agents_md(self, tmp_path):
        """When both exist, .hercules.md wins and AGENTS.md is not loaded."""
        (tmp_path / "AGENTS.md").write_text("Agent guidelines here.")
        (tmp_path / ".hercules.md").write_text("Hercules project rules.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Hercules project rules" in result
    ...
    def test_hercules_md_beats_all_others(self, tmp_path):
        """When all four types exist, only .hercules.md is loaded."""
        (tmp_path / ".hercules.md").write_text("Hercules wins.")
        (tmp_path / "AGENTS.md").write_text("Agents lose.")
        (tmp_path / "CLAUDE.md").write_text("Claude loses.")
        (tmp_path / ".cursorrules").write_text("Cursor loses.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Hercules wins" in result
...
# =========================================================================
# .hercules.md helper functions
# =========================================================================


class TestFindHerculesMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".hercules.md").write_text("rules")
        assert _find_hercules_md(tmp_path) == tmp_path / ".hercules.md"
    ...
    def test_prefers_lowercase(self, tmp_path):
        (tmp_path / ".hercules.md").write_text("lower")
        (tmp_path / "HERCULES.md").write_text("upper")
        assert _find_hercules_md(tmp_path) == tmp_path / ".hercules.md"

    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".hercules.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_hercules_md(sub) == tmp_path / ".hercules.md"

    def test_returns_none_when_absent(self, tmp_path):
        assert _find_hercules_md(tmp_path) is None

    def test_stops_at_git_root(self, tmp_path):
        """Does not walk past the git root."""
        (tmp_path / ".hercules.md").write_text("outside")
        repo = tmp_path / "repo"
        repo.mkdir()
        (repo / ".git").mkdir()
        assert _find_hercules_md(repo) is None
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .hercules.md planted in /tmp, /home, or /
        ...
        parent.mkdir()
        (parent / ".hercules.md").write_text("planted by another user")
        cwd = parent / "work"
        cwd.mkdir()
        # No git root anywhere up the tree.
        with patch("agent.prompt_builder._find_git_root", return_value=None):
            assert _find_hercules_md(cwd) is None

    def test_no_git_root_finds_in_cwd(self, tmp_path):
        """Outside a git repo, a .hercules.md in cwd itself is still found."""
        from unittest.mock import patch

        (tmp_path / ".hercules.md").write_text("local rules")
        with patch("agent.prompt_builder._find_git_root", return_value=None):
            assert _find_hercules_md(tmp_path) == tmp_path / ".hercules.md"
    ...
    def test_walks_parents_inside_git_repo(self, tmp_path):
        ...
        (tmp_path / ".hercules.md").write_text("repo root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_empty_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.delenv("HERCULES_ENVIRONMENT_HINT", raising=False)
        monkeypatch.setattr("hercules_cli.config.load_config", lambda: {"agent": {}})
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
...
class TestBuildSkillsSystemPromptConditional:
    ...
    def test_null_hercules_under_metadata_does_not_crash(self, monkeypatch, tmp_path):
        """Regression: metadata.hercules present but null should not crash."""
        monkeypatch.setenv("HERCULES_HOME", str(tmp_path))
        skill_dir = tmp_path / "skills" / "general" / "nested-null"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            "---\nname: nested-null\ndescription: Null hercules key\nmetadata:\n  hercules:\n---\n"
        )
        result = build_skills_system_prompt(
            available_tools=set(),
            available_toolsets=set(),
