...
from hercules_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(hercules_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <hercules_home>/plugins/<name> and opt it in."""
    plugin_dir = hercules_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERCULES_HOME", str(tmp_path / "hercules_empty"))
