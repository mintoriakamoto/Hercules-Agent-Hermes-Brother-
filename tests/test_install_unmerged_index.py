...
The ``hercules update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m hercules_cli.main gateway run`` — image name
    ``pythonw``, not ``hercules.exe`` — so the ``taskkill /IM hercules.exe`` guard
    ...
    # The hercules.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM hercules.exe' in text
