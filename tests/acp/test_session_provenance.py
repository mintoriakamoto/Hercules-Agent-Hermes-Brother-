...
def test_root_session_no_compression(db):
    ...
    assert prov["currentHerculesSessionId"] == "root1"
    assert prov["rootHerculesSessionId"] == "root1"
    assert prov["parentHerculesSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_hercules_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentHerculesSessionId"] == "old"
    assert prov["rootHerculesSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousHerculesSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootHerculesSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootHerculesSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_hercules_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousHerculesSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"hercules"}
    assert "sessionProvenance" in meta["hercules"]
    assert meta["hercules"]["sessionProvenance"]["currentHerculesSessionId"] == "root1"
