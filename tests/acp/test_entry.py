...
def test_main_version_prints_without_starting_server(monkeypatch, capsys):
    ...
    assert output
    assert "Starting hercules-agent ACP adapter" not in output
...
def test_main_check_prints_ok_without_starting_server(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == "Hercules ACP check OK"


def test_main_setup_runs_model_configuration(monkeypatch):
    calls = {}

    def fake_hercules_main():
        calls["argv"] = sys.argv[:]

    monkeypatch.setattr("hercules_cli.main.main", fake_hercules_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("hercules_cli.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`hercules-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("hercules_cli.dep_ensure.ensure_dependency", fake_ensure)
