...
- 96vc: ACP didn't set HERCULES_EXEC_ASK, so `check_all_command_guards`
...
1. Setting HERCULES_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds HERCULES_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set HERCULES_INTERACTIVE so
    ...
    (HERCULES_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When HERCULES_INTERACTIVE is set and an approval callback is
        ...
        # Without HERCULES_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without HERCULES_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With HERCULES_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with HERCULES_INTERACTIVE the approval path should consult the "
    ...
    def test_interactive_context_var_routes_to_callback_without_env(
        ...
        tok = set_hercules_interactive_context(True)
        try:
            result = check_all_command_guards(
            ...
            reset_hercules_interactive_context(tok)

        assert called_with, (
            "set_hercules_interactive_context(True) should route dangerous "
            "commands through the callback without HERCULES_INTERACTIVE in env"
        )
        assert result["approved"] is True
