...
Regression: the old implementation wrote ``hercules_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``hercules sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.hercules/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_hercules_home(hercules_home, tmp_path, monkeypatch, capsys):
    ...
    cli.HerculesCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("hercules_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.hercules/sessions/saved/
    saved_dir = hercules_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("hercules_conversation_*.json"))
    ...
    assert "hercules --resume 20260101_120000_abc123" in out, out
