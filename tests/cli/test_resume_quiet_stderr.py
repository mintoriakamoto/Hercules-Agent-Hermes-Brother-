...
``tool_progress_mode == "off"`` (set by ``hercules chat --quiet``):
...
def _make_cli(quiet=False, session_id="20260524_111111_xyz", db=None):
    """Build a minimal HerculesCLI bound to only what _init_agent needs for
...
class TestResumeQuietStderr:
    def test_session_not_found_goes_to_stderr_in_quiet_mode(self, capsys):
        ...
        assert "hercules sessions list" in captured.err
