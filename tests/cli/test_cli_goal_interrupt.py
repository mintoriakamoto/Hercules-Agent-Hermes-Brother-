...
minimal ``HerculesCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def hercules_home(tmp_path, monkeypatch):
    """Isolated HERCULES_HOME so SessionDB.state_meta writes stay hermetic."""
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal HerculesCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, hercules_home):
        ...
        with patch("hercules_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, hercules_home):
        ...
        with patch("hercules_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, hercules_home):
    ...
    def test_no_assistant_message_skipped(self, hercules_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, hercules_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, hercules_home):
        ...
        src = inspect.getsource(HerculesCLI.chat)
