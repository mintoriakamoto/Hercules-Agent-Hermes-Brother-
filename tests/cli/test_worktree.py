...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("hercules/hercules-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned hercules/* and pr-* branches."""

    def test_prunes_orphaned_hercules_branch(self, git_repo):
        """hercules/hercules-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "hercules/hercules-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "hercules/hercules-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "hercules/hercules-deadbeef" in result.stdout
        ...
        assert "hercules/hercules-deadbeef" in orphaned
        ...
        assert "hercules/hercules-deadbeef" not in result.stdout
...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"hercules/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"hercules pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
    ...
    def test_live_locked_survives_at_any_age(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "hercules-live", pid=os.getpid())
    ...
    def test_dead_locked_clean_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "hercules-dead", pid=999999)
    ...
    def test_dead_locked_dirty_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "hercules-deaddirty", pid=999999, dirty=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_dead_locked_unpushed_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "hercules-deadunp", pid=999999, unpushed=True)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        import cli
        p = git_repo / ".worktrees" / "hercules-x"
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "hercules/hercules-x", "HEAD"],
