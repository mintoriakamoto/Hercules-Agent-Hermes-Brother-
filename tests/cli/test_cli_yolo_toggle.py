...
Pre-fix bug (issue #33925): ``cli.HerculesCLI._toggle_yolo`` mutated only
``os.environ["HERCULES_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``hercules --yolo`` (process-start env), ``HERCULES_YOLO_MODE=1``,
and ``hercules config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``HerculesCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``HerculesCLI.__init__`` imports a lot of optional
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``HerculesCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``HerculesCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_enables_session_bypass(self):
        ...
        with patch("cli._cprint"):
            HerculesCLI._toggle_yolo(stand_in)
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            HerculesCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            HerculesCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False

    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``HERCULES_YOLO_MODE`` — that path is
        ...
        assert os.environ.get("HERCULES_YOLO_MODE") is None
    ...
    def test_two_independent_sessions_are_isolated(self):
        ...
        try:
            with patch("cli._cprint"):
                HerculesCLI._toggle_yolo(cli_a)
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert HerculesCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            HerculesCLI._toggle_yolo(stand_in)

        assert HerculesCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hercules --yolo`` sets ``HERCULES_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                HerculesCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestIsSessionYoloActiveAttrSafety:
    ...
    (tests use ``HerculesCLI.__new__(HerculesCLI)`` to skip ``__init__``). It must
    ...
    def test_helper_survives_missing_session_id_attr(self):
        ...
        assert HerculesCLI._is_session_yolo_active(no_attr) is False
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            HerculesCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_is_noop_when_ids_match(self):
        ...
        try:
            ...
            HerculesCLI._transfer_session_yolo(stand_in, "same-id", "same-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        HerculesCLI._transfer_session_yolo(stand_in, "", "new")
        HerculesCLI._transfer_session_yolo(stand_in, "old", "")
