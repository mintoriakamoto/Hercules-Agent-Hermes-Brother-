...
def _isolate(tmp_path, monkeypatch):
    """Isolate HERCULES_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal HerculesCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = HerculesCLI.__new__(HerculesCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["hercules-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Hercules' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Hercules' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Hercules' minimum context length."""
