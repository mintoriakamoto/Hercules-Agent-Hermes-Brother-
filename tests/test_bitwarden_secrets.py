...
binary download are exercised manually by `hercules secrets bitwarden
...
def hercules_home(tmp_path, monkeypatch):
    """Point Hercules at an isolated home directory."""
    home = tmp_path / ".hercules"
    home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(home))
    # Some modules cache get_hercules_home; clear if needed.
    import hercules_constants
    if hasattr(hercules_constants, "_HERCULES_HOME_CACHE"):
        hercules_constants._HERCULES_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(hercules_home, monkeypatch):
...
def test_install_bws_happy_path(hercules_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(hercules_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(hercules_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from hercules_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under hercules_home/cache/."""
    home = tmp_path / ".hercules"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
