...
profile's HERCULES_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.hercules/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active HERCULES_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever HERCULES_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    HERCULES_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import hercules_constants

    monkeypatch.setattr(
        hercules_constants, "_get_platform_default_hercules_home", lambda: root
    )
    monkeypatch.setenv("HERCULES_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile HERCULES_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "hercules_home"
    ...
    assert hercules_constants.get_hercules_home().resolve() == profile_home.resolve()
    assert hercules_constants.get_default_hercules_root().resolve() == root.resolve()

    # cron/jobs.py computes HERCULES_DIR from get_hercules_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.HERCULES_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_hercules_home", None, raising=False)
    assert scheduler._get_hercules_home().resolve() == profile_home.resolve()
    assert scheduler._get_hercules_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (HERCULES_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.HERCULES_DIR.resolve() == root.resolve()
