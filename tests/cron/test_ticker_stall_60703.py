...
class TestFutureDatedClaims:
    ...
    def test_future_fire_claim_is_treated_as_stale(self):
        ...
        for j in jobs:
            if j["id"] == job["id"]:
                future = jobs_mod._hercules_now() + timedelta(hours=6)
    ...
    def test_expired_fire_claim_is_reclaimable(self):
        ...
        for j in jobs:
            if j["id"] == job["id"]:
                past = jobs_mod._hercules_now() - timedelta(hours=6)
    ...
    def test_future_run_claim_does_not_skip_oneshot_forever(self):
        ...
        past_fire = (jobs_mod._hercules_now() - timedelta(seconds=30)).isoformat()
