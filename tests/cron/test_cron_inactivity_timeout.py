...
- Unlimited timeout (HERCULES_CRON_TIMEOUT=0)
- Backward compat: HERCULES_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """HERCULES_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """HERCULES_CRON_TIMEOUT env var is respected."""
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """HERCULES_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """HERCULES_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("HERCULES_CRON_TIMEOUT", "abc")
        raw = os.getenv("HERCULES_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """HERCULES_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("HERCULES_CRON_TIMEOUT", "")
        raw = os.getenv("HERCULES_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_hercules_time_importable(self):
        """hercules_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _hercules_now
        assert callable(_hercules_now)

    def test_hercules_constants_importable(self):
        """hercules_constants should be importable from cron context."""
        from hercules_constants import get_hercules_home
        assert callable(get_hercules_home)
