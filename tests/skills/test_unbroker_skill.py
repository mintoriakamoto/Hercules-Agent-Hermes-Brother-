...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and hercules-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # hercules-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_hercules_home():
    prev = os.environ.get("HERCULES_HOME")
    with tempfile.TemporaryDirectory() as d:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("HERCULES_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
