...
from tools.environments.local import _HERCULES_PROVIDER_ENV_FORCE_PREFIX
...
def test_close_terminal_tool_gated_on_desktop(monkeypatch):
    """Hidden unless HERCULES_DESKTOP is set (mirrors read_terminal gating)."""
...
class TestOrphanedPipeReconciliation:
    ...
    `hercules update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_HERCULES_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_HERCULES_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/hercules_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/hercules_bg_" not in bg_command
        assert "cat /tmp/hercules_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/hercules_bg.log",
                "/path with spaces/hercules_bg.pid",
                "/path with spaces/hercules_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/hercules_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/hercules_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/hercules_bg.exit' 2>/dev/null"
...
class TestSigkillEscalation:
    ...
    def test_grace_reader_floors_at_zero(self, monkeypatch):
        ...
        import hercules_cli.config as cfg_mod
