...
def _clean_state():
    ...
    saved = {}
    for k in ("HERCULES_INTERACTIVE", "HERCULES_GATEWAY_SESSION", "HERCULES_EXEC_ASK", "HERCULES_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["HERCULES_INTERACTIVE"] = "1"
        session_key = os.getenv("HERCULES_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No HERCULES_INTERACTIVE or HERCULES_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
