...
Verifies:
  - Tools are gated on HERCULES_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERCULES_KANBAN_TASK", "t_fake")
    home = tmp_path / ".hercules"
    home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the HERCULES_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: HERCULES_HOME isolated, HERCULES_KANBAN_TASK set
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["hercules_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("HERCULES_SESSION_ID", "session-trusted")
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("HERCULES_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated HERCULES_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to HERCULES_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``HERCULES_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``hercules-system`` and
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from HERCULES_PROFILE in the fixture, not the
        # caller-supplied "hercules-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no HERCULES_KANBAN_TASK) still default to
...
def test_create_stamps_session_id_from_env(monkeypatch, worker_env):
    ...
    originating chat session id via HERCULES_SESSION_ID. ``kanban_create``
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (HERCULES_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has HERCULES_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no HERCULES_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # HERCULES_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    monkeypatch.setenv("HERCULES_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no HERCULES_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via HERCULES_KANBAN_BOARD env var,
...
# restarting Hercules. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Hercules home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    HERCULES_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".hercules"
    home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(home))
    # Make sure neither HERCULES_KANBAN_DB nor HERCULES_KANBAN_BOARD pin a
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning HERCULES_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".hercules"
    home.mkdir()
    monkeypatch.setenv("HERCULES_HOME", str(home))
    monkeypatch.setenv("HERCULES_PROFILE", "alt-worker")
    monkeypatch.delenv("HERCULES_KANBAN_DB", raising=False)
    monkeypatch.delenv("HERCULES_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: HERCULES_SESSION_PLATFORM + HERCULES_SESSION_CHAT_ID set.
# - TUI sessions: HERCULES_SESSION_KEY (or HERCULES_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("HERCULES_SESSION_USER_ID", "user-9")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports HERCULES_SESSION_KEY.
    ...
    monkeypatch.delenv("HERCULES_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("HERCULES_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("HERCULES_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.hercules; use a fresh sibling
