...
Verifies that Hercules-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Hercules secrets.

See: https://github.com/NousResearch/hercules-agent/issues/1002
See: https://github.com/NousResearch/hercules-agent/issues/1264
"""
...
from tools.environments.local import (
    ...
    _HERCULES_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.hercules/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Hercules inference secret
        ...
        models`` run inside a Hercules terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _HERCULES_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Hercules-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "HERCULES_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _HERCULES_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_HERCULES_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_HERCULES_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_HERCULES_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_HERCULES_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Hercules venv path instead of the project's own ``.venv`` —
    silently clobbering the Hercules environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Hercules venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.hercules/hercules-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/hercules",
        })
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_HERCULES_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Hercules (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _HERCULES_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _HERCULES_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Hercules-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _HERCULES_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Hercules, and blocklisting
        ...
        leaked_block = general_chain & _HERCULES_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_HERCULES_PROVIDER_ENV_BLOCKLIST)

    def test_claude_code_oauth_token_is_inheritable(self):
        """CLAUDE_CODE_OAUTH_TOKEN is owned by the user's Claude Code install
        (subscription OAuth), not a Hercules inference credential. Stripping it
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _HERCULES_PROVIDER_ENV_BLOCKLIST
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hercules_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        hercules-install-dir prepend (a separate concern, covered by
        TestHerculesBinDirOnPath) so a real ``hercules`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._HERCULES_BIN_DIR
        local_mod._HERCULES_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._HERCULES_BIN_DIR = saved
...
class TestHerculesBinDirOnPath:
    """The hercules install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``hercules`` via the terminal tool must work
    even when the gateway was launched without the hercules install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_hercules_bin_dir / _prepend_hercules_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._HERCULES_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/hercules/bin/hercules" if name == "hercules" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/hercules/bin")
        assert local_mod._resolve_hercules_bin_dir() == "/opt/hercules/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "hercules").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_hercules_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_hercules_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._HERCULES_BIN_DIR = "/opt/hercules/bin"
        out = local_mod._prepend_hercules_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/hercules/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._HERCULES_BIN_DIR = "/opt/hercules/bin"
        once = local_mod._prepend_hercules_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_hercules_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/hercules/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._HERCULES_BIN_DIR = None
        assert local_mod._prepend_hercules_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_hercules_bin_dir(self, monkeypatch):
        """A gateway env missing the hercules dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._HERCULES_BIN_DIR = "/opt/hercules/bin"
        ...
        assert entries[0] == "/opt/hercules/bin"
        assert "/usr/bin" in entries


class TestHerculesInternalDynamicSecrets:
    """Dynamically-named Hercules secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_HERCULES_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_hercules_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``hercules_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_hercules_internal_secret
        assert _is_hercules_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_hercules_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_hercules_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_hercules_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_hercules_internal_secret
        assert _is_hercules_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_hercules_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_hercules_internal_secret
        assert _is_hercules_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_hercules_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_hercules_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_hercules_internal_secret
        assert not _is_hercules_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_hercules_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_hercules_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_hercules_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_hercules_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_hercules_internal_secret("PATH")
        assert not _is_hercules_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _HERCULES_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _HERCULES_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _HERCULES_PROVIDER_ENV_BLOCKLIST
