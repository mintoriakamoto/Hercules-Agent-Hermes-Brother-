...
- _seed_from_env reads API keys from ~/.hercules/.env when not in os.environ
- _resolve_api_key_provider_secret falls back to credential_pool when env vars are empty
- ~/.hercules/.env takes priority over os.environ for Hercules-managed credentials
...
@pytest.fixture
def isolated_hercules_home(tmp_path, monkeypatch):
    """Point HERCULES_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.hercules/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.hercules/.env, not just os.environ.
    ...
    load_hercules_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_hercules_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_hercules_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_hercules_home):
        ...
        _write_env_file(isolated_hercules_home, OPENROUTER_API_KEY="sk-or-dotenv-abc")
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_hercules_home, monkeypatch):
        ...
        """
        _write_env_file(isolated_hercules_home, DEEPSEEK_API_KEY="sk-dotenv-fresh")
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.hercules/.env."""
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        ...
        """Regression for #20591: when both ~/.hercules/.env and os.environ define
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.hercules/.env over a stale shell export. This path resolves
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_hercules_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_hercules_home):
...
class TestAnthropicEnvAuthTypeClassification:
    ...
    def test_admin_key_classified_as_api_key(self, isolated_hercules_home):
