...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._hercules_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(hercules_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(hercules_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hercules_bin(self):
        """After _INSTALL_FAILED, manual install in HERCULES_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        hercules_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(hercules_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(hercules_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._hercules_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == hercules_bin
            assert _tirith_mod._resolved_path == hercules_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# HERCULES_HOME isolation
# ---------------------------------------------------------------------------

class TestHerculesHomeIsolation:
    def test_hercules_bin_dir_respects_hercules_home(self):
        """_hercules_bin_dir must use HERCULES_HOME, not hardcoded ~/.hercules."""
        from tools.tirith_security import _hercules_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"HERCULES_HOME": tmpdir}):
            result = _hercules_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_hercules_home(self):
        """_failure_marker_path must use HERCULES_HOME, not hardcoded ~/.hercules."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"HERCULES_HOME": "/custom/hercules"}):
            result = _failure_marker_path()
        assert result == "/custom/hercules/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets HERCULES_HOME; verify it's active."""
        hercules_home = os.getenv("HERCULES_HOME")
        assert hercules_home is not None, "HERCULES_HOME should be set by conftest"
        assert "hercules_test" in hercules_home, "Should point to test temp dir"

    def test_get_hercules_home_fallback(self):
        """Without HERCULES_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_hercules_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove HERCULES_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".hercules")
            result = _get_hercules_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._hercules_bin_dir",
