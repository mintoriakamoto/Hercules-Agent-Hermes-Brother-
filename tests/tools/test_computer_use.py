...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"HERCULES_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Hercules-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (NousResearch/hercules-agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (NousResearch/hercules-agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (NousResearch/hercules-agent#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Hercules patch.
    ...
    def test_future_renamed_subcommand_is_honored(self):
        ...
        instead of `mcp` keeps working without a Hercules patch."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so HERCULES_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (NousResearch/hercules-agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (NousResearch/hercules-agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (NousResearch/hercules-agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Hercules never declared a cua-driver
    ...
    def test_session_id_format(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        backend = CuaDriverBackend()
        # hercules-{12 hex chars} — short enough to surface in logs
        # without being a privacy hazard, unique enough for concurrent runs.
        assert backend._session_id.startswith("hercules-")
        assert len(backend._session_id) == 9 + 12

    def test_session_id_unique_per_backend(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        a = CuaDriverBackend()._session_id
        b = CuaDriverBackend()._session_id
        assert a != b, "each Hercules run should mint its own session id"
