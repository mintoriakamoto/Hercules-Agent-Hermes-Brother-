"""Contract tests for the Docker image's immutable /opt/hercules install tree."""
...
def test_dockerfile_makes_opt_hercules_readonly_for_hercules_user() -> None:
    ...
    assert "chown -R root:root /opt/hercules" not in text
    assert "chmod -R a+rX /opt/hercules" not in text
    assert "chmod -R a-w /opt/hercules" not in text


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV HERCULES_HOME=/opt/data" in text
    assert "ENV HERCULES_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV HERCULES_DISABLE_LAZY_INSTALLS=1" in text
    assert "HERCULES_TUI_DIR=/opt/hercules/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_hercules() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+hercules:hercules\s+/opt/hercules/\.venv",
        r"chown\s+-R\s+hercules:hercules\s+/opt/hercules/ui-tui",
        r"chown\s+-R\s+hercules:hercules\s+/opt/hercules/gateway",
        r"chown\s+-R\s+hercules:hercules\s+/opt/hercules/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/hercules must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/hercules/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $HERCULES_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the hercules
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/hercules/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/hercules/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/hercules tree.
    assert f"ENV HERCULES_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV HERCULES_LAZY_INSTALL_TARGET=/opt/hercules" not in text
    ...
    # succeed as the unprivileged hercules runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$HERCULES_HOME/lazy-packages"' in stage2, (
