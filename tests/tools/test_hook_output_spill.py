...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_user_overrides_are_respected(self):
        ...
        with patch("hercules_cli.config.load_config", return_value=user_cfg):
            cfg = hos.get_spill_config()
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("hercules_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="hercules-spill-test-")
    ...
    def test_session_id_with_path_separators_is_sanitised(self):
        ...
        self.assertFalse(Path("/etc/passwd-hercules-test").exists())
    ...
    def test_default_directory_uses_hercules_home(self):
        """When no directory override, spill under HERCULES_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="hercules-home-")
        try:
            with patch.dict(os.environ, {"HERCULES_HOME": test_home}):
                # Also patch get_hercules_home to the env var to mirror production.
            ...
            # ~/.hercules/hook_outputs depending on get_hercules_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.hercules/hook_outputs/sess")),
            ]
