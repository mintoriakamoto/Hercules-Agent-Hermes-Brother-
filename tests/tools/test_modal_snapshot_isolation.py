...
def _restore_tool_modules():
    original_hercules_home = os.environ.get("HERCULES_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "hercules_cli"
        or name.startswith("hercules_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_hercules_home is None:
            os.environ.pop("HERCULES_HOME", None)
        else:
            os.environ["HERCULES_HOME"] = original_hercules_home
        _reset_modules(("tools", "hercules_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    hercules_cli = types.ModuleType("hercules_cli")
    hercules_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["hercules_cli"] = hercules_cli
    hercules_home = tmp_path / "hercules-home"
    os.environ["HERCULES_HOME"] = str(hercules_home)
    sys.modules["hercules_cli.config"] = types.SimpleNamespace(
        get_hercules_home=lambda: hercules_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="hercules-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": hercules_home / "modal_snapshots.json",
