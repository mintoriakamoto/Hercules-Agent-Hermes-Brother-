...
Older images made /opt/hercules/ui-tui and /opt/hercules/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERCULES_TUI_DIR=/opt/hercules/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hercules:hercules /opt/hercules/ui-tui" not in text
    assert "chown -R hercules:hercules /opt/hercules/node_modules" not in text
