"""Regression tests for HERCULES_HOME override propagation onto the MCP loop.
...
get_hercules_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: HerculesTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(HerculesTokenStorage("probe-srv")._tokens_path())
