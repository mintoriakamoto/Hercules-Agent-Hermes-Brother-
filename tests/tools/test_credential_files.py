...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        hercules_home.mkdir()
        (hercules_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(hercules_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.hercules/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.hercules/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        hercules_home = tmp_path / ".hercules"
        (hercules_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"HERCULES_HOME": str(hercules_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.hercules")

        assert mounts[0]["container_path"] == "/home/user/.hercules/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.hercules/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.hercules/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape HERCULES_HOME."""
        monkeypatch.setenv("HERCULES_HOME", str(tmp_path / ".hercules"))
        (tmp_path / ".hercules").mkdir()

        # Create a sensitive file one level above hercules_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside hercules_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside HERCULES_HOME must still be registered."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
        (hercules_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_hercules_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of HERCULES_HOME must be allowed."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        subdir = hercules_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside HERCULES_HOME pointing outside must be rejected."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))

        # Create a sensitive file outside hercules_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside hercules_home pointing outside
        symlink = hercules_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes HERCULES_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(hercules_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(hercules_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside HERCULES_HOME via config must still mount."""
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))

        (hercules_home / "oauth.json").write_text("{}")
        self._write_config(hercules_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        (hercules_home / "cache" / "documents").mkdir(parents=True)
        (hercules_home / "cache" / "audio").mkdir(parents=True)
        (hercules_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.hercules/cache/documents" in paths
        assert "/root/.hercules/cache/audio" in paths
        assert "/root/.hercules/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.hercules/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_hercules_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        hercules_home = tmp_path / ".hercules"
        hercules_home.mkdir()
        # Use legacy dir name with content — get_hercules_dir prefers
        # populated old over new.
        legacy_doc = hercules_home / "document_cache"
        legacy_img = hercules_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(hercules_home / "document_cache") in host_paths
        assert str(hercules_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.hercules/cache/documents" in container_paths
        assert "/root/.hercules/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        hercules_home = tmp_path / ".hercules"
        img_dir = hercules_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.hercules/cache/images/generated.png"
        )
    ...
    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        hercules_home = tmp_path / ".hercules"
        (hercules_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("HERCULES_HOME", str(hercules_home))
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        doc_dir = hercules_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        hercules_home = tmp_path / ".hercules"
        ss_dir = hercules_home / "cache" / "screenshots"
