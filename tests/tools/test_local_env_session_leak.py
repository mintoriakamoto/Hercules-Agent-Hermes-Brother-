"""Cross-session HERCULES_SESSION_* leak guard for the local terminal backend.

Regression coverage for the bug where a terminal subprocess could observe a
*different concurrent session's* ``HERCULES_SESSION_KEY`` (and the other
``HERCULES_SESSION_*`` vars).
...
subprocess-env bridge resolves each ``HERCULES_SESSION_*`` from the ContextVar and,
...
def test_engaged_unset_contextvar_strips_foreign_session_key(monkeypatch):
    ...
    os.environ["HERCULES_SESSION_KEY"], this task's ContextVar is unset, and the
    ...
    assert "HERCULES_SESSION_KEY" not in env, (
        "Foreign concurrent session key leaked into subprocess env: "
        f"{env.get('HERCULES_SESSION_KEY')!r}"
    )
...
def test_engaged_strips_all_session_vars_when_unset(monkeypatch):
    """The strip covers every HERCULES_SESSION_* mirror, not just the key."""
    _engage()
    monkeypatch.setenv("HERCULES_SESSION_KEY", "foreign-key")
    monkeypatch.setenv("HERCULES_SESSION_THREAD_ID", "foreign-thread")
    monkeypatch.setenv("HERCULES_SESSION_CHAT_ID", "foreign-chat")
    monkeypatch.setenv("HERCULES_SESSION_USER_ID", "foreign-user")
...
def test_unengaged_process_preserves_os_environ_fallback(monkeypatch):
    ...
    Pure single-process CLI/one-shot sets HERCULES_SESSION_* directly in os.environ
    ...
    monkeypatch.setenv("HERCULES_SESSION_KEY", "cli-session-key")
    monkeypatch.setenv("HERCULES_SESSION_ID", "cli-session-id")

    env = _make_run_env({})

    assert env.get("HERCULES_SESSION_KEY") == "cli-session-key"
    assert env.get("HERCULES_SESSION_ID") == "cli-session-id"
...
def test_engaged_explicit_empty_contextvar_clears(monkeypatch):
    ...
    """
    monkeypatch.setenv("HERCULES_SESSION_KEY", "foreign-after-clear")
    ...
    assert env.get("HERCULES_SESSION_KEY", "") == "", (
        f"Foreign key survived an explicit clear: {env.get('HERCULES_SESSION_KEY')!r}"
    )
...
def test_explicit_empty_thread_id_overrides_stale_value(monkeypatch):
    ...
    no thread id binds HERCULES_SESSION_THREAD_ID="" and that empty value must win
    over an older non-empty value left in os.environ.
    """
    monkeypatch.setenv("HERCULES_SESSION_THREAD_ID", "stale-thread-from-prior-turn")
    ...
    assert env.get("HERCULES_SESSION_THREAD_ID") == "", (
        "Bound-empty thread id did not override the stale value: "
        f"{env.get('HERCULES_SESSION_THREAD_ID')!r}"
    )
    assert env.get("HERCULES_SESSION_KEY") == "mm:chan"
...
def test_sanitize_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "HERCULES_SESSION_KEY": "agent:main:discord:thread:FOREIGN_BG:FOREIGN_BG",
        "HERCULES_SESSION_THREAD_ID": "FOREIGN_BG",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert "HERCULES_SESSION_KEY" not in sanitized, (
        f"Background subprocess inherited foreign key: {sanitized.get('HERCULES_SESSION_KEY')!r}"
    )
    assert "HERCULES_SESSION_THREAD_ID" not in sanitized
...
def test_sanitize_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "HERCULES_SESSION_KEY": "cli-bg-key",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert sanitized.get("HERCULES_SESSION_KEY") == "cli-bg-key"
...
def test_hercules_subprocess_env_bound_contextvar_wins(monkeypatch):
    ...
    try:
        env = hercules_subprocess_env()
        assert env.get("HERCULES_SESSION_KEY") == "agent:main:discord:group:MINE:111"
    finally:
        clear_session_vars(tokens)


def test_hercules_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    """A pure single-process CLI (never engaged) keeps the inherited fallback."""
    monkeypatch.setenv("HERCULES_SESSION_KEY", "cli-fallback-key")
    # not engaged (autouse fixture leaves _session_context_engaged False)
    env = hercules_subprocess_env()
    assert env.get("HERCULES_SESSION_KEY") == "cli-fallback-key"
