...
def test_hercules_provider_subclass_exists():
    """HerculesMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _HERCULES_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _HERCULES_PROVIDER_CLS is not None
    assert issubclass(_HERCULES_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hercules_provider_subclass(tmp_path, monkeypatch):
    ...
    assert _HERCULES_PROVIDER_CLS is not None
    assert isinstance(provider, _HERCULES_PROVIDER_CLS)
    assert provider._hercules_server_name == "srv"
