...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `hercules-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.hercules/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.hercules/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because hercules uses an internal scheduler, not system crontab.
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("HERCULES_INTERACTIVE", false_like_value)
        monkeypatch.delenv("HERCULES_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("HERCULES_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["HERCULES_INTERACTIVE", "HERCULES_GATEWAY_SESSION", "HERCULES_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("HERCULES_INTERACTIVE", "HERCULES_GATEWAY_SESSION", "HERCULES_EXEC_ASK"):
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import hercules_cli.runtime_provider as rp_mod
