...
class TestScanMemoryContent:
    ...
    def test_hercules_config_mod_blocked(self):
        result = _scan_memory_content("edit .hercules/config.yaml to change settings")
        assert "Blocked" in result
        assert "hercules_config_mod" in result
        result = _scan_memory_content("update .hercules/SOUL.md with new personality")
        assert "Blocked" in result
        assert "hercules_config_mod" in result
    ...
    def test_hercules_config_mod_no_false_positives(self):
        """Merely mentioning hercules config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .hercules/config.yaml for settings") is None
        assert _scan_memory_content("Read .hercules/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .hercules/config.yaml file contains runtime options") is None
