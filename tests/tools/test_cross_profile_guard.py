...
def fake_hercules(tmp_path, monkeypatch):
    """Build a two-profile Hercules layout and point HERCULES_HOME at
    the hercules-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_hercules):
        from tools.file_tools import write_file_tool
        target = fake_hercules["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hercules):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_hercules["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "hercules-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_hercules):
    ...
    def test_non_hercules_path_unaffected(self, fake_hercules, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_hercules):
    ...
    def test_cross_profile_patch_bypass(self, fake_hercules):
        from tools.file_tools import patch_tool
        target = fake_hercules["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_hercules):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        self._make_skill_in_profile(fake_hercules["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up HERCULES_HOME (set in
        ...
        assert "not found in active profile 'hercules-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_hercules, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_hercules["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_hercules["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.hercules/profiles/<name>/."""
        # Don't set HERCULES_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_hercules_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_hercules_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hercules):
        """When active profile is 'hercules-security', the prompt warns
        ...
        assert "Active Hercules profile" in src
        assert "cross_profile=True" in src
        assert "~/.hercules/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Hercules profile: default" in src
        assert "Active Hercules profile: {active_profile}" in src
