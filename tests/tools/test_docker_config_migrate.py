...
def _run_migration(hercules_home: Path, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.update(
        {
            "HERCULES_HOME": str(hercules_home),
            "HERCULES_SKIP_CHMOD": "1",
...
def test_docker_config_migrate_does_not_rewrite_invalid_yaml(tmp_path: Path) -> None:
    ...
    assert "hercules config:" in proc.stderr
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    ...
    proc = _run_migration(tmp_path, HERCULES_SKIP_CONFIG_MIGRATION="1")
...
def test_docker_config_migrate_second_boot_preserves_env_byte_for_byte(tmp_path: Path) -> None:
    ...
    ``$HERCULES_HOME/.env``. The first boot migrates the stale config and bumps
