...
class TestWriteDenyExactPaths:
    ...
    def test_hercules_env(self):
        # ``.env`` under the active HERCULES_HOME (profile-aware, not just
        # ``~/.hercules``) must be write-denied. The hermetic test conftest
        # points HERCULES_HOME at a tempdir — resolve via get_hercules_home()
        # to match the denylist.
        from hercules_constants import get_hercules_home
        path = str(get_hercules_home() / ".env")
        assert _is_write_denied(path) is True

    def test_hercules_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.hercules/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "hercules_root"
        ...
        # Sanity check: HERCULES_HOME does point to the profile dir, not the root.
        from hercules_constants import get_hercules_home, get_default_hercules_root
        assert get_hercules_home() == profile_home
        assert get_default_hercules_root() == root
...
class TestWriteAllowed:
    ...
    def test_hercules_control_files_requested_writable(self):
        from hercules_constants import get_hercules_home
