...
class TestDeregisterAuthorization:
    ...
    def test_plugin_root_module_can_deregister_submodule_handler(self):
        ...
        hercules_plugins.pkg (root cleanup code) must be allowed to deregister a
        tool whose handler was defined in hercules_plugins.pkg.handlers.  The
        exact module strings differ, but they share the same plugin package root
        (hercules_plugins.pkg) — ownership is bound to the package, not the leaf
    ...
    def test_opted_in_plugin_submodule_can_deregister(self):
        ...
        root (``hercules_plugins.allowed``).  If the caller is a submodule
        (``hercules_plugins.allowed.cleanup``), the old code looked up
        ``_plugin_override_policy.get("hercules_plugins.allowed.cleanup")`` →
