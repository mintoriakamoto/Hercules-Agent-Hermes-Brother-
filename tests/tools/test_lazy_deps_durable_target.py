...
class TestGatingWithTarget:
    """``HERCULES_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_disable_env_blocks_without_target(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "hercules_cli.config.load_config", lambda: {}, raising=False
        )
...
@pytest.mark.skipif(
    os.environ.get("HERCULES_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set HERCULES_RUN_NETWORK_TESTS=1); CI runs "
