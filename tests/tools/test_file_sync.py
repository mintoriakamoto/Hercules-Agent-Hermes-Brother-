...
def _make_get_files(tmp_files, remote_base="/root/.hercules"):
...
def _make_manager(tmp_files, remote_base="/root/.hercules", upload=None, delete=None):
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.hercules/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
    ...
    def test_sync_back_does_not_overwrite_uploaded_credential_files(self, tmp_path, monkeypatch):
            ...
            lambda: [
                {
                    ...
                    "container_path": "/root/.hercules/credentials/token.json",
                }
            ],
        )
        monkeypatch.setattr(
            "tools.credential_files.iter_skills_files",
            lambda container_base="/root/.hercules": [
                {
                    "host_path": str(skill),
        ...
        def bulk_download(dest: Path) -> None:
            with tarfile.open(dest, "w") as tar:
                for name, data in {
                    "root/.hercules/credentials/token.json": b"remote-token",
                    "root/.hercules/skills/skill.py": b"remote-skill",
                }.items():
        ...
        mgr.sync(force=True)
        mgr.sync_back(hercules_home=tmp_path)
