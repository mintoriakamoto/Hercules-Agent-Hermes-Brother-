...
The contract these tests pin down:
  ...
  * Explicit ``HERCULES_DASHBOARD_WS_HOST`` env var → wins always, regardless
    of the bind host.
...
def clear_ws_host_env(monkeypatch):
    """Ensure no ``HERCULES_DASHBOARD_WS_HOST`` leaks in from the test shell."""
    monkeypatch.delenv("HERCULES_DASHBOARD_WS_HOST", raising=False)
    yield monkeypatch
...
class TestResolveClientWsHost:
    ...
    def test_explicit_env_wins_over_wildcard(
        ...
        monkeypatch.setenv("HERCULES_DASHBOARD_WS_HOST", "10.0.0.7")
        _set_bound(saved_app_state, "0.0.0.0")
    ...
    def test_blank_env_falls_back_to_bind(
        ...
        """An explicitly empty override (e.g. ``HERCULES_DASHBOARD_WS_HOST=``)
        ...
        monkeypatch.setenv("HERCULES_DASHBOARD_WS_HOST", "   ")
        _set_bound(saved_app_state, "0.0.0.0")
        assert web_server._resolve_client_ws_host() == "127.0.0.1"
...
def test_netloc_helper_handles_ipv6_bracket_form():
    ...
    branch when ``HERCULES_DASHBOARD_WS_HOST`` points at an IPv6 address.
