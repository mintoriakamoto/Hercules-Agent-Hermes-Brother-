...
Phase 4 wires `hercules -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"hercules profile create {PROFILE}")
    ...
    r = _sh(container_name, f"hercules -p {PROFILE} gateway start", timeout=60)
...
def test_profile_delete_stops_gateway(
    ...
    _sh(container_name, f"hercules profile create {PROFILE}")
    _sh(container_name, f"hercules -p {PROFILE} gateway start", timeout=60)
    time.sleep(3)

    r = _sh(
        container_name,
        f"hercules profile delete {PROFILE} --yes",
        timeout=30,
    )
