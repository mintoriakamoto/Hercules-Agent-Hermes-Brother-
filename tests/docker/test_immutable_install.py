...
Build the real image and verify at runtime:
  ...
  1. /opt/hercules is not writable by the hercules user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and HERCULES_DISABLE_LAZY_INSTALLS are set
  3. /opt/hercules/.install_method contains "docker" (code-scoped stamp)
  4. $HERCULES_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $HERCULES_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_hercules(
    built_image: str, container_name: str,
) -> None:
    """The hercules user must not be able to modify /opt/hercules.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/hercules as the hercules user
        "touch /opt/hercules/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hercules user can write to /opt/hercules (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/hercules/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hercules user can write to /opt/hercules/.venv: {r.stdout}"
    )


def test_hercules_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    HERCULES_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$HERCULES_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"HERCULES_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/hercules/.install_method (code-scoped), NOT in $HERCULES_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/hercules/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/hercules/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $HERCULES_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        "$HERCULES_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $HERCULES_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $HERCULES_HOME was not healed on boot: "
        f"{r.stdout}"
    )
