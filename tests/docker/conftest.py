...
Override the image with ``HERCULES_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("HERCULES_TEST_IMAGE", "hercules-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``HERCULES_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("HERCULES_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged hercules user
# ---------------------------------------------------------------------------
#
# Background: every Hercules runtime path inside the container drops to UID
# 10000 (the ``hercules`` user) via ``s6-setuidgid hercules``. ``docker exec``
...
# fails (PermissionError swallowed) for hercules, so a test that ran as root
...
# helpers below run every probe under ``-u hercules`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: hercules).
