"""Harness: dashboard opt-in via HERCULES_DASHBOARD.

Today (tini): dashboard starts once when HERCULES_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    r = docker_exec(container_name, "pgrep", "-f", "hercules dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without HERCULES_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `hercules doctor` and any other health check).
    ...
    HERCULES_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without HERCULES_DASHBOARD; "
...
def test_dashboard_restarts_after_crash(
    ...
    # Kill the dashboard. The dashboard process runs as hercules, so the
    # hercules user can kill it (same UID).
...
# whenever `HERCULES_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_auth_gate_engages_on_non_loopback_bind(
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and HERCULES_DASHBOARD_INSECURE "
