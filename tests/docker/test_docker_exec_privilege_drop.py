...
The shim (docker/hercules-exec-shim.sh, installed at /opt/hercules/bin/hercules)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> hercules login` would write /opt/data/auth.json as
...
failed: Hercules is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> hercules …`` (defaulting to root) gets dropped to the
   hercules user before the real binary runs.
2. ``docker exec --user hercules <c> hercules …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $HERCULES_HOME from a ``docker exec`` session land
   as hercules:hercules — the actual user-visible invariant.
4. The HERCULES_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-hercules-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown hercules:hercules`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to hercules:hercules,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$HERCULES_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (hercules_cli.container_boot), which s6 runs
    *strictly after* ``01-hercules-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_hercules_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `hercules` with a Python-style `-c` shim equivalent — there's no
    pure-hercules "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `hercules` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `hercules`
    do anything that writes to disk, then check the file's owner.

    Use `hercules config set` which writes config.yaml under HERCULES_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "hercules", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by hercules, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "hercules:hercules", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected hercules:hercules. "
        "The shim did not drop privileges before invoking hercules."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user hercules already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "hercules", sleep_container,
         "hercules", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user hercules failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up hercules:hercules — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """HERCULES_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "HERCULES_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "hercules", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With HERCULES_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``HERCULES_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``HERCULES_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"HERCULES_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "hercules", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "hercules:hercules", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected hercules:hercules"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/hercules/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid hercules hercules <args>` which resolves `hercules`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> hercules login`` (root) wrote
    ...
    "Hercules is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `hercules config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is hercules-readable.

    Specifically: pretend the operator ran `hercules login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the hercules UID. We use `hercules auth list` since that touches the
    ...
    # `hercules auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `hercules config set` first to
    # provoke a write into HERCULES_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "hercules", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # HERCULES_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "hercules", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the hercules user "
