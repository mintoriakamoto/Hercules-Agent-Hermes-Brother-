...
Converted from the former ``.github/actions/hercules-smoke-test`` composite
...
def test_hercules_help(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"hercules --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"hercules --help produced a traceback: {r.stderr[-2000:]!r}"
    )
...
def test_dashboard_subcommand_present(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"hercules dashboard --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
