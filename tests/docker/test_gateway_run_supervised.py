...
Before the s6 migration, ``docker run nousresearch/hercules-agent gateway
...
crash and a supervised dashboard alongside (when ``HERCULES_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``HERCULES_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``HERCULES_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``hercules`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'hercules.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'hercules.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → hercules gateway run) can take
    ...
    # (main-wrapper.sh → python → hercules gateway run) can take well over
    ...
    # top), NOT the static main-hercules service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-hercules
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``hercules gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    start_container(
        ...
        "HERCULES_GATEWAY_NO_SUPERVISE=1",
        cmd="gateway run",
    )
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "HERCULES_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The HERCULES_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``hercules gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``hercules gateway
    ...
    # (main-wrapper.sh → python → hercules gateway run) can take well
    ...
    # started either — so we'd see the CMD's `hercules gateway run`
    ...
    # `hercules gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*hercules.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `hercules gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-hercules`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``HERCULES_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${HERCULES_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Hercules Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Hercules Gateway
    ...
    assert "⚕" in combined or "Hercules Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Hercules Gateway Starting" in file_contents, (
