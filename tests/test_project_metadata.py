...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install hercules-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"hercules-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `hercules update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid hercules update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `hercules update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `hercules update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (hercules_cli/dingtalk_auth.py)
...
def test_nemo_relay_extra_uses_official_0_3_distribution():
    ...
    assert not any(
        spec == "hercules-agent[nemo-relay]"
        for spec in optional_dependencies["all"]
    )
