...
from acp_adapter.server import HerculesACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["hercules-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "hercules_cli.config",
        mod("hercules_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
