...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"hercules-agent[termux]"' in text
    assert '"hercules-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"hercules-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
