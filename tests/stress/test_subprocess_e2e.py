...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves hercules_cli.kanban_db on its own
  ...
  - worker logs are captured to HERCULES_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "HERCULES_KANBAN_TASK": task.id,
            "HERCULES_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="hercules_e2e_")
    os.environ["HERCULES_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from hercules_cli import kanban_db as kb

    # Point the `hercules` CLI child processes will run at the worktree
    # hercules_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "hercules")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m hercules_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/hercules_e2e_*/worker_*.log
    import glob
