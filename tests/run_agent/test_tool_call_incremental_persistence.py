...
def _make_agent():
    hercules_home = Path(tempfile.mkdtemp(prefix="hercules-test-home-"))
    (hercules_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._hercules_home", hercules_home),
