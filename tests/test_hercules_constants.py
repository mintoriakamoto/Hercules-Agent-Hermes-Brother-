"""Tests for hercules_constants module."""
...
from hercules_constants import (
    ...
    find_hercules_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_hercules_root,
    get_hercules_dir,
    get_hercules_home,
    heal_hercules_managed_node,
    hercules_managed_node_tree_present,
    iter_hercules_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultHerculesRoot:
    """Tests for get_default_hercules_root() — Docker/custom deployment awareness."""

    def test_no_hercules_home_returns_native(self, tmp_path, monkeypatch):
        """When HERCULES_HOME is not set, returns ~/.hercules."""
        monkeypatch.delenv("HERCULES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hercules_root() == tmp_path / ".hercules"

    def test_hercules_home_is_native(self, tmp_path, monkeypatch):
        """When HERCULES_HOME = ~/.hercules, returns ~/.hercules."""
        native = tmp_path / ".hercules"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERCULES_HOME", str(native))
        assert get_default_hercules_root() == native

    def test_hercules_home_is_profile(self, tmp_path, monkeypatch):
        """When HERCULES_HOME is a profile under ~/.hercules, returns ~/.hercules."""
        native = tmp_path / ".hercules"
    ...
    def test_hercules_home_is_docker(self, tmp_path, monkeypatch):
        """When HERCULES_HOME points outside ~/.hercules (Docker), returns HERCULES_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERCULES_HOME", str(docker_home))
        assert get_default_hercules_root() == docker_home

    def test_hercules_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any HERCULES_HOME outside ~/.hercules is treated as the root."""
        custom = tmp_path / "my-hercules-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERCULES_HOME", str(custom))
        assert get_default_hercules_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (HERCULES_HOME=<root>/profiles/<name>),
    ...
    def test_no_hercules_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hercules_constants.sys, "platform", "win32")
    ...
    def test_no_hercules_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        ...
        assert get_default_hercules_root() == home / "AppData" / "Local" / "hercules"


class TestGetHerculesHome:
    """Tests for get_hercules_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When HERCULES_HOME is unset on Windows, use %LOCALAPPDATA%\\hercules."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HERCULES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(hercules_constants.sys, "platform", "win32")
        monkeypatch.setattr(hercules_constants, "_profile_fallback_warned", False)

        assert get_hercules_home() == local_appdata / "hercules"


class TestHerculesManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_hercules_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hercules_constants.sys, "platform", "win32")
        monkeypatch.setenv("HERCULES_HOME", str(home))
        monkeypatch.setattr(hercules_constants, "node_tool_runnable", lambda path: True)

        assert find_hercules_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert hercules_managed_node_tree_present() is True
    ...
    def test_with_hercules_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        ...
        env = with_hercules_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hercules_constants, "heal_hercules_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_hercules_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(hercules_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(hercules_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".hercules"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import hercules_cli._subprocess_compat as subprocess_compat
...
class TestGetHerculesDir:
    """Tests for ``get_hercules_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_hercules_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_hercules_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_hercules_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_hercules_dir("cache/audio", "audio_cache")
