...
class HerculesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors HERCULES_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `hercules portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `hercules portal` to log in, then /billing.")
            return
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from hercules_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `hercules portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
