/**
 * ChatPage — embeds `hercules --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (hercules_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "hercules.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HERCULES_SESSION_TOKEN__ &&
    !window.__HERCULES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `hercules dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__HERCULES_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Hercules TUI has enabled xterm mouse-events
    ...
    //      payloads upload to HERCULES_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to HERCULES_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[hercules-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__HERCULES_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets HERCULES_HOME pointed at the
    ...
    // forwarding them into Hercules. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="hercules-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
