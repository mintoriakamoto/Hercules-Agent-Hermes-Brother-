...
const SIDEBAR_COLLAPSED_KEY = "hercules-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="hercules-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="hercules-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateHercules = status?.can_update_hercules === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'hercules update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "hercules update";
    return (
      t.status.updateHerculesConfirmMessage ??
      `This will run 'hercules update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateHerculesConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateHercules) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateHercules,
      runningLabel: t.status.updatingHercules,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateHerculesConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateHerculesConfirmTitle ?? `${t.status.updateHercules}?`}
    />
    </>
  );
}
