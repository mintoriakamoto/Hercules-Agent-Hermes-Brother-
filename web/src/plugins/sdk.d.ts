/**
 * Hercules Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__HERCULES_PLUGIN_SDK__`` and
 * ``window.__HERCULES_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__HERCULES_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``HerculesPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@hercules/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__HERCULES_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__HERCULES_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__HERCULES_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface HerculesPluginSDK {
...
declare global {
  interface Window {
    __HERCULES_PLUGIN_SDK__?: HerculesPluginSDK;
    __HERCULES_PLUGINS__?: PluginRegistry;
  }
}
