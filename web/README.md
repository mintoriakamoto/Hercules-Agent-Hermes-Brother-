...
cd ../
python -m hercules_cli.main web --no-open
...
`hercules dashboard` on port 9119 serves the **built** bundle from `hercules_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../hercules_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `hercules-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Hercules Agent” — mixed case, no Mondwest, no `text-display` |
