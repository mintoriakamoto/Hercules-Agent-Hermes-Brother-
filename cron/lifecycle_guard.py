...
``hercules gateway restart`` (or ``launchctl kickstart ai.hercules.gateway``
or ``systemctl restart hercules-gateway``).  When the cron fires, the
...
``hercules cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``hercules gateway``, ``launchctl ... hercules-gateway``,
``systemctl ... hercules-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_HERCULES_GATEWAY=1``, and
``hercules gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `hercules gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:hercules\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a hercules-gateway label. macOS launchd
    # labels look like `ai.hercules.gateway` / `hercules-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated hercules services (e.g.
    # `launchctl unload ai.hercules.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bhercules[.\-]?gateway)"
    # Branch C: systemctl ops on a hercules-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bhercules[.\-]?gateway)"
    # Branch D: pkill / kill targeting the hercules gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<HERCULES_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.hercules/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_hercules_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `hercules gateway restart` from a shell outside "
