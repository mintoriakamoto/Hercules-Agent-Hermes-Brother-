...
as ``kind: backend``) or ``~/.hercules/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``hercules tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$HERCULES_HOME/cache/videos/``, creating parents as needed."""
    from hercules_constants import get_hercules_home

    path = get_hercules_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$HERCULES_HOME/cache/videos/``.
