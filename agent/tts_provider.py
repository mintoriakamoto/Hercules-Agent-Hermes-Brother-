...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Hercules with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.hercules/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Hercules
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``hercules tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``hercules setup`` for
