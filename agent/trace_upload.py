"""Upload a Hercules session transcript to Hugging Face as an agent trace.

Hercules stores sessions in its own SQLite store (``hercules_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``hercules trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Hercules' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "hercules-traces"
_HERCULES_VERSION = "hercules-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Hercules OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Hercules' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Hercules conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.hercules/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `hercules trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/hercules-traces`` dataset. Returns a
