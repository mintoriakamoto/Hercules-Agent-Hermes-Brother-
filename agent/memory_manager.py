...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_HERCULES_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Hercules expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``hercules_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_hercules_home()`` themselves.
        """
        if "hercules_home" not in kwargs:
            from hercules_constants import get_hercules_home
            kwargs["hercules_home"] = str(get_hercules_home())
        for provider in self._providers:
            try:
