...
Storage
-------
Bundles live in ``~/.hercules/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``HERCULES_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``hercules bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under HERCULES_HOME.

    Honors ``HERCULES_BUNDLES_DIR`` for tests; falls back to
    ``<HERCULES_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERCULES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_hercules_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `hercules bundles` CLI subcommand.
# ---------------------------------------------------------------------------
