...
# output items.  Hercules injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Hercules-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Hercules threading encrypted reasoning back across
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Hercules-side metadata key and not part
