...
#
# When a user invokes a /skill (or /bundle), Hercules expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_hercules_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``HERCULES_PLATFORM`` env var and
    ``HERCULES_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("HERCULES_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.hercules.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_hercules_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.hercules/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.hercules/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``hercules -s <skill>`` or
    a deployment's ``HERCULES_TUI_SKILLS`` env var could force-load a skill an
