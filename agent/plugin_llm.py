...
Today the only stable plugin surfaces extend an existing Hercules
...
:class:`~hercules_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Hercules' main config
...
handles every provider, fallback chain, and per-task override Hercules
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`hercules_cli.plugins.PluginContext`
