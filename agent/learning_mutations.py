...
- **skills** → the skill name (e.g. ``"debugging-hercules-desktop"``)
...
shared by the CLI (``hercules journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``hercules curator restore``); deleting a memory rewrites its
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (hercules curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: hercules curator restore {name}" if ok else message}
