...
Owns one Codex thread per Hercules session. Drives `turn/start`, consumes
...
# Hercules' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_HERCULES_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Hercules/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Hercules session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _HERCULES_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("HERCULES_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="hercules",
            client_title="Hercules Agent",
            client_version=_get_hercules_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Hercules' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Hercules' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own hercules-tools callback we
            # auto-accept — the user already approved Hercules' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Hercules approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Hercules returns 'once', 'session', 'always', or 'deny'.
...
def _get_hercules_version() -> str:
    """Best-effort Hercules version string for codex's userAgent line."""
    try:
