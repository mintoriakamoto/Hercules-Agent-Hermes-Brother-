...
projection into Hercules' display, approval bridging, transcript projection into
...
"codex_app_server"`. Hercules' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Hercules secret — gateway
        ...
        spawn_env = hercules_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("HERCULES_KANBAN_TASK"):
            kanban_db = spawn_env.get("HERCULES_KANBAN_DB")
            kanban_root = (
