...
from hercules_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``HERCULES_KANBAN_TASK`` / ``HERCULES_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("HERCULES_KANBAN_TASK") or os.getenv("HERCULES_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Hercules Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``hercules_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``HERCULES_PLATFORM`` or
            ``HERCULES_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("HERCULES_PLATFORM")
...
# which becomes the dominant cost of ``hercules`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.hercules/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``hercules`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against HERCULES_HOME, not cwd
        if not p.is_absolute():
            p = (hercules_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.hercules/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.hercules/skills/`` (including via symlinks) or configured
    ...
    # ~/.hercules/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Hercules can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    hercules = metadata.get("hercules") or {}
    if not isinstance(hercules, dict):
        hercules = {}
    return {
        "fallback_for_toolsets": hercules.get("fallback_for_toolsets", []),
        "requires_toolsets": hercules.get("requires_toolsets", []),
        "fallback_for_tools": hercules.get("fallback_for_tools", []),
        "requires_tools": hercules.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    hercules = metadata.get("hercules")
    if not isinstance(hercules, dict):
        return []
    raw = hercules.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Hercules metadata, VCS, virtualenv/dependency, cache, and skill
