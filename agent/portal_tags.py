...
Every Hercules request that hits the Nous Portal — main agent loop, auxiliary
...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=hercules-agent",
        "client=hercules-client-v<__version__>",
    ]

The version is sourced live from ``hercules_cli.__version__`` so it auto-aligns
...
``hercules_cli.__version__`` is the canonical source of truth.
"""
...
def _hercules_version() -> str:
    """Return the current Hercules release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``hercules_cli`` cannot be imported (should
...
def hercules_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=hercules-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=hercules-client-v{_hercules_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=hercules-agent", hercules_client_tag()]
