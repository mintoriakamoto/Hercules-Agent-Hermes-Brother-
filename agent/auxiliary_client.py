...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.hercules/auth.json active provider)
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://hercules-agent.nousresearch.com",
...
# in lockstep with hercules_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``hercules_cli.__version__`` is
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from hercules_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("HERCULES_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.hercules/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from hercules_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from hercules_cli.auth import _agent_key_is_usable
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: hercules auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
