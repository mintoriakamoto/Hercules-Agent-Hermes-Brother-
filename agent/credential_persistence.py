...
``auth.json``.  They intentionally have no dependency on ``hercules_cli.auth`` so
...
# Sources Hercules owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Hercules-owned OAuth/device-code state)
