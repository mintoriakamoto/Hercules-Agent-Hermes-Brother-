...
row taxonomy is inferred from the atlas shape so Hercules can render both legacy
...
# rows (1536x1872), while older Hercules/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Hercules' activity state names. They are not always identical to the
...
# Legacy Hercules/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Hercules activity names -> accepted row-name aliases in descending
