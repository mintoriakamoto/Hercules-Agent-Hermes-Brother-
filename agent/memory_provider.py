...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `hercules backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - hercules_home (str): The active HERCULES_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.hercules``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "hercules").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'hercules memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], hercules_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'hercules memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``hercules_home`` is the active HERCULES_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE HERCULES_HOME.

        ``hercules backup`` only walks HERCULES_HOME, so any provider state kept
        ...
        of the archive, and ``hercules import`` restores them to their original
