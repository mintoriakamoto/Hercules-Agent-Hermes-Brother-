...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Hercules starts and finds ``~/.openclaw/``.

    Points users at ``hercules claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``hercules claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Hercules, run "
        "`hercules claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `hercules claw cleanup` (renames it to ~/.openclaw.pre-migration — "
