...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<HERCULES_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
    ...
    home = os.environ.get("HERCULES_HOME") or os.path.join(
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<HERCULES_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        hercules_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(hercules_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Hercules -HostProfileId hercules -HostVersion 1.0.0 "
...
def hercules_lsp_session_dir() -> str:
