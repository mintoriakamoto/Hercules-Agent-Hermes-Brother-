...
`$HERCULES_HOME/config.yaml` (defaults to `~/.hercules/config.yaml` when `HERCULES_HOME` is unset)
...
Restart the Hercules session for changes to take effect. The MCP connection is
