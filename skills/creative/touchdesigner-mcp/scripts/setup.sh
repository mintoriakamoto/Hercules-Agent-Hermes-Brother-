...
HERCULES_CFG="${HERCULES_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Hercules config has twozero_td MCP entry ──
if [[ ! -f "$HERCULES_CFG" ]]; then
    echo -e " ${FAIL} Hercules config not found at ${HERCULES_CFG}"
    manual_steps+=("Create ${HERCULES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERCULES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Hercules config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Hercules config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$HERCULES_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${HERCULES_CFG} manually"); }
    manual_steps+=("Restart Hercules session to pick up config change")
fi
