...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `hercules plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hercules_home (str): Active HERCULES_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `hercules memory setup` | **Yes** |
| `save_config(values, hercules_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `hercules memory setup`:
...
Every field in `get_config_schema()` is prompted during `hercules memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$HERCULES_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
def save_config(self, values: dict, hercules_home: str) -> None:
    ...
    config_path = Path(hercules_home) / "my-provider.json"
...
the `messages` parameter; Hercules will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hercules_home` kwarg from `initialize()`, not hardcoded `~/.hercules`:

```python
# CORRECT — profile-scoped
from hercules_constants import get_hercules_home
data_dir = get_hercules_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.hercules/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `hercules my-provider status`, `hercules my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `hercules <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `hercules --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: hercules my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the hercules my-provider argparse tree.
