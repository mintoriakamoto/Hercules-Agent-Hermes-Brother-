...
description: "How Hercules detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Hercules' browser tooling:
...
(`hercules-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Hercules `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `hercules-acp`, `hercules-api-server`, and core toolsets (gated on CDP reachability)
- `hercules_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
