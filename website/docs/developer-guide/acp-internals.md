...
The ACP adapter wraps Hercules' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
hercules acp / hercules-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.hercules/.env
  -> configure stderr logging
  -> construct HerculesACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'hercules-agent[acp]==<version>' hercules-acp`, pointed at the `hercules-agent` PyPI release.
...
- `allow_once` -> Hercules `once`
- `allow_always` -> Hercules `always`
- reject options -> Hercules `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Hercules tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["hercules-acp"])
...
Instead it reuses Hercules' runtime resolver:

- `acp_adapter/auth.py`
- `hercules_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Hercules provider/credentials. It also always advertises a terminal setup auth method (`hercules-setup`, args `--setup`) so first-run registry clients can open Hercules' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.hercules/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `hercules-acp` toolset definition
- `hercules_cli/main.py` — `hercules acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `hercules-acp` script
