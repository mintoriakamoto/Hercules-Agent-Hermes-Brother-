...
├── cli.py                    # HerculesCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── hercules_state.py           # SQLite session/state database with FTS5
├── hercules_constants.py       # HERCULES_HOME, profile-aware paths
...
│
├── hercules_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `hercules` subcommands (large file)
...
│   ├── skills_config.py      # hercules skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # hercules tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # hercules gateway start/stop
│
...
```text
User input → HerculesCLI.process_input()
...
Three discovery sources: `~/.hercules/plugins/` (user), `.hercules/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `hercules plugins` or `config.yaml`.
...
Exposes Hercules as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`hercules -p <name>`) gets its own HERCULES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
