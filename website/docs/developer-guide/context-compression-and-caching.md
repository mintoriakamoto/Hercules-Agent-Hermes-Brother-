...
Configure via `hercules plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Hercules has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|hercules|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `hercules`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Hercules raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$HERCULES_HOME`
...
```bash
hercules config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
hercules config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Hercules' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Hercules
  records the resulting compaction events (compression counters, session
  events). Set `hercules` to let Hercules' compression threshold initiate
  app-server compaction, or `off` to disable Hercules-initiated automatic
  compaction entirely (codex may still compact natively).

Hercules' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Hercules' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Hercules
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Hercules can avoid; user-facing docs for `/model`, fallback providers, and
