...
This page is for adding a **built-in Hercules tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Hercules core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Hercules Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_HERCULES_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `hercules_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `hercules_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `hercules chat -q "Use the weather tool for London"`
