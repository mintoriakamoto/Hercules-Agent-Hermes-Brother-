...
If you only run one Hercules agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Hercules agents that should all
...
(`ai.hercules.gateway-<name>.plist`) or systemd user service
(`hercules-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
hercules profile create coder
hercules profile create personal-bot
hercules profile create research
...
Equivalently, in the default profile's `~/.hercules/config.yaml`:
...
You do **not** run `hercules gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `hercules gateway start` / `run` is a
...
home). `hercules status` reports the multiplexer and the profiles it serves;
`hercules status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/hercules-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: hercules-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    hercules gateway "$action"
  else
    hercules -p "$profile" gateway "$action"
  fi
}
...
```bash
hercules-gateways start      # start every configured profile
hercules-gateways stop       # stop every configured profile
hercules-gateways restart    # restart all
hercules-gateways status     # status across all
hercules-gateways list       # delegates to `hercules gateway list`
```

:::tip
The `default` profile is targeted with `hercules gateway <action>` (no `-p`),
not `hercules -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `hercules -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.hercules.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/hercules-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.hercules.gateway.plist` /
`hercules-gateway.service`.
...
```bash
# Default profile
tail -f ~/.hercules/logs/gateway.log
tail -f ~/.hercules/logs/gateway.error.log

# Named profile
tail -f ~/.hercules/profiles/<name>/logs/gateway.log
tail -f ~/.hercules/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.hercules/logs/gateway.log ~/.hercules/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
hercules logs -f                  # follow default profile
hercules -p coder logs -f         # follow one profile
hercules logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
hercules profile list             # profiles + model + gateway state
hercules-gateways status          # full status across every profile
launchctl list | grep hercules    # macOS — PIDs and labels
systemctl --user list-units 'hercules-gateway-*'   # Linux — units
```
...
```
~/.hercules/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.hercules/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
hercules config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.hercules/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=hercules --why="gateways running" \
  sleep infinity &
...
`hercules-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.hercules/.env ~/.hercules/profiles/*/.env
```

## Updating the code

`hercules update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `hercules gateway start` after a previous `hercules gateway stop`. The
...
```bash
ps -ef | grep "hercules_cli.*-p <profile>"
cat ~/.hercules/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.hercules.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.hercules.gateway-<profile>.plist

# Linux
systemctl --user restart hercules-gateway-<profile>.service
```

### Health check

```bash
hercules doctor                  # default profile
hercules -p <profile> doctor     # one profile
```
