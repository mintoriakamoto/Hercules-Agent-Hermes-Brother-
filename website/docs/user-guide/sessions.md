...
1. **SQLite database** (`~/.hercules/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Hercules stores session history so it can resume conversations, but it does not
...
Hercules explicitly injects for that turn.
...
For example, if a user sends an image and asks Hercules to make a meme from it,
Hercules may inspect that image once with vision and run an image-processing
...
`hercules sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`hercules` or `hercules chat`) |
...
hercules -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
hercules -c "my project"   # → resumes "my project #3"
```
...
hercules --resume 20250305_091523_a1b2c3d4
hercules -r 20250305_091523_a1b2c3d4

# Resume by title
hercules --resume "refactoring auth"

# Or with the chat subcommand
hercules chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `hercules sessions list`.

### Conversation Recap on Resume

When you resume a session, Hercules displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Hercules session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.hercules/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `hercules -r "<title>"` from the shell) and pick up where the platform left off.
...
Hercules automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `hercules sessions list` or `hercules sessions browse`.
...
```bash
hercules sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Hercules creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`hercules -c "my project"`), it automatically picks the most recent session in the lineage.
...
Hercules provides a full set of session management commands via `hercules sessions`:
...
# Filter by platform
hercules sessions list --source telegram

# Show more sessions
hercules sessions list --limit 50
```
...
`hercules sessions export` is one surface for every export format, selected with `--format`:
...
hercules sessions export backup.jsonl

# Export sessions from a specific platform
hercules sessions export telegram-history.jsonl --source telegram

# Export a single session
hercules sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
hercules sessions export backup.jsonl --redact
```
...
hercules sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
hercules sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
hercules sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
hercules sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `hercules-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
hercules sessions export --format trace

# One session to a local trace file
hercules sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
hercules sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.hercules/session-exports`).

```bash
# Export one session to Markdown
hercules sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
hercules sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
hercules sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
hercules sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
hercules sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
hercules sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
hercules sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
hercules sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
hercules sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
hercules sessions prune

# Custom age threshold — bare numbers are days
hercules sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
hercules sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
hercules sessions prune --newer-than 5h

# Explicit window with absolute timestamps
hercules sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
hercules sessions prune --source telegram
hercules sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
hercules sessions prune --newer-than 5h --title "smoke test"   # title substring
hercules sessions prune --older-than 30 --max-messages 3        # tiny sessions
hercules sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
hercules sessions prune --model gpt-5 --older-than 1w           # by model (substring)
hercules sessions prune --provider openrouter --older-than 60   # by billing provider
hercules sessions prune --branch feature/old-experiment         # by git branch
hercules sessions prune --user 12345678 --chat-type group       # by messaging origin
hercules sessions prune --max-tokens 500 --older-than 7         # by token usage
hercules sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
hercules sessions prune --newer-than 5h --dry-run

# Skip confirmation
hercules sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `hercules sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `hercules sessions prune` keeps the 90-day cutoff. Every
...
`hercules sessions archive` takes the same filters as `prune` but soft-hides
...
hercules sessions archive --newer-than 5h

# Archive by title substring, preview first
hercules sessions archive --title "dry run" --dry-run
hercules sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `hercules sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`hercules sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
hercules sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`hercules insights`](/reference/cli-commands#hercules-insights).
...
When Hercules cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Hercules uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Hercules in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.hercules/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.hercules/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.hercules/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.hercules/sessions/sessions.json` is the **gateway routing index** — it maps
...
`hercules sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.hercules/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `hercules sessions list`, the cause is
`state.db` not receiving them — run `hercules sessions repair` and watch for a
...
`*.jsonl` files in `~/.hercules/sessions/`. They are no longer written or
read by Hercules. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Hercules process in the same `HERCULES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.hercules/config.yaml`:
...
hercules sessions delete <session_id>
