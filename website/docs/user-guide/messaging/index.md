...
description: "Chat with Hercules from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Hercules from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Hercules](/guides/use-voice-mode-with-hercules).

:::tip
Bots need both a model provider and tool providers (TTS, web). Start with [OpenRouter](/integrations/providers#openrouter) for the model, then configure tools via the [Tool Gateway](/user-guide/features/tool-gateway).
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Hercules Gateway"]
...
For group chats, hooks, and automation flows, Hercules supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Hercules keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Hercules does not hide failures just because the text resembles a silence token.
...
```bash
hercules gateway setup        # Interactive setup for all messaging platforms
```
...
hercules gateway setup        # Configure messaging platforms interactively
hercules gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo hercules gateway install --system   # Linux only: install a boot-time system service
hercules gateway start        # Start the default service
hercules gateway stop         # Stop the default service
hercules gateway status       # Check default service status
hercules gateway status --system         # Linux only: inspect the system service explicitly
```
...
with the `session_reset` section in `~/.hercules/config.yaml`:
...
Configure per-platform overrides in `~/.hercules/gateway.json`:
...
hercules pairing approve telegram XKGH5N7P

# Other pairing commands:
hercules pairing list          # View pending + approved users
hercules pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Hercules appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.hercules/config.yaml`:
...
Off by default. When enabled, Hercules prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.hercules/config.yaml`:
...
```bash
HERCULES_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
hercules gateway install               # Install as user service
hercules gateway start                 # Start the service
hercules gateway stop                  # Stop the service
hercules gateway status                # Check status
journalctl --user -u hercules-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo hercules gateway install --system
sudo hercules gateway start --system
sudo hercules gateway status --system
journalctl -u hercules-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Hercules installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit hercules-gateway` (or `sudo systemctl edit hercules-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `hercules update`. When `hercules update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart hercules-gateway` command (it never blocks on an interactive password prompt).
...
```bash
hercules gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `hercules update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo hercules update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/hercules-gateway`:

```
hercules ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed hercules-gateway*, /usr/bin/systemctl --no-ask-password start hercules-gateway*, /usr/bin/systemctl --no-ask-password restart hercules-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Hercules will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Hercules installations on the same machine (with different `HERCULES_HOME` directories), each gets its own systemd service name. The default `~/.hercules` uses `hercules-gateway`; other installations use `hercules-gateway-<hash>`. The `hercules gateway` commands automatically target the correct service for your current `HERCULES_HOME`.
:::

### macOS (launchd)

```bash
hercules gateway install               # Install as launchd agent
hercules gateway start                 # Start the service
hercules gateway stop                  # Stop the service
hercules gateway status                # Check status
tail -f ~/.hercules/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.hercules.gateway.plist`. It includes three environment variables:
...
- **HERCULES_HOME** — scopes the gateway to your Hercules installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `hercules gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERCULES_HOME` directory gets its own launchd label. The default `~/.hercules` uses `ai.hercules.gateway`; other installations use `ai.hercules.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `hercules-cli` | Full access |
| Telegram | `hercules-telegram` | Full tools including terminal |
| Discord | `hercules-discord` | Full tools including terminal |
| WhatsApp | `hercules-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `hercules-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `hercules-slack` | Full tools including terminal |
| Google Chat | `hercules-google_chat` | Full tools including terminal |
| Signal | `hercules-signal` | Full tools including terminal |
| SMS | `hercules-sms` | Full tools including terminal |
| Email | `hercules-email` | Full tools including terminal |
| Home Assistant | `hercules-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `hercules-mattermost` | Full tools including terminal |
| Matrix | `hercules-matrix` | Full tools including terminal |
| DingTalk | `hercules-dingtalk` | Full tools including terminal |
| Feishu/Lark | `hercules-feishu` | Full tools including terminal |
| WeCom | `hercules-wecom` | Full tools including terminal |
| WeCom Callback | `hercules-wecom-callback` | Full tools including terminal |
| Weixin | `hercules-weixin` | Full tools including terminal |
| BlueBubbles | `hercules-bluebubbles` | Full tools including terminal |
| QQBot | `hercules-qqbot` | Full tools including terminal |
| Yuanbao | `hercules-yuanbao` | Full tools including terminal |
| Microsoft Teams | `hercules-teams` | Full tools including terminal |
| API Server | `hercules-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `hercules-webhook` | Full tools including terminal |
| Raft | `hercules-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.hercules/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
