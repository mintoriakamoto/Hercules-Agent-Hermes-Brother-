...
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Hercules
...
> Run `hercules gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Hercules can read (e.g.,
`~/.hercules/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a hercules restart)
...
- Principal: `hercules-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Hercules calls
...
- **App name**: whatever you want users to see ("Hercules" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/hercules-chat-events`.
...
`ADDED_TO_SPACE` event that Hercules uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Hercules

Add the Google Chat section to `~/.hercules/.env`:
...
Install the dependencies the Google Chat adapter needs (no Hercules extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Hercules is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Hercules detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Hercules session.
...
3. Download the JSON. Move it onto the host that runs Hercules.
4. Register the client with Hercules (run under the profile you want it scoped to):
...
hercules -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Hercules home (e.g.
`~/.hercules/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.hercules/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.hercules/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Hercules isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `hercules gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Hercules will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.hercules/google_chat_user_tokens/<sanitized_email>.json` (filesystem
