...
Hercules connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `cd ~/.hercules/hercules-agent && uv pip install -e ".[sms]"`
...
If you're running Hercules locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Hercules will respond via SMS.
...
Hercules validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Hercules gateway logs for Twilio API errors
