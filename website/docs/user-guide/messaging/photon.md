...
Connect Hercules to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Hercules — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Hercules runs it in a
...
hercules photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Hercules Agent` project on your account.
...
Runtime credentials are written to `~/.hercules/.env`
...
(device token, dashboard project id) lives in `~/.hercules/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Hercules
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Hercules replies with a pairing code. Approve it with:

```bash
hercules pairing approve photon <CODE>
```

Use `hercules pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.hercules/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.hercules/.env`):
...
By default Hercules responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Hercules` and
`@Hercules agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Hercules will reply.

## Status & troubleshooting

```bash
hercules photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Hercules uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run hercules photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `hercules photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `hercules photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `hercules photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Hercules sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Hercules wake words  | JSON list / comma / newline regex patterns for group mentions |
