...
Hercules connects to WhatsApp through a built-in bridge based on **Baileys**. This works by emulating a WhatsApp Web session — **not** through the official WhatsApp Business API. No Meta developer account or Business verification is required.

> Run `hercules gateway setup` and pick **WhatsApp** for a guided walk-through.
...
with third-party bridges. When this happens, Hercules will update the bridge dependency. If the
bot stops working after a WhatsApp update, pull the latest Hercules version and re-pair.
:::
...
3. Run `hercules whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.hercules/platforms/whatsapp/session`. This means:
...
Hercules supports voice on WhatsApp:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `hercules whatsapp`. |
| **Session not persisting** | Check that `~/.hercules/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `hercules whatsapp` if needed. |
| **Bridge crashes or reconnect loops** | Restart the gateway, update Hercules, and re-pair if the session was invalidated by a WhatsApp protocol change. |
| **Bot stops working after WhatsApp update** | Update Hercules to get the latest bridge version, then re-pair. |
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `hercules gateway install` to re-snapshot your current PATH into the plist, then `hercules gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.hercules/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.hercules/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.hercules/platforms/whatsapp/session`
