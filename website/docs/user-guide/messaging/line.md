...
> Run `hercules gateway setup` and pick **LINE** for a guided walk-through.
...
# devtunnel
devtunnel create hercules-line --allow-anonymous
devtunnel port create hercules-line -p 8646 --protocol https
devtunnel host hercules-line
```
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.hercules/logs/gateway.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
...
**"already in use by another profile".** The same channel access token is bound to another running Hercules profile. Stop the other gateway or use a separate channel.
