...
ntfy makes a great lightweight push channel for Hercules: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `hercules gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `hercules-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Hercules dependency.
...
Add these to `~/.hercules/.env`:

```
NTFY_TOPIC=hercules-myname-2026
NTFY_ALLOWED_USERS=hercules-myname-2026
NTFY_HOME_CHANNEL=hercules-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Hercules adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
- Or **pick a long, unguessable topic name** (`hercules-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `hercules-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `hercules-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=hercules-myname-2026' >> ~/.hercules/.env
   echo 'NTFY_ALLOWED_USERS=hercules-myname-2026' >> ~/.hercules/.env
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`hercules send` CLI](/guides/pipe-script-output):

```bash
hercules send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Hercules replies:
...
If you only want Hercules to *push* notifications to ntfy (cron summaries, alerts) and never accept messages back, set both `NTFY_TOPIC` and `NTFY_PUBLISH_TOPIC` to the same value and skip `NTFY_ALLOWED_USERS` entirely. With no allowlist, the agent never responds to inbound messages — your phone gets the pushes, but the conversation is one-way.

## Limits

- **Message size**: ntfy caps message bodies at 4096 chars. Hercules truncates with a warning when this is exceeded.
