...
Connect Hercules to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.hercules/.env`:
...
All standard Hercules commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.hercules/logs/gateway.log`
...
4. Consider enabling verbose logging: `HERCULES_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Hercules automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
HERCULES_LOG_LEVEL=debug hercules gateway
```
...
```bash
hercules chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
