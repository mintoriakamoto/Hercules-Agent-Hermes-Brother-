...
Configure which events the agent sees in `~/.hercules/config.yaml` under the Home Assistant platform's `extra` section:
...
The adapter reads credentials from `~/.hercules/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Hercules profile
...
running Hercules — `curl -H "Authorization: Bearer <token>" <url>/api/` should
