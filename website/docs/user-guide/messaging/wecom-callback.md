...
Connect Hercules to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Hercules supports two WeCom integration modes:
...
> Run `hercules gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Hercules decrypts the message, queues it for the agent
...
(Use `hercules gateway start` only after `hercules gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Hercules and the token the
...
in `~/.hercules/.env` values around `=` will also break signature checks. After
fixing, restart `hercules gateway run`.
...
Check `hercules gateway run` logs for the bound host/port. If the adapter bound to
