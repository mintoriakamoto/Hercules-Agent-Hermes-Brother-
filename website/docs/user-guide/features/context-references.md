...
Type `@` followed by a reference to inject content directly into your message. Hercules expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Hercules env: `$HERCULES_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$HERCULES_HOME/skills/.hub/`
