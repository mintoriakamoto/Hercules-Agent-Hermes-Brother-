...
Both are configured through a single backend selection. Providers are chosen via `hercules tools` or set directly in `config.yaml`.
...
| **xAI (Grok)** | `XAI_API_KEY` or `hercules auth login xai-oauth` | ✔ | — | Paid (SuperGrok or per-token) |

Brave Search, DDGS, and xAI are **search-only** — pair any of them with Firecrawl/Tavily/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let Hercules lazy-install it on first use). xAI runs Grok's server-side `web_search` tool on the Responses API — results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all model output (see the [trust-model caveat](#xai-grok) below).

**Per-capability split:** you can use different providers for search and extract independently — for example SearXNG (free) for search and Firecrawl for extract. See [Per-capability configuration](#per-capability-configuration) below.

:::tip Nous Subscribers
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, web search and extract are available through the **[Tool Gateway](tool-gateway.md)** via managed Firecrawl — no API key needed. New installs can run `hercules setup --portal` to log in and turn on all gateway tools at once; existing installs can flip just web via `hercules tools`.
:::
...
The summary keeps quotes, code blocks, and key facts in their original formatting — it's a content compressor, not a paraphraser. If summarization fails or times out, Hercules falls back to the first ~5 000 chars of raw content rather than a useless error.

### Which model does the summarizing?

The `web_extract` auxiliary task. By default (`auxiliary.web_extract.provider: "auto"`), this is your **main chat model** — same provider, same model as `hercules model`. That's fine for most setups, but on expensive reasoning models (Opus, MiniMax M2.7, etc.) every long-page extract adds meaningful cost.
...
Or pick interactively: `hercules model` → **Configure auxiliary models** → `web_extract`.
...
### Quick setup via `hercules tools`

Run `hercules tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** — just point Hercules at a running SearXNG instance.
...
To enable JSON responses for Hercules, add the following override:
...
Then select SearXNG as the search backend in `~/.hercules/config.yaml`:
...
Or set via `hercules tools` → Web Search & Extract → SearXNG.
...
With this config, Hercules uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.
...
If no backend is explicitly configured, Hercules picks the first available one based on which credentials are set:
...
Run `hercules setup` to see which web backend is detected:
...
source ~/.hercules/hercules-agent/.venv/bin/activate
...
```bash
hercules skills install official/research/searxng-search
```
