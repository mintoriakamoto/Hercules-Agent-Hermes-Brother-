...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .hercules.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Hercules Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Hercules instance and is loaded from `HERCULES_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.hercules.md** / **HERCULES.md** | Project instructions (highest priority) | Walks to git root |
| **AGENTS.md** | Project instructions, conventions, architecture | CWD at startup + subdirectories progressively |
| **CLAUDE.md** | Claude Code context files (also detected) | CWD at startup + subdirectories progressively |
| **SOUL.md** | Global personality and tone customization for this Hercules instance | `HERCULES_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.hercules.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
- `~/.hercules/SOUL.md`
- or `$HERCULES_HOME/SOUL.md` if you run Hercules with a custom home directory

Important details:

- Hercules seeds a default `SOUL.md` automatically if one does not exist yet
- Hercules loads `SOUL.md` only from `HERCULES_HOME`
- Hercules does not probe the working directory for `SOUL.md`
...
Hercules is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.hercules.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Hercules.
...
1. **Scan working directory** — checks for `.hercules.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
