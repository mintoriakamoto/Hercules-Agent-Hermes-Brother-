...
This is the same architecture top-tier coding agents use. Hercules
...
1. Hercules captures a baseline of current diagnostics for the file.
...
…). Hercules auto-detects the binary on PATH or in
`<HERCULES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Hercules at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<HERCULES_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`hercules lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Hercules installs both packages together when you
run `hercules lsp install typescript` or auto-install fires on first
use.

## CLI

```
hercules lsp status          # service state + per-server install status
hercules lsp list            # registry, optionally --installed-only
hercules lsp install <id>    # eagerly install one server
hercules lsp install-all     # try every server with a known recipe
hercules lsp restart         # tear down running clients
hercules lsp which <id>      # print resolved binary path
```

`hercules lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <HERCULES_HOME>/lsp/bin
...
When `install_strategy: auto`, Hercules installs binaries into
`<HERCULES_HOME>/lsp/bin/`. NPM packages land in
`<HERCULES_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Hercules-owned and is
...
Servers are kept alive for the life of the Hercules process. There's
...
**`hercules lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERCULES_HOME>/lsp/bin/`. Run
`hercules lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `hercules lsp status`**
...
When `hercules lsp status` shows a `Backend warnings` section, install
...
`~/.hercules/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.hercules/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `hercules lsp restart` to clear the set;
