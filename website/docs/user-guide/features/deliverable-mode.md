...
persona in `~/.hercules/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.hercules/config.yaml` (switchable per session
via `/personality`).
...
If you use Hercules' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.hercules/config.yaml` under the `mcp_servers`
