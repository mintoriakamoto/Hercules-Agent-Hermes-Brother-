...
Hercules can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.hercules/auth.json` and refreshed automatically on 401; you only log in once per machine (refresh tokens expire after ~6 months; re-run `hercules auth spotify` when they do).

Unlike Hercules' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `hercules auth spotify` walks you through it.
...
### One-shot: `hercules tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `hercules setup` / `hercules setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `hercules tools`.

Hercules drops you straight into the OAuth flow — if you don't have a Spotify app yet, it walks you through creating one inline. Once you finish, the toolset is enabled AND authenticated in one pass.
...
If no `HERCULES_SPOTIFY_CLIENT_ID` is set, Hercules walks you through the app registration inline:
...
4. Saves it to `~/.hercules/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.hercules/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
| Field | Value |
|-------|-------|
| App name | anything (e.g. `hercules-agent`) |
| App description | anything (e.g. `personal Hercules integration`) |
| Website | leave blank |
| Redirect URI | `http://127.0.0.1:43827/spotify/callback` |
| Which API/SDKs? | check **Web API** |

Agree to the terms and click **Save**. On the next page click **Settings** → copy the **Client ID** and paste it into the Hercules prompt. That's the only value Hercules needs — PKCE doesn't use a client secret.

### Running over SSH / in a headless environment

If `SSH_CLIENT` or `SSH_TTY` is set, Hercules skips the automatic browser open during both the wizard and the OAuth step. Copy the dashboard URL and the authorization URL Hercules prints, open them in a browser on your local machine, and proceed normally — the local HTTP listener still runs on the remote host on port `43827`. Your laptop's browser can't reach the remote loopback without an SSH local-forward:
...
```bash
hercules auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Hercules restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `hercules auth logout spotify`.
...
If Home Assistant manages speakers that already support Spotify Connect (for example Sonos, Echo, Nest, or other Connect-capable speakers), they appear in `spotify_devices list` automatically whenever Spotify can see them. Hercules does not need a Home Assistant ↔ Spotify bridge for this path — Spotify handles the device routing natively.

Ask Hercules to transfer playback by the speaker's display name (for example, “transfer Spotify to the kitchen speaker”), or call `spotify_devices list` and pass the exact `device_id` to `spotify_devices transfer` when scripting. If the speaker is missing, open the Spotify app or the speaker's Spotify integration once so Spotify registers it as an active Connect target.
...
Because Spotify tools are regular Hercules tools, a cron job running in a Hercules session can trigger playback on any schedule. No new code needed.
...
1. Cron spins up a headless Hercules session.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `hercules tools` for the cron session to see the Spotify tools.
...
```bash
hercules auth logout spotify
```

Removes tokens from `~/.hercules/auth.json`. To also clear the app config, delete `HERCULES_SPOTIFY_CLIENT_ID` (and `HERCULES_SPOTIFY_REDIRECT_URI` if you set it) from `~/.hercules/.env`, or run the wizard again.
...
**`204 No Content` on `get_currently_playing`** — nothing is currently playing on any device. This is Spotify's normal response, not an error; Hercules surfaces it as an explanatory empty result (`is_playing: false`).

**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Hercules is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `HERCULES_SPOTIFY_REDIRECT_URI` in `~/.hercules/.env` to whatever you registered.

**`429 Too Many Requests`** — Spotify's rate limit. Hercules returns a friendly error; wait a minute and retry. If this persists, you're probably running a tight loop in a script — Spotify's quota resets roughly every 30 seconds.

**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `hercules auth spotify` again.

**Wizard doesn't open the browser** — If you're over SSH or in a container without a display, Hercules detects it and skips the auto-open. Copy the dashboard URL it prints and open it manually.

## Advanced: custom scopes

By default Hercules requests the scopes needed for every shipped tool. Override if you want to restrict access:

```bash
hercules auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
```bash
hercules auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.hercules/.env`:

```
HERCULES_SPOTIFY_CLIENT_ID=<your_id>
HERCULES_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| File | Contents |
|------|----------|
| `~/.hercules/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.hercules/.env` | `HERCULES_SPOTIFY_CLIENT_ID`, optional `HERCULES_SPOTIFY_REDIRECT_URI` |
