...
| CLI | Saved to `~/.hercules/audio_cache/` | MP3 |
...
tts:
  ...
  piper:
    ...
    # voices_dir: ''                            # default: ~/.hercules/cache/piper-voices/
...
If the file contains `{transcript}` or `{{ transcript }}`, Hercules replaces that placeholder with the live TTS text. Otherwise, Hercules appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
tts:
  ...
  gemini:
    voice: Algieba
    persona_prompt_file: ~/.hercules/tts/butler-voice.md
```

### Gemini Audio Tags

Gemini 3.1 Flash TTS supports freeform square-bracket audio tags such as `[whispers]`, `[excitedly]`, `[very slow]`, `[laughs]`, and other expressive delivery notes. Enable `tts.gemini.audio_tags` to have Hercules run a hidden rewrite pass before Gemini TTS. The rewrite inserts inline tags into the TTS script only; the visible chat reply stays unchanged.
...
Each provider has a documented per-request input-character cap. Hercules truncates text before calling the provider so requests never fail with a length error:
...
**Install via `hercules tools`** → Voice & TTS → Piper — Hercules runs `pip install piper-tts` for you. Or install manually: `pip install piper-tts`.
...
On the first TTS call for a voice that isn't cached locally, Hercules runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.hercules/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR — see the [STT section below](#example-doubao--volcengine-asr) for Hercules integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).

#### Placeholders

Your command template can reference these placeholders. Hercules substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces and other shell-sensitive characters are safe.

| Placeholder      | Meaning                                              |
|------------------|------------------------------------------------------|
| `{input_path}`   | Path to the temp UTF-8 text file Hercules wrote        |
...
| `output_format`    | `mp3`   | One of `mp3` / `wav` / `ogg` / `flac`. Auto-inferred from the output extension if Hercules picks a path.      |
| `voice_compatible` | `false` | When `true`, Hercules converts MP3/WAV output to Opus/OGG via ffmpeg so Telegram renders a voice bubble.      |
...
Command-type providers run whatever shell command you configure, with your user's permissions. Hercules quotes placeholder values and enforces the configured timeout, but the command template itself is trusted local input — treat it the same way you would a shell script on your PATH.
...
| A voice-listing API used by `hercules setup` | **Plugin** (override `list_voices()`) |
...
Drop this in `~/.hercules/plugins/my-tts/`:
...
Enable it (`hercules plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `hercules tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `hercules tools` / `hercules setup`. Without this, the plugin still works but its row in the picker is minimal.
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, Hercules can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `HERCULES_LOCAL_STT_COMMAND`.
:::
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `cd ~/.hercules/hercules-agent && uv pip install -e ".[mistral]"`.
...
**Custom local CLI fallback** — Set `HERCULES_LOCAL_STT_COMMAND` if you want Hercules to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
export VOLCENGINE_APP_ID="your-app-id"
export VOLCENGINE_ACCESS_TOKEN="your-access-token"
export HERCULES_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
Hercules writes the incoming voice message to `{input_path}`, runs the command, and reads the `.txt` file produced under `{output_dir}`. Language is auto-detected by the Volcengine bigmodel endpoint.

### Fallback Behavior

If your configured provider isn't available, Hercules automatically falls back:
- **Local faster-whisper unavailable** → Tries a local `whisper` CLI or `HERCULES_LOCAL_STT_COMMAND` before cloud providers
...
This complements the legacy `HERCULES_LOCAL_STT_COMMAND` escape hatch — that env var still works untouched via the built-in `local_command` path. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.

#### STT placeholders

Your command template can reference these placeholders. Hercules substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces are safe.
...
1. If `{output_path}` exists and is non-empty → Hercules reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout → Hercules uses that.
...
For `format: json` / `srt` / `vtt`, Hercules returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner — either configure `format: txt`, or post-process JSON downstream.
...
The shell command runs under the same user as Hercules with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `HERCULES_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
...
| Only the legacy single-command escape hatch is wanted        | `HERCULES_LOCAL_STT_COMMAND` env var (preserved for back-compat)  |
...
Drop this in `~/.hercules/plugins/my-stt/`:
...
Enable it (`hercules plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `hercules tools` / `hercules setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
