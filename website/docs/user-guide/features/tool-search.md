...
**Tool Search** is Hercules' opt-in progressive-disclosure layer for that
...
:::info Built-in Hercules tools never defer
The tools that make up Hercules' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_HERCULES_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Hercules **unwraps the bridge** and
...
- **No JS sandbox.** Hercules uses the simpler "structured tools" mode
