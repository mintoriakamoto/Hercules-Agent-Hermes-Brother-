...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Hercules automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [OpenRouter](/integrations/providers#openrouter) key already covers 300+ models, so most users only need a pool to spread load across multiple keys.
:::
...
If you already have an API key set in `.env`, Hercules auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
hercules auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
hercules auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
hercules auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  hercules_pkce          oauth   hercules_pkce ←
...
Run `hercules auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `hercules auth` | Interactive pool management wizard |
| `hercules auth list` | Show all pools and credentials |
| `hercules auth list <provider>` | Show a specific provider's pool |
| `hercules auth add <provider>` | Add a credential (prompts for type and key) |
| `hercules auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `hercules auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `hercules auth remove <provider> <index>` | Remove credential by 1-based index |
| `hercules auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `hercules auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `hercules model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via hercules model:
hercules auth list
# Shows:
...
hercules auth add Together.ai --api-key sk-together-second-key
```
...
Hercules automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Hercules PKCE OAuth | `~/.hercules/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `hercules auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `hercules auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Hercules can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Hercules-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`hercules_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`hercules_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.hercules/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Hercules' credential store, so its token remains persistable.
