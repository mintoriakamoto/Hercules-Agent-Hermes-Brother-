...
- `SOUL.md` — a durable persona file that lives in `HERCULES_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Hercules is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Hercules now seeds a default `SOUL.md` automatically in:

```text
~/.hercules/SOUL.md
```

More precisely, it uses the current instance's `HERCULES_HOME`, so if you run Hercules with a custom home directory, it will use:

```text
$HERCULES_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Hercules creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Hercules loads `SOUL.md` only from `HERCULES_HOME`
- Hercules does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Hercules falls back to a built-in default identity
...
If Hercules loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERCULES_HOME`, the personality belongs to the Hercules instance itself.

That also makes it easier to teach users:
- "Edit `~/.hercules/SOUL.md` to change Hercules' default personality."
...
- how Hercules should handle uncertainty, disagreement, or ambiguity
...
## What Hercules injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Hercules falls back to a built-in default identity ("You are Hercules Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Hercules ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Hercules, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Hercules its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.hercules/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.hercules/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-hercules)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Hercules speaks
- `display.skin` and `/skin` affect how Hercules looks in the terminal
