...
If the file is empty, whitespace-only, or cannot be read, Hercules falls back to a built-in default identity ("You are Hercules Agent, an intelligent AI assistant..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
