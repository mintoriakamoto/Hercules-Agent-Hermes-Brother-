...
Hercules Agent can drive your desktop — clicking, typing, scrolling,
...
on macOS/Linux, `install.ps1` on Windows. Use `hercules computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `hercules tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Hercules app). `hercules computer-use doctor` will tell you which permission is missing. |
...
or add `computer_use` to your enabled toolsets in `~/.hercules/config.yaml`.

## `hercules computer-use doctor` — your first triage stop

`hercules computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ hercules computer-use doctor
...
agent is acting here." Each Hercules run declares its own cua-driver
**session id** (something like `hercules-3a7b9c14d2e8`); the cursor's
...
Hercules intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Hercules-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Hercules skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Hercules
...
Hercules applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.hercules/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Hercules applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Hercules agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Hercules agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Hercules warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Hercules from inside the RDP / console
...
```
HERCULES_CUA_DRIVER_CMD=/path/to/your/cua-driver
```
...
upstream. **Hercules disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Hercules sets
...
When it's on, `hercules computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Hercules at a binary you built from source instead
of the published release. Hercules resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`HERCULES_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Hercules testing (see notes).
...
### Option B — point Hercules straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`HERCULES_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
HERCULES_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
HERCULES_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Hercules is using your build

- `hercules computer-use status` prints the resolved binary path and
  version.
- `hercules computer-use doctor` confirms the binary is reachable and
...
- **Hercules spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Hercules
  change needed either way.
- **Local builds skip the version check.** Hercules warns when the
...
**First action when anything's off: run `hercules computer-use doctor`.**
...
Run `hercules computer-use install` to fetch the cua-driver binary, or
run `hercules tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `hercules computer-use doctor` will
...
- **Hercules-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Hercules `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Hercules (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Hercules among them)
