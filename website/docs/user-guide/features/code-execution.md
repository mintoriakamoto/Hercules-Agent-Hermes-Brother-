...
The `execute_code` tool lets the agent write Python scripts that call Hercules tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Hercules over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from hercules_tools import ...`
2. Hercules generates a `hercules_tools.py` stub module with RPC functions
3. Hercules opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Hercules
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from hercules_tools import web_search, web_extract
...
```python
from hercules_tools import search_files, read_file
import json
...
```python
from hercules_tools import search_files, read_file, patch
...
```python
from hercules_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.hercules/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Hercules's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Hercules's own python) |
...
### `HERCULES_*` variables in the child

The child process receives only a small, fixed set of operational `HERCULES_*`
...
(plus `HERCULES_RPC_DIR` / `HERCULES_RPC_SOCKET` / `TZ` / `HOME`, which Hercules
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `HERCULES_`
through to the child. That broad prefix was removed for security hardening: it
could leak `HERCULES_*`-named configuration that doesn't match a secret substring
(for example `HERCULES_BASE_URL`, `HERCULES_KANBAN_DB`, or a `HERCULES_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `HERCULES_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Hercules-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `HERCULES_*`
variables, Hercules emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`hercules logs --level
DEBUG`, or check `~/.hercules/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted HERCULES_* var(s)` if a script behaves
as though a `HERCULES_*` variable is missing.

Hercules always writes the script and the auto-generated `hercules_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Hercules tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
