...
- **CLI — `hercules dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `hercules setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERCULES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hercules/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
From a logged-in Hercules install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `hercules dashboard register` uses your existing Nous login to provision an OAuth client and writes `HERCULES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hercules/.env` for you:

```bash
hercules setup            # if you're not already logged into Nous Portal
hercules dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HERCULES_DASHBOARD_OAUTH_CLIENT_ID to ~/.hercules/.env
```
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `hercules_session_at` cookie.
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`hercules dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
