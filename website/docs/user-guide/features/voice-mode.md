...
If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Hercules](/guides/use-voice-mode-with-hercules).

## Prerequisites

Before using voice features, make sure you have:

1. **Hercules Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `hercules model` or set your preferred provider credentials in `~/.hercules/.env`
3. **A working base setup** — run `hercules` to verify the agent responds to text before enabling voice

:::tip
The `~/.hercules/` directory and default `config.yaml` are created automatically the first time you run `hercules`. You only need to create `~/.hercules/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `hercules setup --portal` wires both up at once.
:::
...
cd ~/.hercules/hercules-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.hercules/hercules-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.hercules/hercules-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.hercules/hercules-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`hercules chat`) and the **TUI** (`hercules --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.hercules/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
hercules                # Start the interactive CLI
```
...
1. Start the CLI with `hercules` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.hercules/config.yaml` (default: `ctrl+b`).
:::
...
```bash
hercules gateway        # Start the gateway (connects to configured platforms)
hercules gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@herculesbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.hercules/.env`:
```bash
...
```bash
hercules gateway        # Start with existing configuration
```
...
If you are running Hercules inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.hercules/.env`
...
- Review gateway logs: `tail -f ~/.hercules/logs/gateway.log`
