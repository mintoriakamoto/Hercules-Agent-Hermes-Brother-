...
description: Paste images from your clipboard into the Hercules CLI for multimodal vision analysis.
...
:::tip
OpenRouter exposes vision-capable models (Claude, GPT-5, Gemini) in the same catalog — no extra credentials beyond your API key. See [OpenRouter](/integrations/providers#openrouter).
:::
...
Images are saved to `~/.hercules/images/` as PNG files with timestamped filenames.
...
Type `/paste` and press Enter. Hercules checks your clipboard for an image and attaches it. This is the safest option when your terminal rewrites `Cmd+V`/`Ctrl+V`, or when you copied only an image and there is no bracketed-paste text payload to inspect.

### Ctrl+V / Cmd+V

Hercules now treats paste as a layered flow:
...
If you run the TUI inside a local VS Code-family integrated terminal on macOS, Hercules can install the recommended `workbench.action.terminal.sendSequence` bindings for better multiline and undo/redo parity:
...
**No setup required.** Hercules uses `osascript` (built into macOS) to read the clipboard. For faster performance, optionally install `pngpaste`:
...
**No extra setup required.** Hercules detects WSL2 automatically (via `/proc/version`) and uses `powershell.exe` to access the Windows clipboard through .NET's `System.Windows.Forms.Clipboard`. This is built into WSL2's Windows interop — `powershell.exe` is available by default.

The clipboard data is transferred as base64-encoded PNG over stdout, so no file path conversion or temp files are needed.

:::info WSLg Note
If you're running WSLg (WSL2 with GUI support), Hercules tries the PowerShell path first, then falls back to `wl-paste`. WSLg's clipboard bridge only supports BMP format for images — Hercules auto-converts BMP to PNG using Pillow (if installed) or ImageMagick's `convert` command.
:::
...
4. **Use a messaging platform** — Send images to Hercules via Telegram, Discord, Slack, or WhatsApp. These platforms handle image upload natively and are not affected by clipboard/terminal limitations.
...
This is why Hercules uses a separate clipboard check — instead of receiving image data through the terminal paste event, it calls OS-level tools (`osascript`, `powershell.exe`, `xclip`, `wl-paste`) directly via subprocess to read the clipboard independently.
...
When a user attaches an image — from the CLI clipboard, the gateway (Telegram/Discord photo), or any other entry point — Hercules routes it based on whether your current model actually supports vision:
