...
title: "Git Worktrees"
description: "Run multiple Hercules agents safely on the same repository using git worktrees and isolated checkouts"
---
...
This page shows how to combine worktrees with Hercules so each session has a clean, isolated working directory.

## Why Use Worktrees with Hercules?

Hercules treats the **current working directory** as the project root:

- CLI: the directory where you run `hercules` or `hercules chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.hercules/config.yaml`
...
git worktree add ../repo-feature feature/hercules-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/hercules-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Hercules there:

```bash
cd ../repo-feature

# Start Hercules in the worktree
hercules
```
...
git worktree add ../repo-experiment-a feature/hercules-a
git worktree add ../repo-experiment-b feature/hercules-b
```
...
Each Hercules process:

- Works on its own branch (`feature/hercules-a` vs `feature/hercules-b`).
...
- Hercules checkpoint data under `~/.hercules/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Hercules experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/hercules-checkpoints-docs`, `feature/hercules-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Hercules from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `hercules -w` (Automatic Worktree Mode)

Hercules has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `hercules/hercules-<hash>`).
...
```bash
hercules -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `hercules -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Hercules session its own clean checkout.
