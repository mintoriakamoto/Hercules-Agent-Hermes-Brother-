...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Hercules or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Hercules. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hercules/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hercules/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Hercules integration, either:

- configure the server in `~/.hercules/config.yaml` using the `native-mcp` skill, or
...
### Hercules cannot see the deployed server

The server-building part may be correct while the Hercules config is not. Load the `native-mcp` skill and configure the server in `~/.hercules/config.yaml`, then restart Hercules.
