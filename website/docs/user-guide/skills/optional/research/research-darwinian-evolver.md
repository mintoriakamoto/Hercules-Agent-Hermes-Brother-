...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/research/darwinian-evolver` |
...
import upstream classes into Hercules itself.
...
```bash
mkdir -p ~/.hercules/cache/darwinian-evolver && cd ~/.hercules/cache/darwinian-evolver
...
```bash
cd ~/.hercules/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hercules/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hercules/skills/research/darwinian-evolver
DE_DIR=~/.hercules/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hercules core.
   Custom driver scripts under `~/.hercules/skills/...` are user-side and fine.
