...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${HERCULES_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HERCULES_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HERCULES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client hercules
```
...
```bash
PUBLISH="${HERCULES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client hercules
```
...
```bash
DRIVE="${HERCULES_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `hercules`)    |
