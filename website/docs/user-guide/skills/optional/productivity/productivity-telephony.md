---
title: "Telephony — Give Hercules phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Hercules phone capabilities without core tool changes"
---
...
Give Hercules phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hercules skills install official/productivity/telephony` |
...
This optional skill gives Hercules practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${HERCULES_HOME:-~/.hercules}/.env`
...
It does **not** turn Hercules into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Hercules memory
...
### 1) "I want Hercules to own a real phone number"
Use **Twilio**.
...
- pairs well with Hercules `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${HERCULES_HOME:-~/.hercules}/.env`
...
```bash
SCRIPT="$(find ~/.hercules/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
hercules skills search telephony
hercules skills install official/productivity/telephony
```
...
Then save credentials into Hercules:
...
3. Buy it and save it into `${HERCULES_HOME:-~/.hercules}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Hercules calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Hercules's existing `text_to_speech` support.

Use this when:
- you want the call to use Hercules's configured TTS voice rather than Twilio `<Say>`
...
Recommended Hercules TTS -> Twilio Play workflow:

1. Generate the audio with Hercules `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Hercules to create the message audio with `text_to_speech`
...
Important note:
- Hercules TTS is great for prerecorded outbound messages
...
- Hercules STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Hercules memory.
...
- Do not store arbitrary third-party phone numbers in Hercules memory.
...
3. persist that number to `${HERCULES_HOME:-~/.hercules}/.env`
