...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/productivity/siyuan` |
...
3. Store it in `${HERCULES_HOME:-~/.hercules}/.env`:
   ```
...
```yaml
# In ~/.hercules/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
