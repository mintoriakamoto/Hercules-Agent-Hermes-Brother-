...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/productivity/shopify` |
...
4. Save to `${HERCULES_HOME:-~/.hercules}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Hercules","productType":"Apparel","tags":["test"]}}'
```
