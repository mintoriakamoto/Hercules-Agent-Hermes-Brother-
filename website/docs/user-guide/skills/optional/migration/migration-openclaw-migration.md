...
Migrate a user's OpenClaw customization footprint into Hercules Agent. Imports Hercules-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hercules skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Hercules Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Hercules`, `Memory`, `Persona`, `Import` |
| Related skills | [`hercules-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hercules-agent) |
...
```bash
hercules claw migrate              # Full interactive migration
...
hercules claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hercules setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hercules.py` to:

- import `SOUL.md` into the Hercules home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Hercules memory entries
- merge OpenClaw command approval patterns into Hercules `command_allowlist`
- migrate Hercules-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Hercules working-directory configuration
- copy OpenClaw skills into `~/.hercules/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hercules workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hercules/tts/`
- archive non-secret docs that do not have a direct Hercules destination
...
- `scripts/openclaw_to_hercules.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py`

Do not guess a shorter path like `~/.hercules/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hercules/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Hercules-compatible secrets, currently:
...
Hercules CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Hercules. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Hercules workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Hercules skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.hercules/skills/migration/openclaw-migration/scripts/openclaw_to_hercules.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Hercules unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Hercules targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Hercules destination. Unsupported auth blobs must still be reported as skipped.
...
- Hercules persona state imported
- Hercules memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.hercules/skills/openclaw-imports/`
