...
description: "Configure and use Honcho memory with Hercules -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Hercules -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hercules skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Hercules

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Hercules profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hercules#running-honcho-locally-with-hercules

### Verify

```bash
hercules honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Hercules profile's AI peer
...
Honcho models conversations as interactions between **peers**. Hercules creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Hercules instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Hercules run |
| `global` | Single session across all directories |

Manual override: `hercules honcho map my-project-name`
...
Each Hercules profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block hercules.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `hercules.coder` host block in `honcho.json`
...
```bash
hercules honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Hercules when Honcho memory is active.
...
Config file: `$HERCULES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `hercules honcho setup`. Ensure `memory.provider: honcho` is in `~/.hercules/config.yaml`.

### Memory not persisting across sessions
Check `hercules honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hercules profile create <name> --clone`. For existing profiles: `hercules honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hercules honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hercules honcho status` | Show resolved config, connection test, peer info for active profile |
| `hercules honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hercules honcho disable` | Disable Honcho for the active profile |
| `hercules honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hercules honcho peers` | Show peer identities across all profiles |
| `hercules honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hercules honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hercules honcho sessions` | List known directory-to-session-name mappings |
| `hercules honcho map <name>` | Map current working directory to a Honcho session name |
| `hercules honcho identity` | Seed AI peer identity or show both peer representations |
| `hercules honcho sync` | Create host blocks for all Hercules profiles that don't have one yet |
| `hercules honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hercules + Honcho |
| `hercules memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hercules memory status` | Show active memory provider and config |
| `hercules memory off` | Disable external memory provider |
