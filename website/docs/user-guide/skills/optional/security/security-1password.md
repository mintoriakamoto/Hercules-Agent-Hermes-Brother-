...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/security/1password` |
...
- `tmux` available for stable authenticated sessions during Hercules terminal calls (desktop app flow only)
...
### Service Account (recommended for Hercules)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HERCULES_HOME:-~/.hercules}/.env` (the skill will prompt for this on first load).
...
## Hercules Execution Pattern (desktop app flow)

Hercules terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hercules-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hercules-op.sock"
