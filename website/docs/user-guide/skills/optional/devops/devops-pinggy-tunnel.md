...
| | |
|---|---|
| Source | Optional — install with `hercules skills install official/devops/pinggy-tunnel` |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Hercules' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
