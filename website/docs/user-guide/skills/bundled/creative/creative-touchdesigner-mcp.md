...
| Related skills | `native-mcp`, [`ascii-video`](/docs/user-guide/skills/bundled/creative/creative-ascii-video), [`manim-video`](/docs/user-guide/skills/bundled/creative/creative-manim-video), `hercules-video` |
...
```bash
bash "${HERCULES_HOME:-$HOME/.hercules}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Hercules config (if missing)
...
3. **Restart Hercules session** to pick up the new MCP server
