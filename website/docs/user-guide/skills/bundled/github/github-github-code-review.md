...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _hercules_env="${HERCULES_HOME:-$HOME/.hercules}/.env"; [ -f "$_hercules_env" ] && grep -q "^GITHUB_TOKEN=" "$_hercules_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hercules_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${HERCULES_HOME:-$HOME/.hercules}/skills/github/github-auth/scripts/gh-env.sh"
