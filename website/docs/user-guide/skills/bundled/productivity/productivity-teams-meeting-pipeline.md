...
description: "Operate the Teams meeting summary pipeline via Hercules CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Hercules CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `hercules teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${HERCULES_HOME:-~/.hercules}/.env`:
...
```bash
hercules teams-pipeline validate              # config snapshot — run first after any change
hercules teams-pipeline token-health          # Graph token status
hercules teams-pipeline token-health --force-refresh   # force a fresh token acquisition
hercules teams-pipeline list                  # recent meeting jobs
hercules teams-pipeline list --status failed  # only failed jobs
hercules teams-pipeline show <job-id>         # full detail of one job
hercules teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
hercules teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
hercules teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
hercules teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
hercules teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

hercules teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
hercules teams-pipeline delete-subscription <sub-id>
hercules teams-pipeline maintain-subscriptions            # renew near-expiry ones
hercules teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `hercules teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `hercules cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
