...
`~/.hercules/config.yaml`, `~/.hercules/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/hercules`:

```text
/etc/hercules/
├── config.yaml     # managed config layer (wins over ~/.hercules/config.yaml)
└── .env            # managed env layer (wins over ~/.hercules/.env + shell)
```
...
The location can be relocated with the `HERCULES_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `HERCULES_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Hercules.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export HERCULES_MANAGED_DIR=/opt/org/hercules-policy
```

:::warning
A user who can set `HERCULES_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `hercules doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/hercules/config.yaml` (managed) | `/etc/hercules/.env` (managed) |
| 2 | `~/.hercules/config.yaml` (user) | `~/.hercules/.env` (user) |
...
```bash
hercules config        # shows a header naming the managed source + the pinned keys
hercules doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Hercules refuses and names the source:

```bash
$ hercules config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hercules/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `hercules config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/hercules

# Pin some config values for every user on this machine
sudo tee /etc/hercules/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/hercules/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/hercules
sudo chmod 0644 /etc/hercules/config.yaml /etc/hercules/.env
```

Changes take effect on the next Hercules start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`hercules doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Hercules as `root`), managed scope is advisory.
