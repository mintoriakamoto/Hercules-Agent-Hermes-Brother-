...
Hercules uses two kinds of model slots:
...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `hercules setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `hercules portal info`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `hercules setup` or `hercules model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `hercules model` (or click **Change** in the dashboard) and Hercules will write the dict form for you.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models Hercules recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Hercules writes it to `~/.hercules/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.

### Mid-session switches and context warnings

When you switch models **inside an active session** (Herm TUI model picker, `hercules` CLI, or `/model` on Telegram/Discord), Hercules estimates whether your **next message** will run **preflight context compression** against the new model's window. If the session is already near or above that model's compression threshold (see [Context Compression](./configuration.md#context-compression)), the switch reply includes a warning — the same `warning_message` path used for expensive-model notices. The switch still applies immediately; compression runs on the **first user message after the switch**, before the model answers.
...
Every auxiliary task defaults to `auto` — meaning Hercules tries your main model for that job too. If that route is unavailable or hits a capacity-style failure, `auto` follows any task-specific `auxiliary.<task>.fallback_chain`, then the main `fallback_providers` / `fallback_model` chain, then Hercules' built-in auxiliary discovery chain. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `hercules skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`hercules kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`hercules profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
When you save via the dashboard, Hercules writes to `~/.hercules/config.yaml`:
...
`provider: auto` with `model: ''` tells Hercules to use the main model for that task, while still honoring fallback policy if the main route cannot serve the auxiliary call.
...
- **CLI** (`hercules chat`): next `hercules chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`hercules gateway restart`) if you want to force all sessions to pick up the change.
...
Hercules lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `hercules setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Hercules switched providers on me
...
Inside any `hercules chat` session:
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `hercules config set` only writes scalar values, but it can't carry a custom `base_url`:
...
Both paths feed the same loader (`hercules_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
### `hercules model` subcommand

```bash
hercules model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`hercules model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.hercules/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `hercules config show | grep '^model\.'` and `hercules status`.

### Direct config edit

Edit `~/.hercules/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-Hercules-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Hercules-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Hercules-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__HERCULES_SESSION_TOKEN__`) if you're scripting against a running dashboard.
