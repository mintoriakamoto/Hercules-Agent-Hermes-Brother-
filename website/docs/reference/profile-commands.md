...
This page covers all commands related to [Hercules profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `hercules profile` without a subcommand shows help.
...
## `hercules profile use`

```bash
hercules profile use <name>
```

Sets `<name>` as the active profile. All subsequent `hercules` commands (without `-p`) will use this profile.
...
hercules profile use default
```

## `hercules profile create`

```bash
hercules profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `hercules profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `hercules update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.hercules`), use `hercules skills opt-out` / `hercules skills opt-in`. |
...
hercules profile create work2 --clone-from work

# Clone everything from a specific profile
hercules profile create work2-backup --clone-from work --clone-all
```

## `hercules profile describe`

```bash
hercules profile describe [<name>] [options]
```
...
hercules profile describe researcher

# Set it explicitly
hercules profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
hercules profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
hercules profile describe --all --auto
```

## `hercules profile delete`

```bash
hercules profile delete <name> [options]
```
...
```bash
hercules profile delete mybot
hercules profile delete mybot --yes
```
...
## `hercules profile show`

```bash
hercules profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Hercules home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ hercules profile show work
Profile: work
Path:    ~/.hercules/profiles/work
...
## `hercules profile alias`

```bash
hercules profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Hercules installation.
...
```bash
hercules profile alias work
# Creates/updates ~/.local/bin/work

hercules profile alias work --name mywork
# Creates ~/.local/bin/mywork

hercules profile alias work --remove
# Removes the wrapper script
```

## `hercules profile rename`

```bash
hercules profile rename <old-name> <new-name>
```
...
```bash
hercules profile rename mybot assistant
# ~/.hercules/profiles/mybot → ~/.hercules/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `hercules profile export`

```bash
hercules profile export <name> [options]
```
...
```bash
hercules profile export work
# Creates work.tar.gz in the current directory

hercules profile export work -o ./work-2026-03-29.tar.gz
```

## `hercules profile import`

```bash
hercules profile import <archive> [options]
```
...
```bash
hercules profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

hercules profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`hercules profile export` / `import` are still the right commands for
...
### `hercules profile install`

```bash
hercules profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `hercules -p telemetry`). |
...
hercules profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
hercules profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
hercules profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
hercules profile install ./telemetry/
```

### `hercules profile update`

```bash
hercules profile update <name> [--force-config] [--yes]
```
...
### `hercules profile info`

```bash
hercules profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Hercules version, author, env var requirements, the source URL/path, and
...
`hercules profile list` also shows the distribution name and version in a
`Distribution` column, and `hercules profile show <name>` / `delete <name>`
...
hercules profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
hercules profile install https://github.com/your-org/internal-assistant.git
```
...
`hercules_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Hercules version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Hercules can clone from.
3. Tell recipients to run `hercules profile install <your-repo-url>`.
...
## `hercules -p` / `hercules --profile`

```bash
hercules -p <name> <command> [options]
hercules --profile <name> <command> [options]
```

Global flag to run any Hercules command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
hercules -p work chat -q "Check the server status"
hercules --profile dev gateway start
hercules -p personal skills list
hercules -p work config edit
```

## `hercules completion`

```bash
hercules completion <shell>
```
...
- `hercules profile <TAB>` — subcommands (list, use, create, etc.)
- `hercules profile use <TAB>` — profile names
- `hercules -p <TAB>` — profile names
