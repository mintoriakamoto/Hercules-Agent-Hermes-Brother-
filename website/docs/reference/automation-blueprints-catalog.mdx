...
of fields, and Hercules schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Hercules asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.hercules.blueprint` block in its
