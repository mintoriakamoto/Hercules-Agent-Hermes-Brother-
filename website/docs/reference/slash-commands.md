...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/mintoriakamoto/Hercules-Agent-Hermes-Brother-/issues/30768) for native Windows PowerShell) or when scripting against the CLI.
