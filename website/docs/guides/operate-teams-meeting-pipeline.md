...
```bash
hercules teams-pipeline validate
```
...
```bash
hercules teams-pipeline token-health
hercules teams-pipeline token-health --force-refresh
```
...
```bash
hercules teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
hercules teams-pipeline maintain-subscriptions
hercules teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Hercules cron (recommended if you already run the Hercules gateway)

Hercules ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hercules/scripts/`. First create the script:

```bash
mkdir -p ~/.hercules/scripts
cat > ~/.hercules/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec hercules teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.hercules/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
hercules cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/hercules-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Hercules Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=hercules
EnvironmentFile=/etc/hercules/env
ExecStart=/usr/local/bin/hercules teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/hercules-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Hercules Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now hercules-teams-pipeline-maintain.timer
systemctl list-timers hercules-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/hercules teams-pipeline maintain-subscriptions >> /var/log/hercules/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.hercules/.env` at the top of a wrapper script that crontab calls.
...
```bash
hercules teams-pipeline subscriptions   # should show expirationDateTime advanced
hercules teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
hercules teams-pipeline list
hercules teams-pipeline list --status failed
hercules teams-pipeline show <job-id>
```

### Replay a stored job

```bash
hercules teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
hercules teams-pipeline fetch --meeting-id <meeting-id>
hercules teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `hercules teams-pipeline maintain-subscriptions --dry-run`
- inspect `hercules teams-pipeline list --status failed`
...
- run `hercules teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `hercules teams-pipeline run`
...
- [ ] `hercules teams-pipeline validate` returns an OK snapshot
- [ ] `hercules teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Hercules cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
