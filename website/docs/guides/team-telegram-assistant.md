...
- **Hercules Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.hercules/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Hercules itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Hercules Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_hercules_bot`)
...
Add these lines to `~/.hercules/.env`:
...
```
[Gateway] Starting Hercules Gateway...
...
# View live logs
journalctl --user -u hercules-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `hercules gateway install` to update the plist.
:::
...
```bash
hercules gateway stop && hercules gateway start
```
...
hercules pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.hercules/.env`:
...
Control how much detail the bot shows when using tools. In `~/.hercules/config.yaml`:
...
Customize how the bot communicates by editing `~/.hercules/SOUL.md`:

For a full guide, see [Use SOUL.md with Hercules](/guides/use-soul-with-hercules).
...
```bash
# From the CLI
hercules cron list          # View all scheduled jobs
hercules cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hercules-gateway` (Linux) or `~/.hercules/logs/gateway.log` (macOS) |
| Cron job output | `~/.hercules/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.hercules/cron/jobs.json` |
| Pairing data | `~/.hercules/pairing/` |
| Session history | `~/.hercules/sessions/` |
