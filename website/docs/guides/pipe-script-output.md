...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `hercules send`."
---

# Pipe Script Output to Messaging Platforms

`hercules send` is a small, scriptable CLI that pushes a message to any
messaging platform Hercules is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | hercules send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `hercules
...
hercules send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
hercules send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
hercules send --list

# Filter by platform
hercules send --list telegram
```
...
Any platform Hercules ships adapters for works as a target:
...
`hercules send` resolves the message body in this order:

1. **Positional argument** — `hercules send --to telegram "hi"`
2. **`--file PATH`** — `hercules send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | hercules send --to telegram`

When stdin is a TTY (no pipe), Hercules does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  hercules send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `hercules send` reuses your Hercules config, the same script works on
any host where Hercules is installed — no need to export bot tokens into
...
`hercules send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  hercules send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | hercules send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.hercules/bin/hercules send \
...
```bash
./train.py --epochs 200 && \
  hercules send --to telegram "training done" || \
  hercules send --to telegram "training failed (exit $?)"
```
...
hercules send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(hercules send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `hercules send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `hercules send` calls
the platform's REST endpoint directly using credentials from
`~/.hercules/.env` and `~/.hercules/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `hercules gateway start` and retry.
...
# Machine-readable
hercules send --list --json
```

The listing is built from `~/.hercules/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`hercules gateway
...
| Approach | Multi-platform | Reuses Hercules creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `hercules send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`hercules send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `hercules send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `hercules send` shares with cron delivery.
