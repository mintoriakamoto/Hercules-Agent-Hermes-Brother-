...
- **boto3** — install with `cd ~/.hercules/hercules-agent && uv pip install -e ".[bedrock]"`
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration — Hercules detects the instance role automatically.
:::
...
cd ~/.hercules/hercules-agent && uv pip install -e ".[bedrock]"
...
Hercules auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
Bedrock models use **inference profile IDs** for on-demand invocation. The `hercules model` picker shows these automatically, with recommended models at the top:
...
Bedrock works with all Hercules gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
Hercules checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. Hercules automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).

## One-Click AWS Deployment

For a fully automated deployment on EC2 with CloudFormation:

**[sample-hercules-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-hercules-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
