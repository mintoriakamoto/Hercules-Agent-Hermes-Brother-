...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Hercules tool
- you want Hercules to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Hercules to internal APIs, databases, or company systems without modifying Hercules core

Do not use MCP when:
- a built-in Hercules tool already solves the job well
...
- Hercules remains the agent
- MCP servers contribute tools
- Hercules discovers those tools at startup or reload time
...
If you installed Hercules with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Hercules banner/status should show MCP integration when configured
- ask Hercules what tools it has available
...
## WSL2: bridge Hercules in WSL to Windows Chrome

This is the practical setup when:

- Hercules runs inside WSL2
...
In this setup, Hercules does **not** connect to Chrome directly. Instead:

- Hercules runs in WSL
- Hercules starts a local stdio MCP server
...
```text
Hercules (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Hercules stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Hercules core browser transport
...
```bash
hercules mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
hercules mcp test chrome-devtools-win
```

Then start a fresh Hercules session or run:
...
Once loaded, Hercules can use the MCP-prefixed browser tools directly. For example:
...
If Hercules runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Hercules from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Hercules from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Hercules:
...
2. Hercules-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Hercules will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Hercules to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Hercules to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Hercules remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
hercules mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
hercules mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `hercules mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Hercules to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Hercules to merge, publish, deploy, or spawn runtimes.
...
Now Hercules can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Hercules core.
...
Because Hercules now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
