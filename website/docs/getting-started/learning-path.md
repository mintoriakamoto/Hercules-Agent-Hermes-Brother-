...
:::tip Start Here
If you haven't installed Hercules Agent yet, begin with the [Installation guide](/getting-started/installation) and then run through the [Quickstart](/getting-started/quickstart). Everything below assumes you have a working installation.
:::

:::tip First-time provider setup
First-time users almost always want OpenRouter — a single `OPENROUTER_API_KEY` covers 300+ models. Add it to `~/.hercules/.env` and run `hercules model` to pick one. See [OpenRouter](/integrations/providers#openrouter).
:::
...
7. [Use Voice Mode with Hercules](/guides/use-voice-mode-with-hercules)
...
2. [Build a Hercules Plugin](/developer-guide/plugins)
...
page is for built-in Hercules core development, not the usual user/custom-tool path.
:::

### "I want to train models"

Use reinforcement learning to fine-tune model behavior with Hercules Agent's RL training pipeline (powered by [Atropos](https://github.com/NousResearch/atropos)).
...
| **Code Execution** | Run Python scripts that call Hercules tools programmatically | [Code Execution](/user-guide/features/code-execution) |
