---
title: 上下文压缩与缓存
description: Hercules Agent 如何通过双重压缩系统和 Anthropic prompt 缓存高效管理上下文窗口。
---

# 上下文压缩与缓存

Hercules Agent 使用双重压缩系统和 Anthropic prompt（提示词）缓存，在长对话中高效管理上下文窗口用量。
...
通过 `hercules plugins` → Provider Plugins → Context Engine 进行配置，或直接编辑 `config.yaml`。

关于构建上下文引擎插件，请参阅 [Context Engine 插件](/developer-guide/context-engine-plugin)。

## 双重压缩系统

Hercules 有两个独立运行的压缩层：
...
Anthropic 每次请求最多允许 4 个 `cache_control` 断点。Hercules 使用"system_and_3"策略：
