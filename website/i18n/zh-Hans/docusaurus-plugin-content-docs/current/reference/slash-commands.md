...
**内联跳过：** 追加 `now`、`--yes` 或 `-y` 可为单次调用绕过弹窗——例如 `/reset now`、`/new --yes my-session`、`/clear -y`、`/undo -y`。适用于弹窗在你的终端无法正常渲染的情况（见 [issue #30768](https://github.com/mintoriakamoto/Hercules-Agent-Hermes-Brother-/issues/30768)，原生 Windows PowerShell）或对 CLI 进行脚本化操作时。
