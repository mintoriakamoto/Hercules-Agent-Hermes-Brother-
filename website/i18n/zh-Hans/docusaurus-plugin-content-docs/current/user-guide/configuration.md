...
不设置这些值将保持旧版默认值（`HERCULES_API_TIMEOUT=1800`s、`HERCULES_API_CALL_STALE_TIMEOUT=90`s、原生 Anthropic 900s）。隐式的非流式 stale 检测会在本地端点上自动禁用，并且会在超大上下文下自动放宽。目前不适用于 AWS Bedrock（`bedrock_converse` 和 AnthropicBedrock SDK 路径均使用 boto3 及其自身的超时配置）。请参阅 [`cli-config.yaml.example`](https://github.com/NousResearch/hercules-agent/blob/main/cli-config.yaml.example) 中的注释示例。
