...
| 相关 skill | [`native-mcp`](/user-guide/skills/bundled/mcp/mcp-native-mcp), [`ascii-video`](/user-guide/skills/bundled/creative/creative-ascii-video), [`manim-video`](/user-guide/skills/bundled/creative/creative-manim-video), `hercules-video` |
...
3. 将 `twozero_td` MCP 服务器添加到 Hercules 配置（如果缺失）
...
3. **重启 Hercules 会话**以加载新的 MCP 服务器
