...
- 点击"New SSH key"
- 粘贴公钥内容
- 填写标题，如"hercules-agent-&lt;machine-name>"
...
elif [ -f ~/.hercules/.env ] && grep -q "^GITHUB_TOKEN=" ~/.hercules/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.hercules/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
