...
- 用户希望 **Hercules 本身驱动浏览器** → 使用 Hercules 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
