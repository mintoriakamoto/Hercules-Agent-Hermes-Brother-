...
Hercules Agent 自动将每次对话保存为一个 session。Session 支持对话恢复、跨 session 搜索以及完整的对话历史管理。

## Session 的工作原理

每次对话——无论来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Teams 还是其他任何消息平台——都会以完整消息历史的形式存储为一个 session。Session 记录在：

1. **SQLite 数据库**（`~/.hercules/state.db`）——包含 FTS5 全文搜索的结构化 session 元数据，以及完整消息历史
...
Hercules 存储 session 历史以便恢复对话，但不会在每次对话时重新发送所有历史字节。每轮对话中，模型看到的是：所选系统 prompt、当前对话窗口，以及 Hercules 为该轮显式注入的内容。
...
例如，如果用户发送一张图片并要求 Hercules 制作表情包，Hercules 可能会用视觉能力检查该图片一次并运行图像处理脚本。后续轮次不会自动将原始 JPEG 带入上下文，只携带写入对话的内容，例如用户的请求、简短的图片描述、本地缓存路径或最终的助手回复。

上下文增长最常见的原因不是媒体文件本身，而是冗长的文本：粘贴的转录、完整日志、大型工具输出、长 diff、重复的状态报告以及详细的证明转储。优先使用摘要、文件路径、重点摘录和工具支持的查找，而不是将大型内容复制到聊天中。

:::tip
当 session 变长时使用 `/compress`，用 `/new` 开启新线程，仅在需要从存储中删除旧的已结束 session 时才使用 `hercules sessions prune`。压缩会减少活跃上下文，而不是隐私删除。向 `/new` 传入名称（例如 `/new payments-refactor`）可以预先设置新 session 的初始标题——便于之后通过 `/resume <name>` 或 `/sessions` 选择器找到它。
:::
...
| 来源 | 描述 |
|--------|-------------|
| `cli` | 交互式 CLI（`hercules` 或 `hercules chat`） |
...
# 会自动恢复最新的一个
hercules -c "my project"   # → 恢复 "my project #3"
```
...
Session ID 在退出 CLI session 时显示，也可通过 `hercules sessions list` 查找。

### 恢复时的对话摘要

恢复 session 时，Hercules 会在输入提示符前以样式化面板显示之前对话的紧凑摘要：

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="恢复 Hercules session 时显示的「上次对话」摘要面板的样式化预览。" />
...
要禁用摘要并保留最简单的单行行为，在 `~/.hercules/config.yaml` 中设置：
...
**恢复到 CLI：** 当你想回到桌面时，只需运行 `/resume <title>`（或在 shell 中运行 `hercules -r "<title>"`），从平台停止的地方继续。
...
Hercules 在第一次交换后自动为每个 session 生成简短的描述性标题（3–7 个词）。这在后台线程中使用快速辅助模型运行，不增加延迟。浏览 `hercules sessions list` 或 `hercules sessions browse` 时可以看到自动生成的标题。
...
当 session 的上下文被压缩（通过 `/compress` 手动或自动触发）时，Hercules 会创建一个新的续接 session。如果原 session 有标题，新 session 会自动获得带编号的标题：

```
"my project" → "my project #2" → "my project #3"
```

按名称恢复时（`hercules -c "my project"`），会自动选取谱系中最新的 session。
...
Hercules 通过 `hercules sessions` 提供完整的 session 管理命令集：
...
`hercules sessions export` 是所有导出格式的统一入口，用 `--format` 选择：
...
`--format trace` 生成 Claude Code JSONL — Hugging Face Hub 的 [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces) 可自动识别的转录格式。可以写入本地文件，或加 `--upload` 推送到你自己的私有 `hercules-traces` 数据集（读取 `HF_TOKEN`）：
...
当你想在隐藏或删除旧 session 之前保留一份可读的文件归档时，传入 `--format md` 或 `--format qmd`。Markdown/QMD 导出会为每个 session 写入一个文件到目录中（默认：`~/.hercules/session-exports`）。
...
# 仅清理特定平台的 session
hercules sessions prune --source telegram --older-than 60
...
如需更深入的分析——token 用量、费用估算、工具分解和活动模式——请使用 [`hercules insights`](/reference/cli-commands#hercules-insights)。
...
当 Hercules 无法获取共享聊天的参与者标识符时，回退为该房间共享一个 session。

### 共享与隔离的群组 Session

默认情况下，Hercules 在 `config.yaml` 中使用 `group_sessions_per_user: true`。这意味着：

- Alice 和 Bob 可以在同一个 Discord 频道中与 Hercules 对话，而不共享对话历史
...
| 内容 | 路径 | 描述 |
|------|------|-------------|
| SQLite 数据库 | `~/.hercules/state.db` | 所有 session 元数据 + 带 FTS5 的消息 |
| Gateway 消息 | `~/.hercules/state.db` | SQLite——所有 session 消息的权威存储 |
| Gateway 路由索引 | `~/.hercules/sessions/sessions.json` | 将 session 键映射到活跃 session ID（来源元数据、过期标志） |

SQLite 数据库使用 WAL 模式支持并发读取和单写入，非常适合 gateway 的多平台架构。

:::note 遗留 JSONL 对话记录
在 state.db 成为权威存储之前创建的 session 可能在 `~/.hercules/sessions/` 中留有
`*.jsonl` 文件。Hercules 不再写入或读取这些文件。在确认对应 session 存在于
state.db 后可安全删除。
:::
...
- 清理最多每 `sessions.min_interval_hours`（默认 24）小时运行一次；上次运行时间戳记录在 `state.db` 内部，因此在同一 `HERCULES_HOME` 下的所有 Hercules 进程间共享

默认为**关闭**——session 历史对 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.hercules/config.yaml` 中启用：
...
:::tip
数据库增长缓慢（典型情况：数百个 session 约 10–15 MB），session 历史为跨历史对话的 `session_search` 召回提供支持，因此自动清理默认关闭。如果你运行繁重的 gateway/cron 工作负载且 `state.db` 明显影响性能（已观察到的故障模式：约 1000 个 session 的 384 MB state.db 导致 FTS5 插入和 `/resume` 列表变慢），则启用它。使用 `hercules sessions prune` 进行一次性清理，无需开启自动清理。
:::