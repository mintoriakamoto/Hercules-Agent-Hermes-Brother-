...
description: "将 Hercules 连接到 GitHub，使其自动获取 PR diff、审查代码变更并发布评论——由 webhook 触发，无需手动提示"
---

# 使用 Webhook 自动发布 GitHub PR 评论

本指南介绍如何将 Hercules Agent 连接到 GitHub，使其自动获取 pull request 的 diff、分析代码变更并发布评论——由 webhook 事件触发，无需手动 prompt（提示词）。

当 PR 被打开或更新时，GitHub 会向你的 Hercules 实例发送一个 webhook POST 请求。Hercules 使用一个 prompt 运行 agent，该 prompt 指示其通过 `gh` CLI 获取 diff，并将响应发布回 PR 线程。
...
- Hercules Agent 已安装并运行（`hercules gateway`）
- [`gh` CLI](https://cli.github.com/) 已安装并在 gateway 主机上完成认证（`gh auth login`）
- 你的 Hercules 实例有一个可公网访问的 URL（如果在本地运行，请参阅[使用 ngrok 进行本地测试](#local-testing-with-ngrok)）
...
在你的 `~/.hercules/config.yaml` 中添加以下内容：
...
:::note Payload 中不包含代码
GitHub webhook payload 包含 PR 元数据（标题、描述、分支名、URL），但**不包含 diff**。上方的 prompt 指示 agent 运行 `gh pr diff` 来获取实际变更。`terminal` 工具已包含在默认的 `hercules-webhook` 工具集中，无需额外配置。
:::
...
创建一个分支，推送一个变更，并打开一个 PR。在 30–90 秒内（取决于 PR 大小和模型），Hercules 应该会发布一条审查评论。
...
如果 Hercules 在你的笔记本上运行，使用 [ngrok](https://ngrok.com/) 将其暴露到公网：
...
:::note
`hercules webhook test <name>` 仅适用于通过 `hercules webhook subscribe` 创建的**动态订阅**。它不读取 `config.yaml` 中的路由。
:::
...
加载一个 [Hercules skill](/user-guide/features/skills) 以赋予 agent 一致的审查风格。在 `config.yaml` 的 `platforms.webhook.extra.routes` 中，向你的路由添加 `skills`：
...
> **注意：** 列表中只有第一个找到的 skill 会被加载。Hercules 不会叠加多个 skill——后续条目会被忽略。
...
同一适配器也适用于 GitLab。GitLab 使用 `X-Gitlab-Token` 进行认证（纯字符串匹配，非 HMAC）——Hercules 会自动处理两者。
...
GitLab 的 payload 字段与 GitHub 不同——例如，MR 标题使用 `{object_attributes.title}`，MR 编号使用 `{object_attributes.iid}`。发现完整 payload 结构最简单的方式是使用 GitLab webhook 设置中的 **Test** 按钮，结合 **Recent Deliveries** 日志。或者，在路由配置中省略 `prompt`——Hercules 将把完整 payload 作为格式化 JSON 直接传递给 agent，agent 的响应（在 gateway 日志中通过 `deliver: log` 可见）将描述其结构。
